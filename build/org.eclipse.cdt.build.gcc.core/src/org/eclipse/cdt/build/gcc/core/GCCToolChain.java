...
public class GCCToolChain extends PlatformObject implements IToolChain {
	...
	/**
	 * Pattern to handle gcc (CC) toolchain filename. Following forms supported:
	 * Optional cross-compiler prefix, base name (gcc), optional -number version suffix, optional .exe file extension
	 * For example:
	 *   gcc, gcc.exe, gcc-18, arm-none-eabi-gcc-19.exe
	...
	/**
	 * Pattern to handle g++ (CXX) toolchain filename. Following forms supported:
	 * Optional cross-compiler prefix, base name (g++), optional -number version suffix, optional .exe file extension
	 * For example:
	 *   g++, g++.exe, g++-18, arm-none-eabi-g++-19.exe
	 * Use {@code matcher.group(0)} to retrieve the full filename.
	 */
	...
	private IEnvironmentVariable[] addCompilerOverrides(Path pathToToolChain, IEnvironmentVariable[] envVars2) {
		...
		if (envVars2 != null) {
			envVarsNew.addAll(Arrays.asList(envVars2));
		}
	 ...
	 *   Supports gcc toolchains, with optional cross prefix and optional version suffix.
	...
	private static IPath mapCompilerPath(IPath pathToToolChain, String desiredType) {
		if (!CC.equals(desiredType) && !CXX.equals(desiredType)) {
			return null; // Invalid desired compiler type
		}
		...
		if (isCXX && CC.equals(desiredType)) {
			// Get the filename part (group0) and just swap the g++ part with gcc.
			String mappedFilename = cxxMatcher.group(0).replaceFirst("g\\+\\+", "gcc"); //$NON-NLS-1$ //$NON-NLS-2$
			return pathToToolChain.removeLastSegments(1).append(mappedFilename);
		}
		if (isCC && CXX.equals(desiredType)) {
			// Get the filename part (group0) and just swap the gcc part with g++.
			String mappedFilename = ccMatcher.group(0).replaceFirst("gcc", "g\\+\\+"); //$NON-NLS-1$ //$NON-NLS-2$
			return pathToToolChain.removeLastSegments(1).append(mappedFilename);
		}

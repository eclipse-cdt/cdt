...
public class GCCToolChain extends PlatformObject implements IToolChain {
	...
	/**
	 * Pattern to handle gcc toolchain filename.
	 * Optional cross-compiler prefix, base name gcc, optional -number version suffix, optional .exe file extension.
	 * group(0): the whole match, eg: arm-none-eabi-gcc-19.exe
	 * group(1): optional prefix, eg: arm-none-eabi-
	 * group(2): optional version, digits only, eg: 19
	 * group(3): optional extension, eg: .exe
	 */
	private static final Pattern CC_PATTERN = Pattern.compile("^([\\w\\-]*?)gcc(?:-(\\d+))?(\\.exe)?$"); //$NON-NLS-1$
	/**
	 * Pattern to handle g++ toolchain filename.
	 */
	private static final Pattern CXX_PATTERN = Pattern.compile("^([\\w\\-]*?)g\\+\\+(?:-(\\d+))?(\\.exe)?$"); //$NON-NLS-1$
	 ...
	 * @param pathToToolChain Path to the filename of the gcc or g++ compiler. It may or may not have a cross prefix.
	 *   It may or may not have a version suffix. It may or may not have an extension.
	 ...
	 *   Supports gcc toolchains, with optional cross prefix and version suffix.
	 ...
	 * When {@link #CC}, g++ or g++-version is converted to gcc or gcc-version.
	 * When {@link #CXX}, gcc or gcc-version is converted to g++ or g++-version.
	 *
	 * For example:
	 *   gcc, gcc-18, arm-none-eabi-gcc, arm-none-eabi-gcc-19
	 * converts to:
	 *   g++, g++-18, arm-none-eabi-g++, arm-none-eabi-g++-19
	 *
	...
	private static IPath mapCompilerPath(IPath pathToToolChain, String desiredType) {
		...
		if (isCC || isCXX) {
			// Compiler is either gcc or g++
			String mappedFilename;
			if (isCXX && CC.equals(desiredType)) {
				// Compiler is CPP type and desired type is C
				String prefix = cxxMatcher.group(1) != null ? cxxMatcher.group(1) : EMPTY;
				String version = cxxMatcher.group(2) != null ? "-" + cxxMatcher.group(2) : EMPTY; //$NON-NLS-1$
				String ext = cxxMatcher.group(3) != null ? cxxMatcher.group(3) : EMPTY;
				mappedFilename = prefix + "gcc" + version + ext; //$NON-NLS-1$
			...
			if (isCC && CXX.equals(desiredType)) {
				...
				String prefix = ccMatcher.group(1) != null ? ccMatcher.group(1) : EMPTY;
				String version = ccMatcher.group(2) != null ? "-" + ccMatcher.group(2) : EMPTY; //$NON-NLS-1$
				String ext = ccMatcher.group(3) != null ? ccMatcher.group(3) : EMPTY;
				mappedFilename = prefix + "g++" + version + ext; //$NON-NLS-1$
				return pathToToolChain.removeLastSegments(1).append(mappedFilename);
			} else {

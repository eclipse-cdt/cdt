...
import static org.eclipse.cdt.cmake.core.tests.CMakeServerTestUtils.waitUntil;
...
public class CMakeServerTest {
	...
	final static private String generator = "Unix Makefiles";
	final static private String compileFlags = "-g -O2";
	final static private String minimumCMakeVersion = "3.7";

	@BeforeClass
	public static void showCMakeVersions() {
		System.out.println("Minimum required CMake version is " + minimumCMakeVersion);
		CMakeServerTestUtils.getCMakeInSystemPath()
				.forEach(cmakeExe -> System.out.format("CMakeServerTest.setup(): %s is version %s%n", cmakeExe,
						CMakeServerTestUtils.getCMakeVersion(cmakeExe).get()));
	}

	@Before
	public void setup() throws CMakeServerException {
		Optional<File> hasCMakeExe = CMakeServerTestUtils.getCompatibleCMakeFromSystemPath(minimumCMakeVersion);
		assumeTrue(String.format("Skipping test, no CMake >= %s not found in PATH", minimumCMakeVersion),
				hasCMakeExe.isPresent());
	...
	public void setupSources() throws IOException {
		...
		Files.write(f.toPath(), Arrays.asList(//
				...
				String.format("cmake_minimum_required(VERSION %s)", minimumCMakeVersion), //

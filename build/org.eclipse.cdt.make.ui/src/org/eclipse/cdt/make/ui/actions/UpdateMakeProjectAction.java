...
public class UpdateMakeProjectAction implements IWorkbenchWindowActionDelegate {
	...
	protected static void doProjectUpdate(IProgressMonitor monitor, IProject[] project) throws CoreException {
		SubMonitor progress = SubMonitor.convert(monitor, MakeUIPlugin.getResourceString("UpdateMakeProjectAction.monitor.update"), project.length * 4); //$NON-NLS-1$
		try {
			for (int i = 0; i < project.length; i++) {
				// remove old builder
				project[i].refreshLocal(IResource.DEPTH_ONE, progress.split(1));
				...
				// add new nature
				MakeProjectNature.addNature(project[i], progress.split(1));
				...
				project[i].accept(new TargetConvertVisitor(progress.newChild(1)), 0);
				progress.worked(1);
			}
		} finally {
			monitor.done();

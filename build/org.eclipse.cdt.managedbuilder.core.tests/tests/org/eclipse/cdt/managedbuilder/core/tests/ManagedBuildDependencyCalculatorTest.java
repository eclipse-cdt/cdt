...
public class ManagedBuildDependencyCalculatorTest extends TestCase {
	private IPath resourcesLocation = new Path(
			CTestPlugin.getFileInPlugin(new Path("resources/depCalcProjects/")).getAbsolutePath());

	public ManagedBuildDependencyCalculatorTest(String name) {
		super(name);
	}

	public static Test suite() {
		TestSuite suite = new TestSuite(ManagedBuildDependencyCalculatorTest.class.getName());

		suite.addTest(new ManagedBuildDependencyCalculatorTest("test1DepCalc2"));
		suite.addTest(new ManagedBuildDependencyCalculatorTest("test1DepCalc3"));
		suite.addTest(new ManagedBuildDependencyCalculatorTest("test1DepCalcPreBuild"));
		return suite;
	}

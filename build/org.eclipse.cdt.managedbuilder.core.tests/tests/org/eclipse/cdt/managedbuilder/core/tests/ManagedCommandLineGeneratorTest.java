...
public class ManagedCommandLineGeneratorTest extends TestCase {
	...
	public final void testCustomOptionCommandGenerator2() {
		try {
			...
			assertEquals(Arrays.asList(libs).toString(), Arrays.asList(tool.getLibs()).toString());
			...
			assertEquals(Arrays.asList(userObjs).toString(), Arrays.asList(tool.getUserObjs()).toString());

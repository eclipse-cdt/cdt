...
public class CommandBuilder implements IBuildModelBuilder {
	...
	public int build(OutputStream out, OutputStream err, IProgressMonitor monitor) {
		...
		try {
			...
			SubMonitor progress = SubMonitor.convert(monitor, getNumCommands());
			progress.subTask(ManagedMakeMessages.getResourceString("MakeBuilder.Invoking_Command") + getCommandLine()); //$NON-NLS-1$

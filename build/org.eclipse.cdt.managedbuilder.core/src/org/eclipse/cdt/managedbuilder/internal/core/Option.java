				...
				if (parent instanceof ToolChain) {
					...
					if (foundOption != null) {
						...
						ManagedBuilderCorePlugin
								.log(new Status(IStatus.ERROR, ManagedBuilderCorePlugin.PLUGIN_ID, MessageFormat
										.format("Missing superclass for \"{0}\" for \"{1}\"", superClassId, getId()))); //$NON-NLS-1$
					}
				} else {
					ManagedBuilderCorePlugin.log(new Status(IStatus.ERROR, ManagedBuilderCorePlugin.PLUGIN_ID,
							MessageFormat.format("Missing superclass for \"{0}\" for \"{1}\"", superClassId, getId()))); //$NON-NLS-1$
				}
			}
		}

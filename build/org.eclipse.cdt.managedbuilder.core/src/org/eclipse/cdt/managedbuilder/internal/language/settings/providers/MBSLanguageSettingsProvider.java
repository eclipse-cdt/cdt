							...
							if ((kindsBits & kind) != 0) {
								...
								for (ICLanguageSettingEntry entry : additions) {
									if (entry instanceof ICPathEntry) {
										// have to use getName() rather than getLocation() and not use IPath
										// operations to avoid collapsing ".."
										String pathStr = ((ICPathEntry) entry).getName();
										if (!new Path(pathStr).isAbsolute()) {
											// We need to add project-rooted entry for relative path as MBS
											// counts it this way in some UI
											// The relative entry below also should be added for indexer to
											// resolve from source file locations

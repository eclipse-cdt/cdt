...
public class CatchByReferenceQuickFix extends AbstractAstRewriteQuickFix {
	...
	public void modifyAST(IIndex index, IMarker marker) {
		...
		applyConst(declaration, rewrite);
		applyReference(declaration, rewrite);
	...
	/**
	 * Determine if the new reference type should be const-qualified.
	 */
	protected boolean shouldDeclareConst() {
		return false;
	}

	private void applyConst(IASTSimpleDeclaration declaration, ASTRewrite rewrite) {
		IASTDeclSpecifier declSpecifier = declaration.getDeclSpecifier();
		if (shouldDeclareConst() && !declSpecifier.isConst()) {
			IASTDeclSpecifier replacement = declSpecifier.copy(CopyStyle.withLocations);
			replacement.setConst(true);
			rewrite.replace(declSpecifier, replacement, null);
		}
	}

	private void applyReference(IASTSimpleDeclaration declaration, ASTRewrite rewrite) {
		...
		rewrite.replace(declarator, replacement, null);
	}

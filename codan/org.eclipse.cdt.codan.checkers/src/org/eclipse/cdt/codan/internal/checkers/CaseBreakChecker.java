	...
	class SwitchFindingVisitor extends ASTVisitor {
		...
		public boolean hasValidFallthroughAttribute(IASTStatement statement) {
			...
			if (statement instanceof IASTNullStatement)
				...
				if (lastnode instanceof IASTNullStatement)
					nullstatement = (IASTStatement) lastnode;
			}
			if (nullstatement != null) {
				if (AttributeUtil.hasAttribute(nullstatement, new String[] { StandardAttributes.FALLTHROUGH }))
					return true;
			}
			return false;
		}

	...
	class FunctionCallVisitor extends ASTVisitor {
		...
		public int visit(IASTExpression expr) {
			if (!shouldReportInMacro() && enclosedInMacroExpansion(expr))
				return PROCESS_SKIP;
			if (isDiscardedValueExpression(expr) && isCallToNodiscardFunction(expr, res)) {
				reportProblem(ER_ID, expr, res[0]);
				res[0] = null;
			}
		...
		private boolean isDiscardedValueExpression(IASTExpression expr) {
			...
			if (expr.getParent() instanceof IASTExpressionStatement)
				return true;

			if (checkNestedLeftSide(expr)) {
				return true;
			}
		...
		private boolean isCallToNodiscardFunction(IASTExpression expr, IASTNode[] res) {

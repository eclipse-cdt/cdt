	...
	class FunctionCallVisitor extends ASTVisitor {
		...
		public int visit(IASTExpression expr) {
			...
			if (expr instanceof IASTFunctionCallExpression) {
				...
				if (checkNestedRightSide(func, binding, name)) {
					return PROCESS_CONTINUE;
				}
				...
				if (checkEvaluation(cast)) {
					reportProblem(ER_ID, expr, cast.getTypeId());
				}
			}
			return PROCESS_CONTINUE;
		}
		...
		/**
		 * Check if the function call is a right side of a nested expression. If it's a
		 ...
		 * @param name The name
		 * @return True if it's a nested expression and the func call is the right side, false otherwise
		 */
		private boolean checkNestedRightSide(IASTFunctionCallExpression func, IBinding binding, IASTName name) {
			if (func.getPropertyInParent() == IASTExpressionList.NESTED_EXPRESSION) {
				...
				if (func == allExpr[allExpr.length - 1])
					return true;
			}
			return false;
		}

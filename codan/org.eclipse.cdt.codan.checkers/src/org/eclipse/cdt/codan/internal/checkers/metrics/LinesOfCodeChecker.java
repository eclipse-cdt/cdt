...
public class LinesOfCodeChecker extends AbstractAstFunctionChecker {
	...
	protected void processFunction(IASTFunctionDefinition func) {
		...
		final String rawSignature = body.getRawSignature();
		...
		if (!countFuncBodyTokens && token.getType() == IToken.tLBRACE) {
			token = token.getNext();
		}
		...
		if (usedLines > this.maxLines) {
			reportProblem(ER_LINES_OF_CODE_EXCEEDED_ID, func, usedLines);
		}
	}
}

...
static bool createNamedEvent(EventInfo_t *eventInfo, BOOL manualReset, const wchar_t *prefix, int pid, int counter) {
    wchar_t buffer[50];
    swprintf(buffer, sizeof(buffer) / sizeof(buffer[0]), L"%s%04x%08x", prefix, pid, counter);

    HANDLE event = CreateEventW(NULL, manualReset, FALSE, buffer);
    if (!event || GetLastError() == ERROR_ALREADY_EXISTS) {
        return false;
    }

    eventInfo->handle = event;
    eventInfo->name = wcsdup(buffer);
    return true;
}
...
extern "C"
    ...
    if (!createStandardNamedPipe(stdHandles, STD_INPUT_HANDLE, pid, nLocalCounter) ||
        !createStandardNamedPipe(stdHandles, STD_OUTPUT_HANDLE, pid, nLocalCounter) ||
        !createStandardNamedPipe(stdHandles, STD_ERROR_HANDLE, pid, nLocalCounter)) {

...
public class FileBasedErrorParserTest extends GenericErrorParserTest {

	File errorFile;

	public FileBasedErrorParserTest(File file) {
	...
	public static Test suite() {
		TestSuite suite = new TestSuite(FileBasedErrorParserTest.class.getName());
		...
		for (int i = 0; i < testsfiles.length; i++) {
			if (testsfiles[i].isFile())
				suite.addTest(new FileBasedErrorParserTest(testsfiles[i]));
		}
		return suite;
	}
}

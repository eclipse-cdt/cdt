...
public class AST2CPPAttributeTests extends AST2TestBase {
	...
	private class AttributeNodeFinder extends ASTVisitor {
		...
		private List<ICPPASTAttributeSpecifier> specifiers = new ArrayList<ICPPASTAttributeSpecifier>();

		public List<ICPPASTAttributeSpecifier> getAttributes() {
			return specifiers;
		}

		public int visit(IASTAttribute attribute) {
			if (attribute instanceof ICPPASTAttributeSpecifier) {
				specifiers.add((ICPPASTAttributeSpecifier) attribute);
			}
			return PROCESS_CONTINUE;
		}
	}

	private List<ICPPASTAttributeSpecifier> getAttributeSpecifiers(IASTTranslationUnit tu) {
		AttributeNodeFinder attributeFinder = new AttributeNodeFinder();
		tu.accept(attributeFinder);
		List<ICPPASTAttributeSpecifier> specifiers = attributeFinder.getAttributes();
		return specifiers;
	}

	private ICPPASTAttribute[] getAttributes() throws Exception {
		IASTTranslationUnit tu = parseAndCheckBindings();
		List<ICPPASTAttributeSpecifier> specifiers = getAttributeSpecifiers(tu);
		assertEquals(1, specifiers.size());
		ICPPASTAttributeSpecifier specifier = specifiers.get(0);
		ICPPASTAttribute[] attributes = specifier.getAttributes();
		return attributes;
	}

	private void checkAttributeRelations(List<ICPPASTAttributeSpecifier> specifiers, Class<? extends IASTAttributeOwner>...parentType) {
		for (int i = 0; i < specifiers.size(); i++) {
			ICPPASTAttributeSpecifier specifier = specifiers.get(i);
			IASTNode attributeParent = specifier.getParent();
			IASTAttributeOwner owner = assertInstance(attributeParent, parentType[i]);
			IASTAttribute[] ownerAttributes = owner.getAttributes();
	...
	public void testAttributedMultidimensionalNewArrayExpression() throws Exception {
		...
		IASTAttribute arrayModifierAttribute1 = specifiers.get(0);
		IASTNode arrayModifier1 = arrayModifierAttribute1.getParent();
		IASTAttribute arrayModifierAttribute2 = specifiers.get(1);
	...
	public void testAttributedPtrOperators() throws Exception {
		...
		IASTAttribute pointerAttribute1 = specifiers.get(0);
		IASTNode pointer1 = pointerAttribute1.getParent();
		IASTAttribute pointerAttribute2 = specifiers.get(1);
	...
	public void testEmptyAttributeSpecifier() throws Exception {
		ICPPASTAttribute[] attributes = getAttributes();
		assertEquals(ICPPASTAttribute.EMPTY_CPP_ATTRIBUTE_ARRAY, attributes);
	}
	...
	public void testMultipleAttributes() throws Exception {
		...
		ICPPASTAttribute attr1 = attributes[0];
		assertEquals("attr1", String.valueOf(attr1.getName()));
		ICPPASTAttribute attr2 = attributes[1];
	...
	public void testPackExpansionAttribute() throws Exception {
		...
		ICPPASTAttribute attribute = attributes[0];
		assertTrue(attribute.hasPackExpansion());
	}
	...
	public void testScopedAttribute() throws Exception {
		ICPPASTAttribute[] attributes = getAttributes();
		assertEquals(1, attributes.length);
		ICPPASTAttribute scopedAttribute = attributes[0];
		assertEquals("scope", String.valueOf(scopedAttribute.getScope()));

...
/**
 * Scanner2Tests ported to use the DOMScanner
 */
public class DOMScannerTests extends BaseTestCase {

	public static TestSuite suite() {
		return suite(DOMScannerTests.class);
	}

	private DOMScanner fScanner;
	...
	protected void validateLString(String expectedImage) throws Exception {
		validateToken(IToken.tLSTRING, expectedImage);
	}
	...
	protected void validateProblemCount(int count) throws Exception {
		ILocationResolver lm= fScanner.getLocationResolver();
	...
	private void addDefinition(String string, String string2) {
		fScanner.addDefinition(string.toCharArray(), string2.toCharArray());
	}
	...
	public void testBug45551() throws Exception {
		...
		IASTPreprocessorIncludeStatement[] includes= fScanner.getLocationResolver().getIncludeDirectives();
	...
	public void testBug69134() throws Exception {
		...
		writer
				.write("\"  # |Position|State|Mod|Lev|Action |Terminal and Lookahead or Rule\n\");\n"); 
		writer.write("yyNl ();\n"); 
		writer.write("}\n"); 
		writer.write("# endif\n"); 
		initializeScanner(writer.toString());
	...
	public void testBug73492() throws Exception {
		...
		assertEquals(offset + 5, t.getOffset());

		t = fScanner.nextToken();
		assertEquals(t.getType(), IToken.tSEMI);
		assertEquals(offset + 6, t.getOffset());
	}
	...
	public void testBug72537() throws Exception {
		...
		fScanner.addDefinition("FOO".toCharArray(), "foo".toCharArray()); 
		fScanner.addDefinition("BAR(x)".toCharArray(), "x".toCharArray()); 
	...
	public void testBug75083() throws Exception {
		...
		assertEquals(idx + 8, t.getEndOffset());

		t = fScanner.nextToken();
		assertEquals(t.getType(), IToken.tIDENTIFIER);
		assertEquals(idx + 9, t.getOffset());
		assertEquals(idx + 12, t.getEndOffset());
	}
	...
	public void _testBug75532IProblems() throws Exception {
		Writer writer = new StringWriter();
		...
		initializeScanner(writer.toString());
		fullyTokenize();
		IASTProblem[] problems= fScanner.getLocationResolver().getScannerProblems();
		assertEquals(16, problems.length);
		assertTrue(problems[0].getID() == IProblem.SCANNER_BAD_OCTAL_FORMAT);
		assertTrue(problems[1].getID() == IProblem.SCANNER_BAD_DECIMAL_FORMAT);
		assertTrue(problems[2].getID() == IProblem.SCANNER_BAD_HEX_FORMAT);
		assertTrue(problems[3].getID() == IProblem.SCANNER_BAD_HEX_FORMAT);
		assertTrue(problems[4].getID() == IProblem.SCANNER_DIVIDE_BY_ZERO);
		assertTrue(problems[5].getID() == IProblem.SCANNER_MISSING_R_PAREN);
		assertTrue(problems[6].getID() == IProblem.SCANNER_MISSING_R_PAREN);
		assertTrue(problems[7].getID() == IProblem.SCANNER_ILLEGAL_IDENTIFIER);
		assertTrue(problems[8].getID() == IProblem.SCANNER_BAD_CONDITIONAL_EXPRESSION);
		assertTrue(problems[9].getID() == IProblem.SCANNER_EXPRESSION_SYNTAX_ERROR);
		assertTrue(problems[10].getID() == IProblem.SCANNER_EXPRESSION_SYNTAX_ERROR);
		assertTrue(problems[11].getID() == IProblem.SCANNER_EXPRESSION_SYNTAX_ERROR);
		assertTrue(problems[12].getID() == IProblem.SCANNER_EXPRESSION_SYNTAX_ERROR);
		assertTrue(problems[13].getID() == IProblem.SCANNER_MISSING_R_PAREN);
		assertTrue(problems[14].getID() == IProblem.SCANNER_ASSIGNMENT_NOT_ALLOWED);
		assertTrue(problems[15].getID() == IProblem.SCANNER_BAD_OCTAL_FORMAT);
	}
	...
	public void testBug39688B() throws Exception { // test C99
		...
		assertTrue(new String(debug.getExpansion())
				.equals("fprintf(stderr, __VA_ARGS__)")); 

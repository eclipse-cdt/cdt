...
public abstract class AbstractBuiltinSpecsDetector extends AbstractLanguageSettingsOutputScanner
	...
	private class SDMarkerGenerator implements IMarkerGenerator {
		...
		public void addMarker(final ProblemMarkerInfo problemMarkerInfo) {
			...
			Job markerJob = new Job(CCorePlugin.getResourceString("AbstractBuiltinSpecsDetector.AddScannerDiscoveryMarkers")) { //$NON-NLS-1$
				@Override
				protected IStatus run(IProgressMonitor monitor) {
					...
					try {
						...
						return CCorePlugin.createStatus("Error checking markers.", e); //$NON-NLS-1$
					}
					...
					try {
						...
						if (problemMarkerInfo.file instanceof IWorkspaceRoot) {
							String msgPreferences = CCorePlugin.getFormattedString("AbstractBuiltinSpecsDetector.ScannerDiscoveryMarkerLocationPreferences", providerName); //$NON-NLS-1$
							marker.setAttribute(IMarker.LOCATION, msgPreferences);
						} else {
							String msgProperties = CCorePlugin.getFormattedString("AbstractBuiltinSpecsDetector.ScannerDiscoveryMarkerLocationProperties", providerName); //$NON-NLS-1$
							marker.setAttribute(IMarker.LOCATION, msgProperties);
						}
					} catch (CoreException e) {
						return CCorePlugin.createStatus("Error adding markers.", e); //$NON-NLS-1$
					}
		...
		public void deleteMarkers(IResource rc) {
			...
			try {
				...
				CCorePlugin.log("Error deleting markers.", e); //$NON-NLS-1$
			}
		}
	...
	protected void execute() {
		...
		WorkspaceJob job = new WorkspaceJob(CCorePlugin.getResourceString("AbstractBuiltinSpecsDetector.DiscoverBuiltInSettingsJobName")) { //$NON-NLS-1$
			@Override
			public IStatus runInWorkspace(IProgressMonitor monitor) throws CoreException {
				...
				try {
					...
					CCorePlugin.log(e);
					status = CCorePlugin.createStatus("Error running Builtin Specs Detector", e); //$NON-NLS-1$
				} finally {
					isExecuted = true;
					shutdown();
				}
	...
	protected IStatus runForEachLanguage(IProgressMonitor monitor) {
		MultiStatus status = new MultiStatus(CCorePlugin.PLUGIN_ID, IStatus.OK, "Problem running CDT Scanner Discovery provider " + getId(), null); //$NON-NLS-1$
		...
		try {
			...
			List<String> languageIds = getLanguageScope();
			if (languageIds != null) {
				monitor.beginTask(CCorePlugin.getResourceString("AbstractBuiltinSpecsDetector.ScannerDiscoveryTaskTitle"), //$NON-NLS-1$
				...
				monitor.subTask(CCorePlugin.getFormattedString("AbstractBuiltinSpecsDetector.ClearingMarkers",  markersResource.getFullPath().toString())); //$NON-NLS-1$
				...
				for (String languageId : languageIds) {
					...
					try {
						...
						IStatus s = CCorePlugin.createStatus("Error running Builtin Specs Detector", e); //$NON-NLS-1$
						CCorePlugin.log(s);
						status.merge(s);
					} finally {
						shutdownForLanguage();
			...
			monitor.subTask(CCorePlugin.getResourceString("AbstractBuiltinSpecsDetector.SerializingResults")); //$NON-NLS-1$
			...
			status.merge(CCorePlugin.createStatus("Error running Builtin Specs Detector", e)); //$NON-NLS-1$
	...
	private void runForLanguage(IProgressMonitor monitor) throws CoreException {
		...
		try {
			monitor.beginTask(CCorePlugin.getFormattedString("AbstractBuiltinSpecsDetector.RunningScannerDiscovery",  getName()), //$NON-NLS-1$
			...
			if (isConsoleEnabled) {
				...
				console = CCorePlugin.getDefault().getConsole(CCorePlugin.PLUGIN_ID + ".console.hidden"); //$NON-NLS-1$
			}
			...
			buildRunnerHelper.greeting(CCorePlugin.getFormattedString("AbstractBuiltinSpecsDetector.RunningScannerDiscovery",  getName())); //$NON-NLS-1$
			...
			CCorePlugin.log("Error running Builtin Specs Detector" , e); //$NON-NLS-1$
		} finally {
			try {
				buildRunnerHelper.close();
			} catch (IOException e) {
	...
	private IConsole startProviderConsole() {
		...
		if (isConsoleEnabled && currentLanguageId != null) {
			...
			if (ld != null) {
				String consoleId = CCorePlugin.PLUGIN_ID + '.' + getId() + '.' + currentLanguageId;
	...
	protected String getSpecFile(String languageId) {
		...
		// TODO migration from ManagedCorePlugin (if present)
		IPath workingLocation = CCorePlugin.getDefault().getStateLocation();
	...
	protected String getSpecFileExtension(String languageId) {
		...
		if (ext == null) {
			CCorePlugin.log("Unable to find file extension for language " + languageId); //$NON-NLS-1$
		}
		return ext;
	}
	...
	public void loadAttributes(Element providerNode) {
		...
		if (envPathHashStr != null) {
			try {
				...
				CCorePlugin.log("Wrong integer format [" + envPathHashStr + "]", e); //$NON-NLS-1$ //$NON-NLS-2$
			}
		}
	}

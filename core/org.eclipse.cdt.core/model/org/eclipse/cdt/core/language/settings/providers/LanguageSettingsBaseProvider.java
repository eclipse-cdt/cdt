 ...
 *
 * This implementation supports "static" list of entries for languages specified
...
public class LanguageSettingsBaseProvider extends AbstractExecutableExtensionBase
	 ...
	 *            - name of the provider to be presented to a user.
	 * @param languages
	 *            - list of languages the {@code entries} provided for.
	 *            {@code languages} can be {@code null}, in this case the
	 *            {@code entries} are provided for any language.
	 * @param entries
	 *            - the list of language settings entries this provider
	 *            provides. If {@code null} is passed, the provider creates an
	 *            empty list.
	 */
	 ...
	 * the extension point is done in 2 steps. First, the class is created as an
	 * executable extension using the default provider. Then this method is used
	 * to configure the provider. <br>
	 * <br>
	...
	/**
	 * {@code LanguageSettingsBaseProvider} keeps the list of key-value pairs so
	 * extenders of this class can customize the provider. The properties of
	 * {@code LanguageSettingsBaseProvider} come from the extension in
	 * plugin.xml although the extenders can provide their own method. <br>
	 * <br>
	 * Please note that empty string value is treated as "default" value and the
	 * same as {@code null} and the same as missing property, which allows
	 * {@link #equals(Object)} evaluate the property as equal while comparing
	 * providers.
	 *
	 * @param key
	 *            - property to check the value.
	 * @return value of the property. If the property is missing returns empty
	 *         string.
	 */
	 ...
	 * @param key
	 *            - property to check the value.
	 * @return boolean value of the property. If the property is missing or
	 *         cannot be interpreted as boolean returns {@code false}.
	 */
	 ...
	 *
	 * @param languageId
	 *            - language id. If {@code null}, then entries defined for the
	 *            language scope are returned. See {@link #getLanguageScope()}
	 *
	 * @return unmodifiable list of setting entries or {@code null} if no
	 *         settings defined. the list is internally pooled and guaranteed to
	 *         be the same object for equal lists.
	 * @since 6.0
	 */
	@Override
	...
	/**
	 * @return the unmodifiable list of languages this provider provides for. If

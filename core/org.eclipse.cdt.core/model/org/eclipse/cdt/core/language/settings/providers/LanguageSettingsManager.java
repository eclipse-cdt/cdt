...
public class LanguageSettingsManager {
	/**
	 * Returns the list of setting entries of a certain kind (such as include
	 * paths) for the given configuration description, resource and language.
	 * This is a combined list for all providers taking into account settings of
	 * parent folder if settings for the given resource are not defined. For
	 * include paths both local (#include "...") and system (#include <...>)
	 ...
	 *            and it is possible to specify composite kind. Use
	...
	/**
	 * Returns the list of setting entries of the given provider for the given
	 * configuration description, resource and language. This method reaches to
	 * the parent folder of the resource recursively if the resource does not
	 * define the entries for the given provider.
	 *
	 * @param provider
	 ...
	 *            - language id.
	 *
	 * @return the list of setting entries. Never returns {@code null} although
	 *         individual providers return {@code null} if no settings defined.
	 * @since 6.0
	 */
	 ...
	 * @return the workspace provider. If workspace provider is not defined a
	 ...
	 * provider could be replaced internally without need to change
	...
	/**
	 * Checks if the provider is a workspace level provider. This method is
	 * intended to check providers retrieved from a configuration. Raw providers
	 * from {@link #getRawProvider(ILanguageSettingsProvider)} are not
	 * considered as workspace providers.
	 *
	 * @param provider
	 *            - provider to check.
	 * @return {@code true} if the given provider is workspace provider,
	...
	/**
	 * Helper method to get to real underlying provider collecting entries as
	 * opposed to wrapper which is normally used for workspace provider.
	 * 
	 * @see LanguageSettingsProvidersSerializer#isWorkspaceProvider(ILanguageSettingsProvider)
	 *
	 * @param provider
	 *            - the provider to get raw provider for. Can be either
	 *            workspace provider or regular one.
	 * @return raw underlying provider for workspace provider or provider itself
	 *         if no wrapper is used.
	 */
	 ...
	 *
	 * @param providers
	 *            - array of user defined workspace providers. Note that those
	 *            providers will shadow extension providers with the same ID.
	 *            All not shadowed extension providers will be added to the list
	 *            to be present as workspace providers. {@code null} is
	 *            equivalent to passing an empty array and so will reset
	 *            workspace providers to match extension providers.
	...
	/**
	 * Copy language settings provider. It is different from clone() methods in
	 * that it does not throw {@code CloneNotSupportedException} but returns
	 * {@code null} instead.
	 *
	 * @param provider
	 *            - language settings provider to copy.
	 * @param deep
	 *            - {@code true} to request deep copy including copying settings
	 *            entries or {@code false} to return shallow copy with no
	 *            settings entries.
	 *
	 ...
	 * {@code org.eclipse.cdt.core.LanguageSettingsProvider}. A new copy of the
	 * extension provider is returned.
	 *
	 * @param id
	 ...
	 *         {@link ILanguageSettingsEditableProvider}) or {@code null} if
	 *         provider is not copyable.
	 */
	 ...
	 * @param deep
	 *            - {@code true} to check for deep equality testing also
	 *            settings entries or {@code false} to test shallow copy with no
	 *            settings entries.
	 * @return - {@code true} if the provider matches the extension or
	 *         {@code false} otherwise.
	 */
	 ...
	 * @return list of language IDs for the resource. The list can contain
	 *         {@code null} ID. Never returns {@code null} but empty list if no
	...
	/**
	 * Find language IDs for the resource in given build configuration. Under
	 * the hood build component is inquired and the language IDs would commonly
	 * come from the input type(s).
	 *
	 * @param resource
	 *            - the resource to find languages for.
	 * @param cfgDescription
	 * @return list of language IDs for the resource. Never returns {@code null}
	 ...
	 * {@code org.eclipse.cdt.core.LanguageSettingsProvider} extension point.
	 * <br>
	 * Note that only {@link ILanguageSettingsEditableProvider} can be owned by
	 * a configuration.
	 *
	 * @param id
	...
	/**
	 * Tells if language settings entries of the provider are persisted with the
	 * project (under .settings/ folder) or in workspace area. Persistence in
	 * the project area lets the entries migrate with the project.
	 *
	 * @param provider
	 *            - provider to check the persistence mode.
	 * @return {@code true} if LSE persisted with the project or {@code false}
	 *         if in the workspace.
	 */
	 ...
	 *            - {@code true} if with the project, {@code false} if in
	 *            workspace area.
	 */
	...
	/**
	 * Save language settings providers of the workspace (global providers) to
	 * persistent storage.
	 *
	 * @throws CoreException
	 *             if something goes wrong.
	...
	/**
	 * Save language settings providers of a project to persistent storage in a
	 * background job.
	 *
	 * @param prjDescription
	 *            - project description of the project.
	...
	/**
	 * Save language settings providers of the workspace (global providers) to
	 * persistent storage in a background job.
	 */
	...
	/**
	 * Create a list of providers with intention to assign to a configuration
	 * description.
	 * 
	 * The list will contain global providers for ones where attribute
	 * "prefer-non-shared" is {@code true} and a new copy for those where
	 * attribute "prefer-non-shared" is {@code false}. Attribute
	 * "prefer-non-shared" is defined in extension point
	 * {@code org.eclipse.cdt.core.LanguageSettingsProvider}.
	 * 
	 * @param ids

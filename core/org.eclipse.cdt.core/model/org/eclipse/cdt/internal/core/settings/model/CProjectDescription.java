...
public class CProjectDescription implements ICProjectDescription, ICDataProxyContainer {
	 ...
	 * fIsApplying => false setModified (false) set the ICSettingsStorage to
	 * readonly
	 */
	...
	/**
	 * Convert the current CConfigurationDescriptions to cached versions This
	 * occurs during the SetCProjectDescription Operation
	 */
	...
	void configurationCreated(ICConfigurationDescription des) {
		...
		fCfgMap.put(id, des);
		try {
			fProject.getBuildConfig(id);
		} catch (CoreException e) {
			if (e.getStatus().getCode() == IResourceStatus.BUILD_CONFIGURATION_NOT_FOUND) {
				try {
					Set<String> configNames = new HashSet<>();
					for (IBuildConfiguration config : fProject.getBuildConfigs()) {
						configNames.add(config.getName());
					}
					configNames.add(id);

					IProjectDescription projectDesc = fProject.getDescription();
					projectDesc.setBuildConfigs(configNames.toArray(new String[configNames.size()]));
					fProject.setDescription(projectDesc, new NullProgressMonitor());
	...
	public ICStorageElement getRootStorageElement() throws CoreException {
		...
		// if (fRootStorageElement == null) {
		// fRootStorageElement =
		// CProjectDescriptionManager.getInstance().createStorage(fProject,
	...
	ICSettingsStorage getStorageBase() throws CoreException {
		if (fStorage == null)
			// fStorage = new CStorage((InternalXmlStorageElement)
			// getRootStorageElement());
	...
	private void setModified(boolean modified) {
		...
		if (!modified) {
			...
			// no need to do that for config cache since they always maintain
			// the "isModified == false"
		}
	}
	...
	void switchToCachedAppliedData(CProjectDescription appliedCache) {
		...
		for (int i = 0; i < cfgs.length; i++) {
			...
			if (des != null) {
				...
				// ICResourceDescription rcDes = des.getResourceDescription(new
				// Path("dd"), false);
				// rcDes = des.getResourceDescription(new Path("dd"), false);
				// ICBuildSetting bs = des.getBuildSetting();
				// ICLanguageSetting lss[] = ((ICFolderDescription)
				// rcDes).getLanguageSettings();
			}
		}
	}

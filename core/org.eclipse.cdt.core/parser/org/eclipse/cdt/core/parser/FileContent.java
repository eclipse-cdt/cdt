...
public abstract class FileContent {
	...
	public static FileContent create(ITranslationUnit tu) {
		...
		if (location == null) {
			...
			fileContent = new InternalFileContent(location.toPortableString(), new CharArray(tu.getContents()));
		} else {
			IResource res = tu.getResource();
			if (res instanceof IFile) {
				fileContent = InternalParserUtil.createWorkspaceFileContent((IFile) res);
			} else {
				fileContent = InternalParserUtil.createExternalFileContent(location.toPortableString(),

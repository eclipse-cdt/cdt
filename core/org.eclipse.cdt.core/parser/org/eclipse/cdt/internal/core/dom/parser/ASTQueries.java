...
public class ASTQueries {
	...
	protected static IBinding[] getLabels(IASTFunctionDefinition functionDefinition) {
	    ...
	    if (action.labels != null) {
		    for (int i = 0; i < action.labels.length && action.labels[i] != null; i++) {
		        IASTLabelStatement labelStatement = (IASTLabelStatement) action.labels[i];
	...
	protected static IBinding resolveLabel(IASTName labelReference) {
		...
		if (functionDefinition != null) {
            for (IBinding label : getLabels(functionDefinition)) {

...
final class CPPASTAmbiguityResolver extends ASTVisitor {
	...
	/*
	 * Used to avoid recursion in visit(IASTDeclaration).
	 */
	private boolean fNestedVisit;
	...
	public int visit(IASTDeclaration decl) {
		if (!fNestedVisit && decl instanceof IASTFunctionDefinition) {
			...
			if (fClassNestingLevel > 0) {
				...
				// definition, but set a flag (fNestedVisit) to avoid recursion.
				fNestedVisit = true;
				decl.accept(this);
				fNestedVisit = false;
			}

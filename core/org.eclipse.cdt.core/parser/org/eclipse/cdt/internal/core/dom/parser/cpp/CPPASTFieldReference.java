	...
	private ICPPEvaluation createEvaluation() {
		...
		if (!ownerEval.isTypeDependent()) {
			...
			if (ownerType != null) {
				...
				// this triggers a search for the definition of the function if it is not already in the binding
				if(binding instanceof CPPFunction && ((CPPFunction)binding).getDefinition() == null) {
					fName.getTranslationUnit().getDefinitionsInAST(binding);
				}

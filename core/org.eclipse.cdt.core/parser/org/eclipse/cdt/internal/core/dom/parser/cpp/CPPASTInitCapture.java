...
public class CPPASTInitCapture extends CPPASTCaptureBase implements ICPPASTInitCapture {
	...
	public CPPASTInitCapture copy(CopyStyle style) {
		final CPPASTInitCapture copy = new CPPASTInitCapture(fDeclarator.copy(style));
	...
	public void setDeclarator(ICPPASTDeclarator declarator) {
		assertNotFrozen();
		Objects.requireNonNull(fDeclarator, "Init capture declarator must not be null."); //$NON-NLS-1$
	...
	public IASTName getIdentifier() {
		return fDeclarator != null ? fDeclarator.getName() : null;
	}

...
public class CPPASTLiteralExpression extends ASTNode implements ICPPASTLiteralExpression {
	...
	private IBinding getUserDefinedLiteralOperatorBinding() {
		try {
			IBinding udlOperator = CPPSemantics.findUserDefinedLiteralOperator(this);
			if(udlOperator != null && !(udlOperator instanceof ProblemBinding)) {
				return udlOperator;
			}
		} catch (DOMException e) {
		}
	...
	private ICPPEvaluation createEvaluation() {
		...
		IBinding udlOperator = getUserDefinedLiteralOperatorBinding();
		if(udlOperator != null && literalEval != EvalFixed.INCOMPLETE) {
			if(udlOperator instanceof ICPPFunction) {
				...
				ICPPASTName name = new CPPASTName(udlOpFunction.getNameCharArray());
				name.setBinding(udlOpFunction);

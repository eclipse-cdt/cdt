...
public class CPPASTLiteralExpression extends ASTNode implements ICPPASTLiteralExpression {
	...
	private char[] fNumericCompilerSuffixes;
	...
	public CPPASTLiteralExpression(int kind, char[] value, char[] numericCompilerSuffixes) {
		fKind = kind;
		setValue(kind, value);
		setNumericCompilerSuffixes(numericCompilerSuffixes);
	}
	...
	public CPPASTLiteralExpression copy(CopyStyle style) {
		CPPASTLiteralExpression copy = new CPPASTLiteralExpression(fKind, this.getValue(), fNumericCompilerSuffixes);
		...
		copy.fImplicitNames = fImplicitNames;
	...
	public void setValue(char[] value) {
		assertNotFrozen();
		setValue(fKind, value);
	}

	private void setValue(int kind, char[] value) {
	...
	/**
	 * Adds support for additional numeric compiler suffixes.
	...
	private static char[] getSuffix(int kind, char[] value, char[] suffix) {
		...
		return (offset > 0) ? CharArrayUtils.subarray(value, offset, -1) : suffix;
	}
	...
	public void setValue(String value) {
		assertNotFrozen();
		setValue(fKind, value.toCharArray());
	}

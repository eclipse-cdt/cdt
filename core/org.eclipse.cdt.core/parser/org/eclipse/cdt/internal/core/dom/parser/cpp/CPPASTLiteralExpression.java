...
public class CPPASTLiteralExpression extends ASTNode implements ICPPASTLiteralExpression {
	 ...
	 */
	private char[] fLiteral = CharArrayUtils.EMPTY;
	...
	public CPPASTLiteralExpression copy(CopyStyle style) {
		CPPASTLiteralExpression copy = new CPPASTLiteralExpression(fKind, this.getValue());
		copy.fNumericCompilerSuffixes = fNumericCompilerSuffixes.clone();
	...
	public void setKind(int kind) {
		assertNotFrozen();
		fKind = kind;
		// Make sure that the value reflects the new kind.
	 ...
	 */
	public void setNumericCompilerSuffixes(char[] numericCompilerSuffixes) {
		assertNotFrozen();
	...
	/**
	 * Reset the lazy evaluated fields. This is needed if any of the other fields change.
	 */
	...
	private char[] getLiteral(char[] value, char[] suffix) {
		if (value == null) {
	...
	private char[] getSuffix(char[] value) {
		...
		return (offset > 0) ? CharArrayUtils.subarray(value, offset, -1) : fSuffix;
	}
	...
	private int integerLiteral(char[] value) {
		int i = 0;
		char c = value[i++];
	 ...
	 */
	private int handleDecimalOrExponent(char[] value, char c, int i) {
		if (c == '.') {
			return afterDecimalPoint(value, i);
		} else if ((c | 0x20) == 'e') {
	 ...
	 */
	private int afterDecimalPoint(char[] value, int i) {
		char c = value[++i];
		while (Character.isDigit(c) && i < value.length) {
			c = value[++i];
		}
	 ...
	 */
	private int exponentPart(char[] value, int i) {
		char c = value[++i];
	...
	private int probablyBinary(char[] value, int i) {
		char c = value[++i];
	...
	private int probablyHex(char[] value, int i) {
		/* hexadecimal-literal
	...
	// Assumes value[i] == '.'
	private int hexFloatAfterDecimal(char[] value, int i) {
		// 0xHHH.
		char c = value[++i];
		if (isHexDigit(c)) {
			while (isHexDigit(c) && i < value.length) {
	...
	// Assumes image[i] == 'p'
	private int hexFloatExponent(char[] value, int i) {
		// 0xHH.HH[pP][-+]?DDDD
		char c = value[++i];
	...
	public void setValue(String value) {
		assertNotFrozen();
		this.setValue(value.toCharArray());
	}

...
public abstract class CPPASTNameBase extends ASTNode implements ICPPASTName {
	...
	private IBinding resolveBindingPromiscuous() {
		// If we have a cached binding and it's not a ProblemBinding, re-resolving 
		// it in promiscuous mode wouldn't return anything different, so just
		// return the cached binding.
		if (fBinding != null && !(fBinding instanceof IProblemBinding)) {
			return fBinding;
		}

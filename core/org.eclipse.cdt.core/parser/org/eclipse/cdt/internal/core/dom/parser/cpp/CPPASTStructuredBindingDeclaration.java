...
public class CPPASTStructuredBindingDeclaration extends CPPASTSimpleDeclaration
	...
	public ICPPExecution getExecution() {
		...
		ICPPExecution[] nameExecutions = new ICPPExecution[names.length];
		for (int i = 0; i < names.length; ++i) {
			final IBinding binding = names[i].resolveBinding();
			...
			if (initializerEval == EvalFixed.INCOMPLETE) {
				nameExecutions[i] = ExecIncomplete.INSTANCE;
			}
			nameExecutions[i] = new ExecDeclarator((ICPPBinding) binding, initializerEval);
		}

...
public class CPPVariable extends PlatformObject implements ICPPInternalDeclaredVariable {
	...
	private static final IStructuredBindingNameEvaluationStrategy INCOMPLETE_INITIALIZER = index -> EvalFixed.INCOMPLETE;
	...
	public ICPPEvaluation getInitializerEvaluation() {
		...
		CPPASTStructuredBindingDeclaration structuredBinding = getStructuredBindingDeclaration();
	...
	private ICPPEvaluation evaluationOfStructuredBindingInitializer(
			CPPASTStructuredBindingDeclaration structuredBinding) {
		IASTImplicitName implicitName = structuredBinding.getImplicitNames()[0];
		CPPImplicitVariable variable = (CPPImplicitVariable) implicitName.getBinding();
		ICPPEvaluation initializerEvaluation = variable.getInitializerEvaluation();
		IType initializerType = variable.getType();

		int index = ArrayUtil.indexOf(structuredBinding.getNames(), fDefinition);
		IStructuredBindingNameEvaluationStrategy evaluationStrategy = getInitializerEvaluationStrategy(structuredBinding,
				initializerEvaluation, initializerType);
		return evaluationStrategy.getEvaluation(index);
	}
	...
	private CPPASTStructuredBindingDeclaration getStructuredBindingDeclaration() {
		IASTNode definition = getDefinition();
		if (definition != null) {
			IASTNode parent = definition.getParent();
			if (parent instanceof CPPASTStructuredBindingDeclaration) {
				return (CPPASTStructuredBindingDeclaration) parent;
			}
		}
		return null;
	}
	...
	private class ArrayElement implements IStructuredBindingNameEvaluationStrategy {
	...
	private class TupleElement implements IStructuredBindingNameEvaluationStrategy {
		private IASTInitializer initializer;
		private ICPPEvaluation initializerEvaluation;
	...
	private class MemberElement implements IStructuredBindingNameEvaluationStrategy {
		private IASTInitializer initializer;
		private ICPPEvaluation initializerEvaluation;
		private ICPPClassType initializerType;
	...
	private IStructuredBindingNameEvaluationStrategy getInitializerEvaluationStrategy(
			ICPPASTStructuredBindingDeclaration structuredBinding, ICPPEvaluation initializerEvaluation,

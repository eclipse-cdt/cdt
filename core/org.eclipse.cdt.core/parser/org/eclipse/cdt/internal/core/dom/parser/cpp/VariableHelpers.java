...
/**
 * Collects functionality used by both CPPVariable and CPPVariableTemplate and
 * the according subclasses.
 */
public class VariableHelpers {
	...
	public static IType createType(ICPPVariable variable, IASTName definition, IASTName[] declarations,
		...
		for (int i = 0; i <= length; i++) {
			...
			if (definition != null && !doneWithDefinition && (i == length
					|| ((ASTNode) definition).getOffset() < ((ASTNode) declarations[i]).getOffset())) {
				n = definition;
				doneWithDefinition = true;
				--i; // We still have to come back to the declaration at
						// position i.
			} else if (i < length) {
				n = declarations[i];
			} else {

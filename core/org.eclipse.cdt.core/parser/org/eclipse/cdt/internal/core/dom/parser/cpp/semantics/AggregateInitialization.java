...
class AggregateInitialization {
	...
	private Cost checkElement(IType type, IValue initialValue, Cost worstCost) throws DOMException {
		...
		if (initFromStringLiteral(nestedType, fInitializers[fIndex])) {
			...
			fIndex++;
			// TODO merge this check with initFromStringLiteral()
			long sizeofCharArray = 0;
			if (((IArrayType) nestedType).getSize() != null) {
				IValue size = ((IArrayType) nestedType).getSize();
				if (size.numberValue() != null) {
					sizeofCharArray = ((IArrayType) nestedType).getSize().numberValue().longValue();
				}
			}

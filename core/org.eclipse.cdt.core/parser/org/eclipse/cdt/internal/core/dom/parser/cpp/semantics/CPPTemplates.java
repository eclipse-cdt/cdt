...
public class CPPTemplates {
	...
	public static IBinding createSpecialization(ICPPClassSpecialization owner, IBinding decl, IASTNode point) {
		...
		if (decl instanceof ICPPClassTemplatePartialSpecialization) {
			...
			CPPEnumerationSpecialization specializedEnumeration = new CPPEnumerationSpecialization(enumeration, 
					owner, tpMap, fixedType); 
			...
			for (int i = 0; i < enumerators.length; ++i) {
				...
				IValue specializedValue = instantiateValue(enumerator.getValue(), tpMap, -1, within, Value.MAX_RECURSION_DEPTH, point);
				specializedEnumerators[i] = new CPPEnumeratorSpecialization(enumerator, specializedEnumeration, 
						tpMap, specializedValue);
			}

 ...
 */
public class DestructorCallCollector extends ASTVisitor {
	...
	public static IASTImplicitDestructorName[] getTemporariesDestructorCalls(ICPPASTExpression expression) {
		...
		DestructorCallCollector collector = new DestructorCallCollector(expression);
	...
	public static IASTImplicitDestructorName[] getLocalVariablesDestructorCalls(IASTStatement statement) {
		...
		DestructorCallCollector collector =
				new DestructorCallCollector((IASTImplicitDestructorNameOwner) statement);
		statement.accept(collector);
		return collector.getDestructorCalls();
	}

	private DestructorCallCollector(IASTImplicitDestructorNameOwner owner) {
		this.owner = owner;
		...
		if (owner instanceof ICPPASTExpression) {
			if (name instanceof IASTImplicitName && !(name.getParent() instanceof ICPPASTNewExpression)) {
				...
				if (binding instanceof ICPPConstructor) {
					...
					if (destructor != null) {
						ICPPVariable var = getBoundToVariable(name);
						if (var == null) {
	...
	private void addDestructorCall(IASTName name, ICPPMethod destructor) {
	...
	private static ICPPMethod findDestructor(ICPPClassType classType, IASTNode point) {
		return ClassTypeHelper.getMethodInClass(classType, MethodKind.DTOR, point);
	}

	private ICPPVariable getBoundToVariable(IASTName name) {

...
public class EvalBinary extends CPPDependentEvaluation {
	...
	private ICPPEvaluation createOperatorOverloadEvaluation(ICPPFunction overload, IASTNode point, ICPPEvaluation arg1, ICPPEvaluation arg2) {
		...
		if(overload instanceof ICPPMethod) {
			...
			ICPPASTName name = new CPPASTName(overload.getNameCharArray());
			name.setBinding(overload);
			IASTIdExpression idExpr = new CPPASTIdExpression(name);
			ICPPEvaluation op = EvalID.create(idExpr);
	...
	public IValue getValue(IASTNode point) {
		ICPPFunction overload = getOverload(point);
		if (overload != null) {
			ICPPEvaluation operatorCall = createOperatorOverloadEvaluation(overload, point, fArg1, fArg2);
			return operatorCall.getValue(point);
		}
		...
		if(v1.numericalValue() != null && v2.numericalValue() != null) {
	...
	public ICPPEvaluation computeForFunctionCall(ActivationRecord record, ConstexprEvaluationContext context) {
		...
		if(isBinaryOperationWithAssignment(fOperator)) {
			if (isPointerToArray(fixed1) && hasIntType(fixed2)) {
				...
				int rhs = fixed2.getValue(null).numericalValue().intValue();
			...
			if(fixed2 instanceof EvalPointer) {
				...
				newValue = new EvalFixed(fixed2.getType(null), fixed2.getValueCategory(null), fixed2.getValue(null));
			}
			...
			return new EvalCompositeAccess(fixed1, fixed2.getValue(null).numericalValue().intValue());
		} else if ((isArray(fixed1) || isArray(fixed2)) && (hasIntType(fixed1) || hasIntType(fixed2))) {
			int offset = hasIntType(fixed1) ? fixed1.getValue(null).numericalValue().intValue() : fixed2.getValue(null).numericalValue().intValue();
			...
			pointer.setPosition(eval.getValue(null).numericalValue().intValue());
			return pointer;
		}
		return eval;
	}
	...
	private boolean isArray(ICPPEvaluation eval) {
		return eval.getType(null) instanceof IArrayType;
	}
	...
	private boolean isPointerToArray(ICPPEvaluation argument) {
		if (argument instanceof EvalPointer) {
			...
			ICPPEvaluation pointerValue = evalPointer.dereference().getValue();

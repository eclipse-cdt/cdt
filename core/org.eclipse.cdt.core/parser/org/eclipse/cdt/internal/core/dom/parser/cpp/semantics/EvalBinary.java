	...
	public ICPPEvaluation computeForFunctionCall(ActivationRecord record, ConstexprEvaluationContext context) {
		...
		if(isBinaryOperationWithAssignment(fOperator)) {
			if (isPointerToArray(fixed1) && hasIntType(fixed2, context)) {
				...
				int rhs = fixed2.getValue(context.getPoint()).numericalValue().intValue();
			...
			return new EvalCompositeAccess(fixed1, fixed2.getValue(context.getPoint()).numericalValue().intValue());
		} else if ((isArray(fixed1, context) || isArray(fixed2, context)) && (hasIntType(fixed1, context) || hasIntType(fixed2, context))) {
			int offset = hasIntType(fixed1, context) ? fixed1.getValue(context.getPoint()).numericalValue().intValue() : fixed2.getValue(context.getPoint()).numericalValue().intValue();
			...
			pointer.setPosition(eval.getValue(context.getPoint()).numericalValue().intValue());
			return pointer;
		}
		return eval;
	}

	...
	public static boolean bindingIsNoexcept(IBinding b) {
		if (b != null) {
			if (b instanceof ICPPFunction) {
				ICPPFunction f = (ICPPFunction) b;
				ICPPFunctionType ft = f.getType();
				if (EvalUtil.evaluateNoexceptSpecifier(ft.getNoexceptSpecifier())) {
					return true;
				} else {
					// check for empty throw()
					return f.getExceptionSpecification() == IType.EMPTY_TYPE_ARRAY;
				}
				...
				} else if (v.getType() instanceof CPPClosureType) {
					CPPClosureType t = (CPPClosureType) v.getType();
					return EvalUtil
							.evaluateNoexceptSpecifier(t.getFunctionCallOperator().getType().getNoexceptSpecifier());
				}
			}
		}
		assert false;

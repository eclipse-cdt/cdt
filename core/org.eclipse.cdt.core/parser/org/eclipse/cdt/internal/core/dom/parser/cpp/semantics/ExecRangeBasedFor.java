...
public class ExecRangeBasedFor implements ICPPExecution {
	...
	private ICPPExecution loopOverArray(IVariable rangeVar, ICPPEvaluation valueRange, ActivationRecord record, ConstexprEvaluationContext context) {
		ICPPEvaluation[] range = valueRange.getValue(null).getAll();
		for(int i = 0; i < range.length; i++ ) {
			ICPPEvaluation value = new EvalFixed(range[i].getType(null), range[i].getValueCategory(null), range[i].getValue(null));
	...
	private ICPPEvaluation deref(ICPPEvaluation ptr, boolean isRef, ActivationRecord record, ConstexprEvaluationContext context) {
		...
		if(isRef) {
			...
			return new EvalFixed(derefEval.getType(null), derefEval.getValueCategory(null), derefEval.getValue(null));
		}
	}
	
	private ICPPEvaluation inc(ICPPEvaluation ptr, IASTNode point) {
	...
	private static ICPPEvaluation callFunction(ICPPClassType classType, ICPPFunction func, ICPPEvaluation rangeEval, ActivationRecord record, ConstexprEvaluationContext context) {
		...
		if(func instanceof ICPPMethod) {
			...
			ICPPASTName name = new CPPASTName(func.getNameCharArray());
			name.setBinding(func);
	...
	public ICPPExecution executeForFunctionCall(ActivationRecord record, ConstexprEvaluationContext context) {
		...
		boolean rangeIsConst = isConst(initClauseEval.getType(null));
		IType type = SemanticUtil.getNestedType(valueRange.getType(null), ALLCVQ | TDEF | REF);

...
public class ExecSwitch implements ICPPExecution {
	...
	private boolean isSatisfiedCaseStatement(ICPPExecution stmtExec, IValue controllerValue, ActivationRecord record, ConstexprEvaluationContext context) {
		if (stmtExec instanceof ExecCase) {
			...
			Number caseVal = caseStmtExec.getCaseExpressionEvaluation().getValue(null).numericalValue();
			Number controllerVal = controllerValue.numericalValue();
	...
	public ICPPExecution instantiate(InstantiationContext context, int maxDepth) {
		...
		boolean evaluationsDidChange = false;
		for(int i = 0; i < bodyStmtExecutions.length; i++) {
			...
			if(newBodyStmtExec != bodyStmtExec) {
				evaluationsDidChange = true;
			}
			newBodyStmtExecutions[i] = newBodyStmtExec;
		}
		
		if (newControllerExprEval == controllerExprEval && newControllerDeclExec == controllerDeclExec && !evaluationsDidChange) {
			return this;
		}

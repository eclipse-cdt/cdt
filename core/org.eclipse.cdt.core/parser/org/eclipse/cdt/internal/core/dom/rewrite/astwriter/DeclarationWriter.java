...
public class DeclarationWriter extends NodeWriter {
	...
	protected void writeDeclaration(IASTDeclaration declaration, boolean writeSemicolon) {
		...
		if (declaration instanceof IASTASMDeclaration) {
			...
			writeStructuredBinding((ICPPASTStructuredBindingDeclaration) declaration);
	...
	private void writeStructuredBinding(ICPPASTStructuredBindingDeclaration declaration) {
		...
		refQualifier.ifPresent(qualifier -> printRefQualifier(qualifier));
		scribe.printSpace();
		...
		declaration.getInitializer().accept(visitor);

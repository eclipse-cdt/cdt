...
public class ScannerUtility {
	...
	public static String reconcilePath(String originalPath ) {
		...
		if (UNCPathConverter.isUNC(originalPath)) {
			...
			Assert.isTrue(len >= 2, "Unknown UNC path format"); //$NON-NLS-1$
			Assert.isTrue(ein[0] == ein[1], "Unknown UNC path format"); //$NON-NLS-1$
			Assert.isTrue(ein[0] == BSLASH || ein[0] == SLASH, "Unknown UNC path format"); //$NON-NLS-1$

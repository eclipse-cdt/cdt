...
public class PDOMCPPMemberBlock {
	...
	private void visitBlock(IPDOMVisitor visitor) throws CoreException {
		...
		int item = 0;
		long memberRecord;
		while ((memberRecord = getMemberRecord(item++)) != 0) {
	...
	private PDOMNode getMember(int memberIndex) throws CoreException {
		if (memberIndex < getNextPosition()) {
	...
	private void setVisibility(int memberIndex, int visibility) throws CoreException {
		int newVisibility = visibility & VISIBILITY_MASK;

		int targetVisibilityBytes = memberIndex / VISIBILITY_VALUES_PER_BYTE;
		int visibilityBlockOffset = memberIndex % VISIBILITY_VALUES_PER_BYTE;
		int newVisibilityMask = (byte) (newVisibility << visibilityBlockOffset * VISIBILITY_BITS);
		int resetMask = ~(VISIBILITY_MASK << visibilityBlockOffset * VISIBILITY_BITS);

		long visibilityOffset = record + MEMBER_VISIBILITIES + targetVisibilityBytes;
		int currentVisibility = getDB().getByte(visibilityOffset);

		currentVisibility &= resetMask;
		currentVisibility |= newVisibilityMask;

		getDB().putByte(visibilityOffset, (byte) currentVisibility);
	}
	...
	private int getVisibility(int memberIndex) throws CoreException {
		int targetVisibilityBytes = memberIndex / VISIBILITY_VALUES_PER_BYTE;
		int visibilityBlockOffset = memberIndex % VISIBILITY_VALUES_PER_BYTE;
		int visibilityMask = VISIBILITY_MASK << visibilityBlockOffset * VISIBILITY_BITS;

		long visibilityOffset = record + MEMBER_VISIBILITIES + targetVisibilityBytes;
		int currentVisibility = getDB().getByte(visibilityOffset);
		int maskedVisibility = currentVisibility & visibilityMask;

		maskedVisibility >>>= visibilityBlockOffset * VISIBILITY_BITS;

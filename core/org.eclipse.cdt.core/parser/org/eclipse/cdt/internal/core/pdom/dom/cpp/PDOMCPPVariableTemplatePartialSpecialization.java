...
public class PDOMCPPVariableTemplatePartialSpecialization extends PDOMCPPVariableTemplate
	...
	public ICPPTemplateDefinition getPrimaryTemplate() {
		try {
			if (this instanceof PDOMCPPFieldTemplatePartialSpecialization)
				return new PDOMCPPFieldTemplate(getLinkage(), getDB().getRecPtr(record + PRIMARY));
			else
				return new PDOMCPPVariableTemplate(getLinkage(), getDB().getRecPtr(record + PRIMARY));

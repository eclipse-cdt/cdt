...
public class IndexerPreferences {
	...
	private static void migrateProperties(Properties props) {
		if (props.get(KEY_INDEX_UNUSED_HEADERS_WITH_DEFAULT_LANG) == null) {
			...
			if (Boolean.parseBoolean(props.get(KEY_INDEX_ALL_FILES))) {
	...
	public static boolean preferDefaultLanguage(Properties props) {
		if (Boolean.parseBoolean(props.get(KEY_INDEX_UNUSED_HEADERS_WITH_ALTERNATE_LANG)) &&
				Boolean.parseBoolean(props.get(KEY_INDEX_ALL_FILES)) &&
				!Boolean.parseBoolean(props.get(KEY_INDEX_UNUSED_HEADERS_WITH_DEFAULT_LANG))) {
			return false;
		}
		return true;
	}

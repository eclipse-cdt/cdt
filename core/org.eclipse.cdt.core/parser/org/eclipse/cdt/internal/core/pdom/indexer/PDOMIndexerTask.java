...
public abstract class PDOMIndexerTask extends AbstractIndexerTask implements IPDOMIndexerTask {
	...
	protected PDOMIndexerTask(ITranslationUnit[] forceFiles, ITranslationUnit[] updateFiles,
		...
		if (checkProperty(IndexerPreferences.KEY_INDEX_ALL_FILES)) {
			...
			if (i1 == i2) {
				...
				strategy = i1 == CProject.hasCCNature(getCProject().getProject()) ? UnusedHeaderStrategy.useCPP
						: UnusedHeaderStrategy.useC;
			}
		...
		String privatePattern = CCorePreferenceConstants.getPreference(CCorePreferenceConstants.INCLUDE_PRIVATE_PATTERN,
				project, null);
	...
	protected final IncludeExportPatterns getIncludeExportPatterns() {
		ICProject project = getCProject();
		String exportPattern = CCorePreferenceConstants.getPreference(CCorePreferenceConstants.INCLUDE_EXPORT_PATTERN,
				project, null);
		String beginExportsPattern = CCorePreferenceConstants
				.getPreference(CCorePreferenceConstants.INCLUDE_BEGIN_EXPORTS_PATTERN, project, null);
		String endExportsPattern = CCorePreferenceConstants
				.getPreference(CCorePreferenceConstants.INCLUDE_END_EXPORTS_PATTERN, project, null);
	...
	protected void traceEnd(long start, IWritableIndex index, boolean wasCancelled) {
		...
		if (fWriteInfoToLog && !wasCancelled && index != null) {
			...
			final int sum = fStatistics.fDeclarationCount + fStatistics.fReferenceCount
			...
			final String msg = NLS.bind(Messages.PDOMIndexerTask_indexerInfo,
					new Object[] { getCProject().getElementName(), nfGroup.format(info.fCompletedSources),
							nfGroup.format(info.fCompletedHeaders), nfTime.format(totalTime / 1000.),
							nfGroup.format(fStatistics.fDeclarationCount), nfGroup.format(fStatistics.fReferenceCount),
							...
							nfGroup.format(fStatistics.fProblemBindingCount), nfPercent.format(problemPct) });
		...
		if (checkDebugOption(IPDOMIndexerTask.TRACE_STATISTICS, TRUE)) {
			...
			if (Boolean.parseBoolean(System.getProperty("SHOW_COMPRESSED_INDEXER_INFO"))) { //$NON-NLS-1$
				...
				System.out.print(cal.get(Calendar.YEAR) + twoDigits.format(cal.get(Calendar.MONTH) + 1)

	...
	private IEnvironmentVariable getVariable() {
		try {
			...
			if (useCmakeToolLocation) {
				locations.add(getPreferences().get(CMAKE_LOCATION, EMPTY_STRING));
				for (String key : getPreferences().node(CMAKE_GENERATOR_LOCATION).keys()) {
					locations.add(getPreferences().node(CMAKE_GENERATOR_LOCATION).get(key, EMPTY_STRING));
				}

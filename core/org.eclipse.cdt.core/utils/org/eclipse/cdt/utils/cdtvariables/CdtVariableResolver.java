...
public class CdtVariableResolver {
	 ...
	 * macro names are allowed. It is possible to prevent macro from expanding using ${=varname].
	 *
	...
	static public String resolveToString(String string, IVariableSubstitutor substitutor) throws CdtVariableException {
		...
		final String VARIABLE_IGNORE_PREFIX = "${="; //$NON-NLS-1$
		final String VARIABLE_IGNORE_DOUBLE_PREFIX = "${=="; //$NON-NLS-1$
		...
		while (matcher.find()) {
			...
			resolved = resolved.replace(VARIABLE_PREFIX, VARIABLE_IGNORE_PREFIX);
			resolved = resolved.replace(VARIABLE_IGNORE_DOUBLE_PREFIX, VARIABLE_IGNORE_PREFIX);
			if (limit-- < 0) {
		...
		String result = buffer.toString().replace(VARIABLE_IGNORE_PREFIX, VARIABLE_PREFIX);

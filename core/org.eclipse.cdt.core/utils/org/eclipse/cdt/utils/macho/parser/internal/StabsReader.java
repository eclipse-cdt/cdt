...
class StabsReader implements ISymbolReader {
	...
	private final static int STAB_CONSTANT_SIZE_64 = 16; // 4 + 1 + 1 + 2 + 8

	public StabsReader(byte[] data, byte[] stabstr, boolean littleEndian) {
		this(data,stabstr,littleEndian,false);
	}
	/**
	 * This constructor is non-API in CDT 6.0.X and is not intended to be referenced.
	 */
	StabsReader(byte[] data, byte[] stabstr, boolean littleEndian, boolean is64bit) {
		stabData = data;
		stabstrData = stabstr;
		isLe = littleEndian;
		is64 = is64bit;
	...
	private void parse() {
		int size = is64 ? STAB_CONSTANT_SIZE_64 : StabConstant.SIZE;

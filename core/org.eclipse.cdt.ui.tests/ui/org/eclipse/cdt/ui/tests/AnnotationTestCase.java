...
public class AnnotationTestCase extends BaseUITestCase {
		...
		try {
			...
			synchronized (fAnnotationModelLockObject) {
				...
				while (iter.hasNext()) {
					...
					if ((a != null) && (testedAnnotationId.equals(a.getType()))) {
						annotations = (Annotation[]) ArrayUtil.append(Annotation.class, annotations, a);
					}
				}
			}
			annotations = (Annotation[]) ArrayUtil.trim(Annotation.class, annotations);
		}
		catch (Exception e) {
			fail(e.getMessage());
		}

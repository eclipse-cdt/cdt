...
//		EditorTestHelper.joinBackgroundActivities((AbstractTextEditor) fEditor);
	...
	protected void assertContentAssistResults(int offset, int length,
			Map<String, String[][]> expected, boolean isCompletion,
			boolean isTemplate, boolean filterResults, CompareType compareType)
			throws Exception {
		ContentAssistResult r = invokeContentAssist(offset, length,
				isCompletion, isTemplate, filterResults);
		...
		for (String proposal : expected.keySet()) {
			...
			String exp = "";
			String guess = "";
			int minLength = (expectedGuesses.length < result.length) ? expectedGuesses.length
					: result.length;

...
public class CopyQualifiedNameAction extends SelectionDispatchAction {
	...
	@Override
	public void run(IStructuredSelection t) {
		try {
			ICElement element = getCElement(t);
			if (element == null) {
			...
			List<String> qualNames = Collections.emptyList();
	...
	@Override
	public void run(ITextSelection t) {
		if (t.isEmpty()) {
			MessageDialog.openInformation(getShell(), ActionMessages.CopyQualifiedNameAction_InfoDialogTitel,
					ActionMessages.CopyQualifiedNameAction_NoElementToQualify);
			return;
		...
		Clipboard clipboard = new Clipboard(getShell().getDisplay());
		Transfer[] transfers = new Transfer[qualNames.size()];
		Arrays.fill(transfers, TextTransfer.getInstance());
		try {
			clipboard.setContents(qualNames.toArray(), transfers);
		} catch (SWTError e) {
			if (e.code != DND.ERROR_CANNOT_SET_CLIPBOARD) {
				throw e;
			}

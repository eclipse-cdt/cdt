...
/** 
 * Nearly copy past of CCombo but fixed to have border around text, but no border around text history button.
 * This is required because button can't be smaller and border around button makes CCombo not to fit 
 * into build console action toolbar.  
 */
class BuildConsoleCCombo extends Composite {
	Text text;
	List list;
	...
	public BuildConsoleCCombo(Composite parent, int style) {
		...
		text = new Text (this, textStyle | SWT.BORDER);
		...
		listener = new Listener () {
			public void handleEvent (Event event) {
				...
				if (BuildConsoleCCombo.this == event.widget) {
					comboEvent (event);
					return;
				}
		...
		filter = new Listener() {
			public void handleEvent(Event event) {
				...
				if (shell == BuildConsoleCCombo.this.getShell ()) {
					handleFocus (SWT.FocusOut);
				}
			}
		};
	...
	static int checkStyle (int style) {
		int mask = SWT.READ_ONLY | SWT.FLAT | SWT.LEFT_TO_RIGHT | SWT.RIGHT_TO_LEFT;
	...
	public Point computeSize (int wHint, int hHint, boolean changed) {
		...
		if ( fTextWidth > 0 ) {
			textWidth = Math.max (spacer * fTextWidth, textWidth);
		}
		gc.dispose();
	...
	public void setWidth(int numOfSymbols) {
		fTextWidth = numOfSymbols;
	}
}

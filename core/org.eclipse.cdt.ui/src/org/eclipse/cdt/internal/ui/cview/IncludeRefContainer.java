 ...
 * IncludeRefContainer
 */
public class IncludeRefContainer extends CElementGrouping implements IPropertyChangeListener {

	ICProject fCProject;

	public static String groupProp = "IncludeGrouping"; //$NON-NLS-1$

	public enum Representation {
		List(0), Single(1), Compact(2);
		...
		public static Representation fromInt(int x) {
			for (Representation type : values()) {
				if (type.toInt() == x) {
					return type;
				}
			}
	...
	public Representation representation = Representation.Compact;
	...
	public IncludeRefContainer(ICProject cproject) {
		...
		representation = Representation.fromInt(store.getInt(groupProp));
	...
	public void propertyChange(PropertyChangeEvent event) {
		if (!event.getProperty().equals(groupProp))
			return;
		var nv = event.getNewValue();
		if (!(nv instanceof Integer))
			return;

		representation = Representation.fromInt((Integer) nv);

...
public class IBConversions {
	...
	public static Optional<ITranslationUnit> fileToTU(IFile file) {
		...
		if (tu == null) {
			...
			return Optional.ofNullable(CoreModel.getDefault().create(file.getProject())).map(cproject -> {
				return Optional.ofNullable(CoreModel.getRegistedContentTypeId(file.getProject(), file.getName()))
						.map(contentTypeId -> new TranslationUnit(cproject, file, contentTypeId)).orElse(null);
			});
		}

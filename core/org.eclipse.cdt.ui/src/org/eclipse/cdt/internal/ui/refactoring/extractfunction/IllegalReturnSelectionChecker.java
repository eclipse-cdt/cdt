...
class IllegalReturnSelectionChecker extends AbstractSelectionChecker {
			...
			node.accept(new ASTVisitor() {
				...
				public int visit(IASTStatement statement) {
					if (statement instanceof IASTReturnStatement) {
						if (ASTQueries.findAncestorWithType(statement, ICPPASTLambdaExpression.class) == null) {
							returnStmnts = true;
	...
	private boolean isExitPoint(IASTStatement statement) {
		...
		return isThrowStatement(statement) || isExitStatement(statement) || statement instanceof IASTReturnStatement;
	}

...
public class InlineLocalVariableRefactoring extends CRefactoring {
	...
	public RefactoringStatus checkInitialConditions(IProgressMonitor pm)
		...
		if (nodes.isEmpty()) {
			...
			return initStatus;
		} else if (!(nodes.getNodesToWrite().get(0) instanceof IASTIdExpression) || nodes.getNames().isEmpty()) {
		...
		functionDef = ASTQueries.findAncestorWithType(nodes.getNodesToWrite().get(0), IASTFunctionDefinition.class);
	...
	private CPPASTSimpleTypeConstructorExpression createSimpleTypeCtorExpr(CPPASTInitializerList initializerList,
	...
	protected RefactoringDescriptor getRefactoringDescriptor() {
		...
		RefactoringDescriptor desc = new InlineLocalVariableRefactoringDescriptor(project.getProject().getName(),
				"Inline Local Variable Refactoring", //$NON-NLS-1$

...
public class InlineTempInputPage extends UserInputWizardPage {
	...
	private static String getInitializerString(IASTDeclarator decl) {
		final IASTInitializerClause clause = InlineTempRefactoring.getInitializer(decl);
		final ASTWriterVisitor v = new ASTWriterVisitor();
		clause.accept(v);
	...
	private static class InputForm extends Composite {
		...
		public InputForm(Composite parent, final InlineTempSettings settings) {
			...
			lbl.setText(NLS.bind(Messages.InputPage_label, 
					settings.getSelected().getName(),
					getInitializerString(settings.getDeclarator())));

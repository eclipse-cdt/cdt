...
public final class InsertionPoint {
	 ...
	 * @param group The current move action group. Needed to find visibility labels that
	 * 		have been created during the refactoring
	...
	public static InsertionPoint calculate(CRefactoringContext context, 
			MoveActionGroup group, 
			int targetVisibility, IASTNode declaration, IASTNode definition, 
			IASTCompositeTypeSpecifier targetClass,
			InsertType type) {
		
		final InsertionPoint insertion = new InsertionPoint(context, group, 
				targetVisibility, targetClass);
	...
	private final MoveActionGroup group;
	...
	public void perform(ModificationCollector mc, IASTNode newNode, 
		...
		if (this.insertionTarget.isWithinClass()) {
			...
			if (label == null) {
				...
				this.group.addLabel(this.insertionTarget.targetClassDef, label);
	 ...
	 * Before actually searching the class, the artificial label store of the 
	 * {@link #group MoveActionGroup} is searched.
	 * 
	...
	private ICPPASTVisibilityLabel findVisibility(IASTCompositeTypeSpecifier target, 
		...
		final ICPPASTVisibilityLabel artificial = this.group.getArtificialLabel(target, visibility);
		if (artificial != null) {
			return artificial;
		}

...
public class MoveActionGroup implements MoveAction {
	 ...
	 * any MoveAction
	 */
	private final Map<IASTCompositeTypeSpecifier, Map<Integer, ICPPASTVisibilityLabel>> labels;
	...
	public MoveActionGroup(Information<? extends MemberTableEntry> information) {
		...
		this.labels = new HashMap<IASTCompositeTypeSpecifier, Map<Integer, ICPPASTVisibilityLabel>>();
	...
	public void addLabel(IASTCompositeTypeSpecifier classDef, ICPPASTVisibilityLabel label) {
		Map<Integer, ICPPASTVisibilityLabel> labels = this.labels.get(classDef);
		if (labels == null) {
			labels = new HashMap<Integer, ICPPASTVisibilityLabel>();
			this.labels.put(classDef, labels);
		}
		labels.put(label.getVisibility(), label);
	}
	
	
	
	public ICPPASTVisibilityLabel getArtificialLabel(IASTCompositeTypeSpecifier classDef, int visibility) {
		Map<Integer, ICPPASTVisibilityLabel> labels = this.labels.get(classDef);
		if (labels != null) {
			return labels.get(visibility);
		}
		return null;
	}

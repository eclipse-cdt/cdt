...
public class CSourceHover extends AbstractCEditorTextHover {
	...
	protected static class ComputeSourceRunnable implements ASTRunnable {
						...
						if (name != null) {
							...
							if (binding != null) {
								...
								if (binding instanceof IProblemBinding) {
									...
									fSource= computeSourceForEnumerator(ast, name, binding);
								} else {
		 ...
		 * @throws CoreException 
		 */
		private String computeSourceForEnumerator(IASTTranslationUnit ast, IASTName name, IBinding binding) throws CoreException {
			// Search for the macro definition
			IName[] defs = ast.getDefinitions(binding);
			for (IName def : defs) {
				IValue val = ((IEnumerator) binding).getValue();
				if (val != null) {
					return val.toString();
				} else {
					String source= computeSourceForName(def, binding);

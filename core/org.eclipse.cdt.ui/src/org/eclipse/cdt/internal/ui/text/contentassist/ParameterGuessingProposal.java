...
public class ParameterGuessingProposal extends FunctionCompletionProposal {
	...
	public static ParameterGuessingProposal createProposal(CContentAssistInvocationContext context, CCompletionProposal proposal, IFunction function, String prefix) {
		...
		ParameterGuessingProposal ret = new ParameterGuessingProposal(replacement, replacementOffset, replacementLength, proposal.getImage(), proposal.getDisplayString(), proposal.getIdString(), proposal.getRelevance(), context.getViewer(), function,
				invocationOffset, parseOffset, translationUnit, document);
		 ...
		 */
		ret.fAssignableElemebts = getAssignableElements(context.getViewer(), getStatementStartOffset(document, getStartOffset(parseOffset, prefix)));
	...
	public ParameterGuessingProposal(String replacementString, int replacementOffset, int replacementLength,
			...
			int invocationOffset, int parseOffset, IASTTranslationUnit translationUnit, IDocument document) {
		super(replacementString, replacementOffset, replacementLength, image, displayString, idString, relevance,
				viewer, function,  invocationOffset, parseOffset, translationUnit, document);
	...
	/**
	 * Adds special handling for the case of invocation after open bracket.
	 */
	@Override
	...
	private ICompletionProposal[][] guessParameters(char[][] parameterNames) throws Exception {
		...
		for (int i= count - 1; i >= 0; i--) {
			...
			ICompletionProposal[] argumentProposals= guesser.parameterProposals(fParametersTypes[i], paramName, position, fAssignableElemebts, true, isLastParameter);
	 ...
	 */
	private static int getStartOffset(int parseOffset, String prefix) {
		return parseOffset - prefix.length();
	}

	private static ArrayList<IBinding> getAssignableElements(ITextViewer viewer, int startOffset) {
		ArrayList<IBinding> allBindings = new ArrayList<>();
		IASTCompletionNode node = null;
		// Create a content assist context that points to the start of the statement.
		CContentAssistInvocationContext newContext = new CContentAssistInvocationContext(viewer, startOffset, getCEditor(), true, false);
		try {
	...
	/**
	 * Returns the beginning of statement index.

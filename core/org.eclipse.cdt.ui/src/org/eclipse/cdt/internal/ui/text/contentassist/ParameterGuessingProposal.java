...
public class ParameterGuessingProposal extends FunctionCompletionProposal {
	...
	public static ParameterGuessingProposal createProposal(CContentAssistInvocationContext context,
		 ...
		 */
		ret.fAssignableElemebts = getAssignableElements(context.getViewer(),
				getStatementStartOffset(document, getStartOffset(parseOffset, prefix)));
	...
	private static ArrayList<IBinding> getAssignableElements(ITextViewer viewer, int startOffset) {
		...
		CContentAssistInvocationContext newContext = new CContentAssistInvocationContext(viewer, startOffset,
				getCEditor(), true, false);
		try {
	...
	/**
	 * Returns the beginning of statement index. The beginning of statement could be: - Index after ; - Index
	 * after { - Index after }
	 */

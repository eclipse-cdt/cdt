...
/**
 * Determines if a file or folder got customized build settings and if so
 * decorates with the "wrench" overlay.
 */
		...
		if (config != null
			...
			for (ILanguageSettingsProvider provider : ((ILanguageSettingsProvidersKeeper) cfgDescription)
				...
				for (String languageId : languages) {
					List<ICLanguageSettingEntry> list = provider.getSettingEntries(config, rc, languageId);
					if (list != null) {
						List<ICLanguageSettingEntry> listDefault = provider.getSettingEntries(config, parent,
								languageId);
						// != is OK here due as the equal lists will have the
						// same reference in WeakHashSet
						if (list != listDefault)
							return true;
					}
				}

...
public class IndexerBlock extends AbstractCOptionPage {
	...
	public void createControl(Composite parent) {
		...
		if (fIndexerConfigMap.size() > 2) {
			...
			fReindexOnIndexerChange = ControlFactory.createCheckBox(group, CUIPlugin.getResourceString("IndexerBlock.0")); //$NON-NLS-1$
		}
        ...
        if (needBuildConfigOptions()) {
			...
			fReindexOnConfigChange = ControlFactory.createCheckBox(group, CUIPlugin.getResourceString("IndexerBlock.1")); //$NON-NLS-1$
        }

        ...
        try {
            ...
            if(commentAtStart && !commentFollows && !commentAhead) {
                try {
                	...
                	boolean contentAlreadyThere = (firstLineContainsText && content.toString().contains(commentContent.trim()));
                    if (content == null || content.toString().trim().length() == 0 || contentAlreadyThere)
                    {
                        ...
                        if (!firstLineContainsText)
                        {
                            ...
                            buf.setLength(0);
                            buf.append(" " + indent(content, indentationWithPrefix + " ", fgDefaultLineDelim).substring((indentationWithPrefix + " ").length())); //$NON-NLS-1$
                        }
                        else
                        {
                            buf.append(fgDefaultLineDelim);

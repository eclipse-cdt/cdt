...
public class DoxygenSingleAutoEditStrategy extends DoxygenMultilineAutoEditStrategy {
	...
	public void customizeDocumentAfterNewLine(IDocument doc, final DocumentCommand c) {
        ...
        try {
            ...
            String commentPrefixWithIndent = indentation + commentPrefix;
            ...
            if(commentAtStart && !commentFollows && !commentAhead) {
                try {
                    ...
                    if (content == null || content.toString().trim().length() == 0)
                    {
                        ...
                        buf.append(commentPrefixWithIndent + " "); //$NON-NLS-1$
                        c.shiftsCaret= false;
                        c.caretOffset= c.offset + buf.length();
                    } else {
                        if (!firstLineContainsText)
                        {
                            ...
                            buf.setLength(0);
                            buf.append(indent(content, commentPrefixWithIndent + " ", fgDefaultLineDelim).substring(commentPrefixWithIndent.length())); //$NON-NLS-1$
                        }
                        else
                        {
                            buf.append(fgDefaultLineDelim);
                            buf.append(indent(content, commentPrefixWithIndent + " ", fgDefaultLineDelim)); //$NON-NLS-1$
                        }

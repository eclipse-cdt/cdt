...
public class UIElementTreeBuilderHelper implements IUIElementTreeBuilderHelper {
	...
	private UIElement getUIWidget(Element element, UIAttributes uiAttributes) {
		...
		String id= (String) uiAttributes.get(InputUIElement.ID);
		...
		if (type.equalsIgnoreCase(InputUIElement.INPUTTYPE)) {
			...
			List elements= TemplateEngine.getChildrenOfElement(element);
			for(Iterator i= elements.iterator(); i.hasNext(); ) {
				...
				if(new Boolean(item.getAttribute(InputUIElement.SELECTED)).booleanValue()) {
					defaultValue= value;
				}
				
				if(label==null || value==null) {
					String msg= Messages.getString("UIElementTreeBuilderHelper.InvalidEmptyLabel"); //$NON-NLS-1$
					CUIPlugin.getDefault().log(CCorePlugin.createStatus(MessageFormat.format(msg, new Object[] {id})));
				} else {
			...
			CUIPlugin.getDefault().log(CCorePlugin.createStatus(msg));
		}

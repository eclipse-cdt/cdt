...
ECLIPSE_HOME=$(cd ../../.. && pwd)  # install.sh will modify this line.  DO NOT REMOVE THE FOLLOWING MARKER: @#@#
PLUGIN_DIR=$ECLIPSE_HOME/plugins

OSGI_JAR=`find $PLUGIN_DIR -name 'org.eclipse.osgi_*.jar' -printf "%f\n"`
SWT_JAR=`find $PLUGIN_DIR -name 'org.eclipse.swt.*.jar' -printf "%f\n"`
SWT_PLUGIN=`echo $SWT_JAR | sed -e "s/_[0-9]*\..*.jar//"`
FS_JAR=`find $PLUGIN_DIR -name 'org.eclipse.core.filesystem.*.jar' -printf "%f\n" | grep -v java7`
FS_PLUGIN=`echo $FS_JAR | sed -e "s/_[0-9]*\..*.jar//"`
LINUX_JAR=`find $PLUGIN_DIR -name 'org.eclipse.cdt.core.linux.*.jar' -printf "%f\n"`

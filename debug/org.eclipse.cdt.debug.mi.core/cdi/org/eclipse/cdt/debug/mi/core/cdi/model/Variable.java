...
public abstract class Variable extends VariableDescriptor implements ICDIVariable {
	...
	private boolean isAccessModifier(String name) {
		return name.equals("private") || name.equals("public") || name.equals("protected");
	}
			...
			for (int i = 0; i < vars.length; i++) {
				...
				if (t instanceof ICDIArrayType) {
					...
					if (subType instanceof ICDIStructType || t instanceof IncompleteType) {
						if (isCPPLanguage()) {
							if (!isFake() || (isFake() && !isAccessModifier(fName))) {
								childFake = true;
								childType = t;
							} else {
				...
				if (childFake && isAccessModifier(childName)) {

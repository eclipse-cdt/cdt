...
public class CAddDynamicPrintfInteractiveRulerAction extends Action implements IUpdate {
	...
	public CAddDynamicPrintfInteractiveRulerAction(IWorkbenchPart part, IDocument document, IVerticalRulerInfo rulerInfo) {
		super(ActionMessages.getString("CAddDynamicPrintfInteractiveRulerAction_label") + "\t" + //$NON-NLS-1$ //$NON-NLS-2$
		    CDebugUIUtils.formatKeyBindingString(SWT.MOD3, ActionMessages.getString("CRulerToggleBreakpointAction_accelerator")) );  //$NON-NLS-1$
		fPart = part;
		fDocument = document;
		...
		if (fDynamicPrintfBreakpointsTarget == null){
			...
			for (int i= 0; i < elements.length; i++) {
				...
				if (id != null && id.equals("org.eclipse.cdt.debug.ui.ToggleCDynamicPrintfTargetFactory")) { //$NON-NLS-1$
					try{
						Object obj = elements[i].createExecutableExtension(IConfigurationElementConstants.CLASS);

...
public class ReverseToggleCommandHandler extends DebugCommandHandler implements IDebugContextListener, IElementUpdater {
	...
	IReverseTraceMethod traceMethod = ReverseTraceMethod.STOP_TRACE;
	IReverseTraceMethod lastTraceMethod = ReverseTraceMethod.STOP_TRACE;
    ...
    protected Class<?> getCommandType() {
        return IChangeReverseMethodHandler.class;
    }
    ...
    private IChangeReverseMethodHandler fTargetAdapter = null;
    ...
    private IChangeReverseMethodHandler getAdapter(IAdaptable adaptable) {
    	IChangeReverseMethodHandler adapter  = adaptable.getAdapter(IChangeReverseMethodHandler.class);
        if (adapter == null) {
            IAdapterManager adapterManager = Platform.getAdapterManager();
            if (adapterManager.hasAdapter(adaptable, getCommandType().getName())) {
                adapter = (IChangeReverseMethodHandler)adapterManager.loadAdapter(adaptable, IChangeReverseMethodHandler.class.getName());
            }
        }
        return adapter;
    }
        ...
        if (radioState.equals("UseSoftTrace")) { //$NON-NLS-1$
            ...
            if (CDebugUIPlugin.getDefault().getPreferenceStore().getString(
                ICDebugPreferenceConstants.PREF_REVERSE_TRACE_METHOD_HARDWARE).equals(ICDebugPreferenceConstants.PREF_REVERSE_TRACE_METHOD_BRANCH_TRACE)) {
                    ...
                    traceMethod = ReverseTraceMethod.GDB_TRACE;
            }
            else {
            ...
            if (fTargetAdapter != null) {
                if (fTargetAdapter.toggleNeedsUpdating()){
                	IReverseTraceMethod currMethod = fTargetAdapter.getTraceMethod(fActiveContext);
    ...
    public void updateElement(UIElement element,
       ...
       if(fTargetAdapter != null){
    	   IReverseTraceMethod reverseMethod = fTargetAdapter.getTraceMethod(fActiveContext);
           ICommandService commandService = PlatformUI.getWorkbench().getService(ICommandService.class);

...
public class ReverseToggleCommandHandler extends DebugCommandHandler implements IDebugContextListener, IElementUpdater {
    ...
    public Object execute(ExecutionEvent event) throws ExecutionException {
        ...
        if (radioState.equals("UseSoftTrace")) { //$NON-NLS-1$
            ...
            if (fTargetAdapter != null) {
                ...
                int currMethod = fTargetAdapter.getTraceMethod(fActiveContext);
                    if(currMethod == -1) {
                        if( lastTraceMethod != -1 && lastTraceMethod != 0) {
                            if(CDebugUIPlugin.getDefault().getPreferenceStore().getString(
                            ICDebugPreferenceConstants.PREF_REVERSE_TRACE_METHOD_HARDWARE).equals("UseBranchTrace")) //$NON-NLS-1$
                                traceMethod = 1;
                            else if (CDebugUIPlugin.getDefault().getPreferenceStore().getString(
        ...
        new WorkbenchJob("") { //$NON-NLS-1$
            ...
            public IStatus runInUIThread(IProgressMonitor monitor) {
                ...
                if(prop && request.getStatus() != null && request.getStatus().getCode() != 0 ) {
                    ...
                    MessageDialogWithToggle dialogbox = new MessageDialogWithToggle(activeShell, "Error",
                            null, "Hardware Tracing Method not available, Reverse debugging is switched Off, please select another method", MessageDialog.QUESTION,
    ...
    public void updateElement(UIElement element,
       ...
       if(fTargetAdapter != null){
           int reverseMethod = fTargetAdapter.getTraceMethod(fActiveContext);
           ICommandService commandService = PlatformUI.getWorkbench().getService(ICommandService.class);

...
import org.eclipse.cdt.debug.core.model.IChangeTraceMethodHandler;
...
public class ReverseToggleCommandHandler extends DebugCommandHandler implements IDebugContextListener, IElementUpdater {
    ...
    String traceMethod = STOP_TRACE;
    String lastTraceMethod = STOP_TRACE;
    ...
    protected Class<?> getCommandType() {
        return IChangeTraceMethodHandler.class;
    }
    ...
    private IChangeTraceMethodHandler fTargetAdapter = null;
    ...
    private IChangeTraceMethodHandler getAdapter(IAdaptable adaptable) {
        IChangeTraceMethodHandler adapter  = adaptable.getAdapter(IChangeTraceMethodHandler.class);
        if (adapter == null) {
            IAdapterManager adapterManager = Platform.getAdapterManager();
            if (adapterManager.hasAdapter(adaptable, getCommandType().getName())) {
                adapter = (IChangeTraceMethodHandler)adapterManager.loadAdapter(adaptable, IChangeTraceMethodHandler.class.getName());
            }
        }
        return adapter;
    }
        ...
        if (radioState.equals("UseSoftTrace")) { //$NON-NLS-1$
            ...
            if (CDebugUIPlugin.getDefault().getPreferenceStore().getString(
                ICDebugPreferenceConstants.PREF_REVERSE_TRACE_METHOD_HARDWARE).equals("UseBranchTrace")) { //$NON-NLS-1$
                    ...
                    traceMethod = PROCESSOR_TRACE;
                }
                else
                    traceMethod = GDB_TRACE;
            }
            else {
            ...
            if (fTargetAdapter != null) {
                fTargetAdapter.setTraceMethod(traceMethod);
            }
            ...
            if (fTargetAdapter != null) {
                ...
                String currMethod = fTargetAdapter.getTraceMethod(fActiveContext);
                    if(currMethod == STOP_TRACE) {
                        if( lastTraceMethod != STOP_TRACE && lastTraceMethod != FULL_TRACE) {
                            if(CDebugUIPlugin.getDefault().getPreferenceStore().getString(
                            ICDebugPreferenceConstants.PREF_REVERSE_TRACE_METHOD_HARDWARE).equals("UseBranchTrace")) //$NON-NLS-1$
                                traceMethod = BRANCH_TRACE;
                            else if (CDebugUIPlugin.getDefault().getPreferenceStore().getString(
    ...
    public void updateElement(UIElement element,
       ...
       if(fTargetAdapter != null){
           String reverseMethod = fTargetAdapter.getTraceMethod(fActiveContext);
           ICommandService commandService = PlatformUI.getWorkbench().getService(ICommandService.class);
           ...
           try{
               if (traceMethod != STOP_TRACE && traceMethod != FULL_TRACE) {
               ...
               else if (traceMethod == FULL_TRACE) {
                   ...
                   else if (lastTraceMethod == FULL_TRACE) {
                       HandlerUtil.updateRadioState(commandService.getCommand(REVERSE_TOGGLE_COMMAND_ID), "UseSoftTrace"); //$NON-NLS-1$

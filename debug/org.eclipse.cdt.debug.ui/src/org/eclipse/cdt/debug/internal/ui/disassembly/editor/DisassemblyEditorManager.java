	...
	public void openEditor(IWorkbenchPage page, Object debugContext) throws DebugException {
		...
		if (disassemblyContext != null) {
			...
			if (editor == null) {
				...
				if (sp == null) {
					throw new DebugException(new Status(IStatus.ERROR, CDebugUIPlugin.getUniqueIdentifier(), 0,
							"No disassembly editor found", null));
				}
				...
				try {
					...
					if (steppingModeTarget != null) {
						if (ICDebugConstants.PREF_VALUE_STEP_MODE_CONTEXT.equals(CDebugCorePlugin.getDefault()
								.getPluginPreferences().getString(ICDebugConstants.PREF_STEP_MODE)))
		...
		if (isDisassemblyEditorPart(partRef)) {
			...
			if (part instanceof IEditorPart) {
				...
				if (input instanceof DisassemblyEditorInput) {
					...
					if (steppingModeTarget != null
							&& ICDebugConstants.PREF_VALUE_STEP_MODE_CONTEXT.equals(CDebugCorePlugin.getDefault()
									.getPluginPreferences().getString(ICDebugConstants.PREF_STEP_MODE)))

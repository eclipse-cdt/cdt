...
public class CDebugPreferencePage extends PreferencePage implements IWorkbenchPreferencePage {
	...
	private void storeValues() {
		...
		if (fShowSourceNotFoundAllTime.getSelection()) {
			InstanceScope.INSTANCE.getNode(CCorePlugin.PLUGIN_ID)
					.put(CCorePreferenceConstants.SHOW_SOURCE_NOT_FOUND_EDITOR, CCorePreferenceConstants.ALL_THE_TIME);
		}
		if (fShowSourceNotFoundOnlyCaseSourceNotFound.getSelection()) {
			InstanceScope.INSTANCE.getNode(CCorePlugin.PLUGIN_ID)
					.put(CCorePreferenceConstants.SHOW_SOURCE_NOT_FOUND_EDITOR, CCorePreferenceConstants.SOMETIMES);
		}
		if (fShowSourceNotFoundNever.getSelection()) {
			InstanceScope.INSTANCE.getNode(CCorePlugin.PLUGIN_ID)
					.put(CCorePreferenceConstants.SHOW_SOURCE_NOT_FOUND_EDITOR, CCorePreferenceConstants.NEVER);
		}
	}
	...
	{
		switch (value) {
		case CCorePreferenceConstants.SOMETIMES:
			fShowSourceNotFoundOnlyCaseSourceNotFound.setSelection(true);
			break;
		case CCorePreferenceConstants.NEVER:
			fShowSourceNotFoundNever.setSelection(true);
			break;
		case CCorePreferenceConstants.ALL_THE_TIME:
		default:
	...
	private void setDefaultValues() {
		fCharsetEditor.loadDefault();
		fWideCharsetEditor.loadDefault();
		String defaultSourceShow = DefaultScope.INSTANCE.getNode(CCorePlugin.PLUGIN_ID)
				.get(CCorePreferenceConstants.SHOW_SOURCE_FILES_IN_BINARIES, "all_time"); //$NON-NLS-1$

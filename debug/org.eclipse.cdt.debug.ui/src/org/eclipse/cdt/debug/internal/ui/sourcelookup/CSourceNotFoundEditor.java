...
public class CSourceNotFoundEditor extends CommonSourceNotFoundEditor {
	...
	protected void locateFile() {
		...
		dialog.setFilterExtensions(new String[] {"*." + FilenameUtils.getExtension(missingFile)}); //$NON-NLS-1$
		...
		if (res != null) {
			...
			if (IOCase.INSENSITIVE.checkEquals(FilenameUtils.getName(missingFile), FilenameUtils.getName(res))) {
				String compPath = FilenameUtils.getFullPath(missingFile);
				IPath newSourcePath = new Path(FilenameUtils.getFullPath(res));

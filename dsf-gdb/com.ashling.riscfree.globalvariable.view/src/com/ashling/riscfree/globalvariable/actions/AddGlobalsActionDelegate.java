		...
		if (status != null && !status.isOK()) {
			if (status.isMultiStatus()) {
				status = new MultiStatus(status.getPlugin(), status.getCode(), status.getChildren(),
						ActionMessages.getString("AddGlobalsActionDelegate.Error(s)_occured_adding_globals_1"), //$NON-NLS-1$
						status.getException());
			}
				...
				new IStructuredContentProvider() {
					...
					public String getText(Object element) {
						if (element instanceof IGlobalVariableDescriptor) {
							...
							return (path.length() > 0 ? ('\'' + path + "\'::") : "") //$NON-NLS-1$//$NON-NLS-2$
									+ ((IGlobalVariableDescriptor) element).getName();
						}
						return null;
					}
			...
			if (launch != null && !launch.isTerminated()) {
				try {
					...
					if (dsfSession != null) {
						...
						dsfTracker.dispose();
						fGlobals = globalVariableService
								.getInitialGlobals(((IAdaptable) element).getAdapter(IDMContext.class))
						...
						IGlobalVariableDescriptor[] descriptors = globalVariableService.getGlobals()
								.toArray(new IGlobalVariableDescriptor[0]);
						dlg.setInitialSelections(descriptors);
						if (dlg.open() == Window.OK) {
							List<Object> list = Arrays.asList(dlg.getResult());
							IGlobalVariableDescriptor[] selections = list
									.toArray(new IGlobalVariableDescriptor[list.size()]);
							// <CUSTOMIZATION>
							...
							globalVariableService.addGlobals(((IAdaptable) element).getAdapter(IDMContext.class),
									selections);

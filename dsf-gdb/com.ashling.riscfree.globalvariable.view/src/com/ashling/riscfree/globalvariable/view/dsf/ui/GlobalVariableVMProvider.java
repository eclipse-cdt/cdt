 ...
 *******************************************************************************/
package com.ashling.riscfree.globalvariable.view.dsf.ui;
...
import com.ashling.riscfree.globalvariable.view.viewmodel.GdbGlobalVariableVMNode;
import com.ashling.riscfree.globalvariable.view.viewmodel.GlobalVariableVMNode;
 ...
 */
/**
 * @implNote this class is copied from VariableVMProvider.java in CDT
 * @author vinod.appu
 *
 */
public class GlobalVariableVMProvider extends VariableVMProvider {
	 ...
	 */
	public GlobalVariableVMProvider(AbstractVMAdapter adapter, IPresentationContext context, DsfSession session) {
		super(adapter, context, session);

		final IPreferenceStore store = GdbUIPlugin.getDefault().getPreferenceStore();
		final IPreferenceStore cDebugPluginPreferenceStore = CDebugUIPlugin.getDefault().getPreferenceStore();
		...
		cDebugPluginPreferenceStore.addPropertyChangeListener(fPreferencesListener);
	...
	protected void configureLayout() {
		...
		IVMNode subExpressioNode = new GdbGlobalVariableVMNode(this, getSession(), varAccess);
		...
		((GlobalVariableVMNode) subExpressioNode)
				.setCastToTypeSupport(new DsfCastToTypeSupport(getSession(), GlobalVariableVMProvider.this, varAccess));
		...
		if (event instanceof DoubleClickEvent && !isDisposed()) {
			...
			if (selection instanceof IStructuredSelection) {
				...
				if (element instanceof IncompleteChildrenVMC) {
					...
					if (node instanceof GdbGlobalVariableVMNode && node.getVMProvider() == this) {
						...
						if (selection instanceof ITreeSelection) {
							...
							((GdbGlobalVariableVMNode) node).incrementChildCountLimit(exprCtx);
	...
	protected void handlePropertyChanged(final IPreferenceStore store, final PropertyChangeEvent event) {
		...
		} else if (IGlobalVariableConstants.SHOW_FULL_PATHS_PROPERTY.equals(property)) {
			getPresentationContext().setProperty(IGlobalVariableConstants.SHOW_FULL_PATHS_PROPERTY,
					event.getNewValue());
			processEvent = true;
		}
		if (processEvent) {

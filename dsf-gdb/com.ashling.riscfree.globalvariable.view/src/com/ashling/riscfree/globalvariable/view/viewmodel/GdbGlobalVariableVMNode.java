...
import com.ashling.riscfree.globalvariable.view.dsf.ui.Messages;
import com.ashling.riscfree.globalvariable.view.viewmodel.GlobalVariableVMNode.VariableExpressionFactory;
import com.ashling.riscfree.globalvariable.view.viewmodel.GlobalVariableVMNode.VariableExpressionVMC;
 ...
 */
/**
 * @implNote this class is copied from GDBVariableVMNode.java in CDT
 * @author vinod.appu
 *
 */
public class GdbGlobalVariableVMNode extends GlobalVariableVMNode {
	 ...
	 */
	public class GdbVariableExpressionVMC extends GlobalVariableVMNode.VariableExpressionVMC
	...
	public class IncompleteChildrenVMC extends AbstractVMContext {
		...
		public IncompleteChildrenVMC(IExpressionDMContext exprDmc, int childCountLimit) {
			super(GdbGlobalVariableVMNode.this);
	...
	private static Status internalError() {
		return new Status(Status.ERROR, Activator.PLUGIN_ID, Messages.Internal_Error);
	}

	/**
	 * Constructor (passthru)
	 */
	public GdbGlobalVariableVMNode(AbstractDMVMProvider provider, DsfSession session,
						...
						new ViewerDataRequestMonitor<Boolean>(getSession().getExecutor(), update) {
							...
							protected void handleCompleted() {
								...
								if (limitRequired) {
									...
									GdbGlobalVariableVMNode.super.updateElementCountInSessionThread(update);
								}
							}
						});
	...
	public int getDeltaFlags(Object e) {
		...
		if (e instanceof FetchMoreChildrenEvent) {
			...
			flags |= IModelDelta.CONTENT;
		} else if (e instanceof IGlobalsChangedEvent) {
			// The child count limit must be reset.
			flags |= IModelDelta.CONTENT;
			...
			if (IGdbDebugPreferenceConstants.PREF_INITIAL_CHILD_COUNT_LIMIT_FOR_COLLECTIONS.equals(property) ||
					IGlobalVariableConstants.SHOW_FULL_PATHS_PROPERTY.equals(property)) {
				flags |= IModelDelta.CONTENT;
			}
		}
	...
	public void buildDelta(Object e, VMDelta parentDelta, int nodeOffset, RequestMonitor rm) {
		...
		if (e instanceof FetchMoreChildrenEvent) {
			...
			resetChildCountLimits(((IExecutionDMContext) ((IGlobalsChangedEvent) e).getDMContext()));
		} else if (e instanceof PropertyChangeEvent) {
			String property = ((PropertyChangeEvent) e).getProperty();
			if (IGdbDebugPreferenceConstants.PREF_INITIAL_CHILD_COUNT_LIMIT_FOR_COLLECTIONS.equals(property)) {
	...
	public void buildDeltaForExpressionElement(Object element, int elementIdx, Object event, VMDelta parentDelta,
		...
		} else if (event instanceof IGlobalsChangedEvent) {
			resetChildCountLimits(((IExecutionDMContext) ((IGlobalsChangedEvent) event).getDMContext()));

...
package com.ashling.riscfree.globalvariable.view.viewmodel;
...
import com.ashling.riscfree.globalvariable.view.IGlobalVariableConstants;
...
import com.ashling.riscfree.globalvariable.view.dsf.IGlobalsChangedEvent;

/**
 * @implNote this class is copied from VariableVMNode.java in CDT
 * @author vinod.appu
 *
 */
public class GlobalVariableVMNode extends AbstractExpressionVMNode
	...
	public GlobalVariableVMNode(AbstractDMVMProvider provider, DsfSession session,
	...
	private void getLocalsUpdateElementsInSessionThread(final IChildrenUpdate update) {
		...
		final IExpressions expressionService = getServicesTracker().getService(IExpressions.class);
		ICommandControlDMContext context = DMContexts.getAncestorOfType(frameDmc, ICommandControlDMContext.class);
		IGlobalVariableService globalvariableService = null;
		if (context != null) {
			//Track core specific service in case of heterogeneous multicore launch
			globalvariableService = MulticoreServiceUtility.trackCoreService(IGlobalVariableService.class,
					getServicesTracker(), context.getCommandControlId(), getSession());
		}
		if (globalvariableService == null) {
			globalvariableService = getServicesTracker().getService(IGlobalVariableService.class);
		}

		if (frameDmc == null || expressionService == null || globalvariableService == null) {
			handleFailedUpdate(update);
			return;
		}

		final IGlobalVariableService gobalService = globalvariableService;
		...
		final DataRequestMonitor<IGlobalVariableDMContext[]> rm = new ViewerDataRequestMonitor<IGlobalVariableDMContext[]>(
				dsfExecutor, update) {
			@Override
			public void handleCompleted() {
				if (!isSuccess()) {
				...
				IGlobalVariableDMContext[] localsDMCs = getData();
				...
				for (IGlobalVariableDMContext localDMC : localsDMCs) {
					...
					gobalService.getGlobalVariableData(localDMC, rm);
					countRM++;
				}
		...
		globalvariableService.getGlobals(frameDmc, rm);
	}
	...
	public int getDeltaFlags(Object e) {
		if (e instanceof IGlobalsChangedEvent || e instanceof ISuspendedDMEvent || e instanceof IMemoryChangedEvent
	...
	protected String getExpressionDisplayName(IExpressionDMContext dmc, String name) {
		...
		//name would be in '<file_name/path>'::<variable_name> format for global variables
		if (isShowFullPath() && name.contains("::")) {
			String variableName= name.substring(name.indexOf("::")+2);
			String path = name.substring(1, name.indexOf("::") - 1); // remove ''
			return new StringBuilder().append(variableName).append(" (").append(path).append(")").toString();
		}
		if (name.contains("::")) {
	...
	private boolean isShowFullPath() {
		return CDebugUIPlugin.getDefault().getPreferenceStore().getBoolean(IGlobalVariableConstants.SHOW_FULL_PATHS_PROPERTY);
	}
}

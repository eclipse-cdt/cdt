...
public class EpiphanyVisualizerCPU extends EpiphanyVisualizerContainer
{
	...
	/** Mesh links associated to this node */
	...
	public EpiphanyVisualizerCPU(int id, IEpiphanyConstants eConstants, String progName, Color coreColor, boolean loadEnabled) {
		...
		m_eCore.setVirtualBounds(eConstants.getEcoreBounds());
		...
		m_router.setVirtualBounds(eConstants.getRouterBounds());
		m_router.setSelectable(false);
		...
		for (LinkDirection d : LinkDirection.values()) {
			...
			l.setVirtualBounds(eConstants.getMeshLinksBounds()[i]);

			// mesh links are not selectable
	...
	/** When an Epiphany CPU is (de-)selected, reflect that by (de-)selecting its eCore */
	@Override

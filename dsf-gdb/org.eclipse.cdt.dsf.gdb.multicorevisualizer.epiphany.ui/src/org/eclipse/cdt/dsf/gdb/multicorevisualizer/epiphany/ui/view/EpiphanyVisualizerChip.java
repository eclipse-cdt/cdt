...
public class EpiphanyVisualizerChip extends EpiphanyVisualizerContainer {
	...
	private class ProcessColor {
		...
		// Colors that were previously used for the first few processes:
//		private static final Color[] PROC_COLORS = {
//		Colors.getColor(174, 141, 215), // violet
//		Colors.getColor(182, 215, 141), // green
//		Colors.getColor(215, 151, 141), // red
//		Colors.getColor(100, 178, 204), // blue
//		Colors.getColor(199, 204, 100)  // yellow
//	   };
		...
		public Color getColor(int pid) {
			...
			if (m_pidColorMap.containsKey(pid)) {
				...
				// generate random color - Skipping low values to make result more readable for visualizer (paler colors)
				// note: we want a given pid to always generate the same color, so we use it as the base of the seed.
				// Adding some salt experimentally to give a nicer color spread
				// TODO: test this when we have real PIDs coming from Epiphany / e-server to make sure
				// it works ok.
	...
	public EpiphanyVisualizerChip(IEpiphanyConstants constants) {
		...
		m_statusBar.setRelativeBounds(m_eConstants.getStatusBarBounds());
		...
		m_IOContainer.setRelativeBounds(m_eConstants.getEpiphanyIoContainerBounds());
		m_IOContainer.setDrawBounds(true);
		...
		m_CPUsContainer.setRelativeBounds(m_eConstants.getEpiphanyCpuContainerBounds());
	...
	public EpiphanyVisualizerIO addIO(IOPosition position, boolean connected) {
		...
		io.setRelativeBounds(m_eConstants.getEpiphanyIoBounds()[position.ordinal()]);
	...
	public EpiphanyVisualizerCPU addCPU(int id, int pid, String progName, boolean loadMetersEnabled) {
		...
		cpu.setRelativeBounds(m_eConstants.getEpiphanyCpuContainersBounds()[id]);

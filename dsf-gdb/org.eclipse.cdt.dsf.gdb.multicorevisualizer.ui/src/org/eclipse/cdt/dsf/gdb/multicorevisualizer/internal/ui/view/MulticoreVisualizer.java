...
{	
	...
	protected void initializeLoadMeterTimer() {
		...
		m_updateLoadMeterTimer = DSFDebugModel.getLoadTimer((DSFSessionState) m_dataSrc, m_loadMeterTimerPeriod, this);
	...
	public synchronized void disposeDataSource(String id) {
		...
		// It's possible that a session unknown to the visualizer ends 
		if (model == null) {
			return;
		}
	...
	private void setModelSrcMapping(IVisualizerModelDataSource dataSrc, VisualizerModel model) {
		m_dataSrcModelMap.put(dataSrc, model);
	}

	/** Removes a mapping between a model and its data source */
	private void clearModelSrcMapping(IVisualizerModelDataSource dataSrc) {
	...
	public boolean setDebugSession(String sessionId) {
			...
			sessionId != null)
		{
			...
			if (m_dataSrc == null) {
				...
				setModelSrcMapping(m_dataSrc, null);
			}
			changed = true;
		}
	...
	public void update(boolean recreateModel) {
		...
		if (recreateModel) {
			...
			clearModelSrcMapping(m_dataSrc);
		}
	...
	public synchronized void createVisualizerModel(IVisualizerModelDataSource dataSrc) {
		...
		if (dataSrc.isAvailable()) {
			...
			setModelSrcMapping(dataSrc, model);

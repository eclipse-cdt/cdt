...
import org.eclipse.cdt.dsf.gdb.multicorevisualizer.internal.utils.PersistentSettingsManager.PersistentSetting;
...
public class MulticoreVisualizer extends GraphicCanvasVisualizer implements IPinnable {
	...
	protected PersistentSetting<Boolean> m_loadMetersEnabled;
	...
	protected PersistentSetting<Integer> m_loadMeterTimerPeriod;
	 ...
	 * 
	 */
	protected void initializePersistentParameters() {
		// setting managers
		m_persistentSettingsManager = new PersistentSettingsManager(ECLIPSE_ID, m_visualizerInstanceId);  

		// define persistent parameters:
		m_loadMetersEnabled = m_persistentSettingsManager.getNewParameter(Boolean.class, 
				"enableLoadMeters", false, false); //$NON-NLS-1$
		m_loadMeterTimerPeriod = m_persistentSettingsManager.getNewParameter(Integer.class, 
				"loadMeterTimerPeriod", false, LOAD_METER_TIMER_MEDIUM); //$NON-NLS-1$
	}
	...
	protected void initializeLoadMeterTimer() {
		...
		m_updateLoadMeterTimer = getLoadTimer(m_sessionState, m_loadMeterTimerPeriod.value());		
	...
	public void setLoadMeterTimerPeriod(int p) {
		...
		if (m_loadMeterTimerPeriod.value() == p) return; 
	...
	public void setLoadMetersEnabled(boolean enabled) {
		...
		fDataModel.setLoadMetersEnabled(m_loadMetersEnabled.value());
		disposeLoadMeterTimer();
		initializeLoadMeterTimer();
	}
	...
	protected void createActions()
	{
		...
		m_enableLoadMetersAction = new EnableLoadMetersAction(m_loadMetersEnabled.value());
	...
	protected void updateActions()
	{
    	...
    	m_loadMetersRefreshSubSubmenu.setVisible(m_loadMetersEnabled.value());
	...
	public void getVisualizerModelDone(VisualizerModel model) {
		model.setLoadMetersEnabled(m_loadMetersEnabled.value());
		updateLoads(model);
		model.sort();

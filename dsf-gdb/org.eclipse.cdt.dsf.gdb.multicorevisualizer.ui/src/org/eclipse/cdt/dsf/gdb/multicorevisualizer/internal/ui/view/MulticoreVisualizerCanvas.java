	...
	implements ISelectionProvider
{
	...
	{
		m_model = model;
		// an old filter might not be applicable to a new model
		clearFilter();
		requestRecache();
		requestUpdate();
	}
	...
	public synchronized void requestRecache(boolean state, boolean sizes, boolean load) {
		...
		m_canvasFilter.genDynamicFilterList();
	}
...
//			// put status bar just under the CPU area
//			m_statusBar.setBounds(cpu_margin, y   , width - cpu_margin , statusBarHeight);
	...
	public void paintCanvas(GC gc) {
		...
		if (m_canvasFilter.isFilterActive()) {
			m_statusBar.setMessage(m_canvasFilter.getStats());

			...
			procService.getExecutionData(threadContext, 
				new ImmediateDataRequestMonitor<IThreadDMData>() {
					...
					protected void handleFailure() {
						// are we in all-stop mode? If so we can't ask GDB for more info about the
						// new thread at this moment.  So we still add it to the model, on core zero 
						// and with a OS thread id of zero.  The next time the execution is stopped, 
						// the model will be re-created and show the correct thread ids and cores.
						if (isSessionAllStop(sessionId) ) {
							// put it on core zero

...
public class MulticoreVisualizerLoadMeter extends MulticoreVisualizerGraphicObject {
	...
	private Color getLoadColor() {		
		int load = (m_currentLoad != null) ? m_currentLoad : 0;
		
		if(load < m_overloadThreshold) {
	...
	public void paintContent(GC gc) {
		...
		if (load < m_overloadThreshold && m_showOnlyIfOverload) 
			return;
		...
		int x,y,w,h;
		x = m_bounds.x;
		y = (int) (m_bounds.y + m_bounds.height * ((100.0f - load) / 100.0f));
		w = m_bounds.width;
		h = (int)  (m_bounds.height - m_bounds.height * ((100.0f - load) / 100.0f));

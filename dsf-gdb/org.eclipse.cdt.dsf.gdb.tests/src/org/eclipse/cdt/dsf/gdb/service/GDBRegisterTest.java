...
public class GDBRegisterTest extends CommonDsfTest {
	...
	protected void doSetupSession() {
		...
		miProcesses = new MIProcesses(session);
	...
	public void testRegisterPersistence$() throws Exception {
		...
		MIControlDMContext controlDmc = new MIControlDMContext(session.getId(), "TestControl");
		...
		IMIContainerDMContext containerA = miProcesses.createContainerContext(processDmcA, "container");
		IMIContainerDMContext containerB = miProcesses.createContainerContext(processDmcB, "container");
		...
		assertTrue(buildDescriptors[0].getContainerId().equals(PROCESS_A));
...
//		gdbRegisters.getRegisterGroups(container, newDataRequestMonitor());

...
public class LaunchUtilsTest {
    ...
    public void testGDBVersionComparison() {
    	List<Versions> versions = new ArrayList<>(10);
    	...
    	for (Versions v : versions) {
    		assertEquals("Comparing " + v.version1 + " and " + v.version2,
    				v.expectedResult,
    				GdbDebugServicesFactory.compareVersions(v.version1, v.version2));
    		assertEquals("Comparing " + v.version2 + " and " + v.version1,
    				-v.expectedResult,
    		        GdbDebugServicesFactory.compareVersions(v.version2, v.version1));
    	}
    }
}

...
public class DsfLoadSymbolsCommandHandler extends AbstractHandler {
	...
	void loadSymbols(final IWorkbenchPart part, final IStructuredSelection selection, String sessionId, final boolean all) {
		...
		dsfSession.getExecutor().execute(new DsfRunnable() {
			...
			public void run() {
				if (all) {
					queueLoadAllSymbols(dsfSession, part);
				} else {
	...
	@ConfinedToDsfExecutor("fSession.getExecutor()")
	private void queueLoadAllSymbols(final DsfSession session, final IWorkbenchPart part) {
	...
	private void doRefresh(DsfSession session, Object element) {
		if (element instanceof IWorkbenchPart) {
			IVMProvider provider = VMHandlerUtils.getVMProviderForPart((IWorkbenchPart) element);

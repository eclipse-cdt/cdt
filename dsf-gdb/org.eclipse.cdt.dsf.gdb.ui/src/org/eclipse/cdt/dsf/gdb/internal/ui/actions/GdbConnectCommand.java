...
public class GdbConnectCommand implements IConnect {
    						...
    						protected void handleCompleted() {
    							if (isSuccess() && getData()) {
    								...
    								if (backend.getSessionType() == SessionType.REMOTE) {
    									...
    									String processShortName = processPath.lastSegment();    									 										getAndCheckPathOnLocalSystem(processPath);
     ...
     * in order to attach the proceess instantly (in case of an absolute path) or 
     * to select the image from the dialog (in case of a relative path). 
     * */

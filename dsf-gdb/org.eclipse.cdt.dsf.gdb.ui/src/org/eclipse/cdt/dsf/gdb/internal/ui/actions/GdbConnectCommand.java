...
public class GdbConnectCommand implements IConnect {
	...
	// A map of processName to path, that allows us to remember the path to the
	// binary file
	// for a process with a particular name. We can then re-use the same binary
	// for another
	// process with the same name. This allows a user to connect to multiple
	// processes
	// with the same name without having to be prompted each time for a path.
	// This map is associated to the current debug session only, therefore the
	// user can
	// reset it by using a new debug session.
	// This map is only needed for remote sessions, since we don't need to
	// specify
	...
	public GdbConnectCommand(DsfSession session) {
		...
		fTracker = new DsfServicesTracker(GdbUIPlugin.getBundleContext(),
				session.getId());
	}
	...
	/**
	 * This job will prompt the user to select a set of processes to attach too.
	 * We need a job because prompter.handleStatus will block and we don't want
	 * to block the executor.
	 */
	protected class PromptForPidJob extends Job {
		...
		public PromptForPidJob(String name, boolean newProcessSupported,
				IProcessExtendedInfo[] procs, DataRequestMonitor<Object> rm) {
			super(name);
		...
		protected IStatus run(IProgressMonitor monitor) {
			IStatus promptStatus = new Status(
					IStatus.INFO,
					"org.eclipse.debug.ui", 200/* STATUS_HANDLER_PROMPT */, "", null); //$NON-NLS-1$//$NON-NLS-2$
			final IStatus processPromptStatus = new Status(IStatus.INFO,
					"org.eclipse.cdt.dsf.gdb.ui", 100, "", null); //$NON-NLS-1$//$NON-NLS-2$

			final IStatusHandler prompter = DebugPlugin.getDefault()
					.getStatusHandler(promptStatus);

			final Status NO_PID_STATUS = new Status(
					IStatus.ERROR,
					GdbUIPlugin.PLUGIN_ID,
					-1,
					LaunchMessages
							.getString("LocalAttachLaunchDelegate.No_Process_ID_selected"), //$NON-NLS-1$
					null);
			...
			try {
				PrompterInfo info = new PrompterInfo(fNewProcessSupported,
						fProcessList);
				Object result = prompter
						.handleStatus(processPromptStatus, info);
				if (result == null) {
					...
					// This is the case where the user typed in a pid number
					// directly
					fRequestMonitor
							.setData(new IProcessExtendedInfo[] { new ProcessInfo(
									(Integer) result, "") }); //$NON-NLS-1$
				} else {
					fRequestMonitor.setStatus(NO_PID_STATUS);
				}
	...
	/**
	 * This job will prompt the user for a path to the binary to use, and then
	 * will attach to the process. We need a job to free the executor while we
	 * prompt the user for a binary path. Bug 344892
	 */
	private class PromptAndAttachToProcessJob extends UIJob {
		...
		public PromptAndAttachToProcessJob(String pid, String title,
				String procName, RequestMonitor rm) {
			super(""); //$NON-NLS-1$
			fPid = pid;
			fTitle = title;
			fProcName = procName;
		...
		public IStatus runInUIThread(IProgressMonitor monitor) {
			...
			if (binaryPath == null) {
				...
				if (shell != null) {
					...
					if (supposedBinaryPath == null) {
						...
						try {
							if (fSharedLibLocations != null
									&& fSharedLibLocations.size() > 0) {
			...
			if (binaryPath == null) {
				// The user pressed the cancel button, so we cancel the attach
				// gracefully
				fRm.done();
			} else {
				...
				fExecutor.execute(new DsfRunnable() {
					@Override
					public void run() {
						IGDBProcesses procService = fTracker
								.getService(IGDBProcesses.class);
						ICommandControlService commandControl = fTracker
								.getService(ICommandControlService.class);

						if (procService != null && commandControl != null) {
							IProcessDMContext procDmc = procService
									.createProcessContext(
											commandControl.getContext(), fPid);
							procService.attachDebuggerToProcess(procDmc,
									finalBinaryPath,
									new DataRequestMonitor<IDMContext>(
											fExecutor, fRm) {
										@Override
										protected void handleSuccess() {
											// Store the path of the binary so
											// we can use it again for another
											// process
											// with the same name. Only do this
											// on success, to avoid being stuck
											// with
							...
							fRm.setStatus(new Status(IStatus.ERROR,
									GdbUIPlugin.PLUGIN_ID,
									IDsfStatusConstants.INTERNAL_ERROR,
									"Cannot find services", null)); //$NON-NLS-1$
							fRm.done();
						}
					}
				});
	...
	public void connect(RequestMonitor requestMonitor) {
		...
		fExecutor.execute(new DsfRunnable() {
			@Override
			public void run() {
				final IProcesses procService = fTracker
						.getService(IProcesses.class);
				ICommandControlService commandControl = fTracker
						.getService(ICommandControlService.class);

				if (procService != null && commandControl != null) {
					final ICommandControlDMContext controlCtx = commandControl
							.getContext();
					procService.isDebugNewProcessSupported(controlCtx,
							new DataRequestMonitor<Boolean>(fExecutor, null) {
								@Override
								protected void handleCompleted() {
									final boolean newProcessSupported = isSuccess()
											&& getData();

									procService
											.getRunningProcesses(
													controlCtx,
													new DataRequestMonitor<IProcessDMContext[]>(
															fExecutor, rm) {
														@Override
														protected void handleSuccess() {
															...
															final CountingRequestMonitor countingRm = new CountingRequestMonitor(
																...
																protected void handleSuccess() {
																	new PromptForPidJob(
																			LaunchUIMessages
																					.getString("ProcessPrompter.PromptJob"), newProcessSupported, procInfoList.toArray(new IProcessExtendedInfo[0]), //$NON-NLS-1$
																			new DataRequestMonitor<Object>(
																					fExecutor,
																					rm) {
																				@Override
																				...
																				protected void handleSuccess() {
																					...
																					if (data instanceof String) {
																						...
																						rm.setStatus(new Status(
																								...
																								GdbUIPlugin.PLUGIN_ID,
																								IDsfStatusConstants.INTERNAL_ERROR,
																								"Invalid return type for process prompter", null)); //$NON-NLS-1$
																						rm.done();
																					}
																				}
																			})
															...
															if (getData().length > 0
																	&& getData()[0] instanceof IThreadDMData) {
																// The list of
																// running
																// processes
																// also contains
																...
																for (IProcessDMContext processCtx : getData()) {
																	...
																	if (processData instanceof IGdbThreadDMData) {
																		cores = ((IGdbThreadDMData) processData)
																				.getCores();
																		owner = ((IGdbThreadDMData) processData)
																				.getOwner();
																	}
																	procInfoList
																			.add(new ProcessInfo(
	...
	private void startNewProcess(ICommandControlDMContext controlDmc,
			String binaryPath, RequestMonitor rm) {
		final IGDBProcesses procService = fTracker
				.getService(IGDBProcesses.class);
		procService.debugNewProcess(controlDmc, binaryPath,
				new HashMap<String, Object>(),
				new DataRequestMonitor<IDMContext>(fExecutor, rm));
	}

	private void attachToProcesses(final ICommandControlDMContext controlDmc,
			IProcessExtendedInfo[] processes, final RequestMonitor rm) {
		...
		if (procService != null && backend != null) {
			...
			// to be able to check if we are allowed to attach to the next
			// process.
			// Attaching to all of them in parallel would assume that all attach
			// are supported.

			// Create a list of all our processes so we can attach to one at a
			// time.
			// We need to create a new list so that we can remove elements from
			// it.
			final List<IProcessExtendedInfo> procList = new ArrayList<IProcessExtendedInfo>(
					Arrays.asList(processes));

			class AttachToProcessRequestMonitor extends
					ImmediateDataRequestMonitor<IDMContext> {
				...
				protected void handleCompleted() {
					if (!isSuccess()) {
						// Failed to attach to a process. Just ignore it and
						// move on.
					}
					...
					if (procList.size() > 0) {
						...
						// This is because some backends may not support
						// multi-process.
						// If the backend does not support multi-process, we
						// only attach to the first process.
						procService.isDebuggerAttachSupported(controlDmc,
								new ImmediateDataRequestMonitor<Boolean>() {
									...
									protected void handleCompleted() {
										if (isSuccess() && getData()) {
											...
											// Remove process from list and
											// attach to it.
											IProcessExtendedInfo process = procList
													.remove(0);
											String pidStr = Integer
													.toString(process.getPid());

											if (backend.getSessionType() == SessionType.REMOTE) {
												// For remote attach, we must
												// set the binary first so we
												// need to prompt the user.

												// If this is the very first
												// attach of a remote session,
												// check if the user
												// specified the binary in the
												// launch. If so, let's add it
												// to our map to
												// avoid having to prompt the
												// user for that binary.
												// This would be particularly
												// annoying since we didn't use
												// to have
												// to do that before we
												// supported multi-process.
												// Must do this here to be in
												// the executor
												// Bug 350365
												if (fProcessNameToBinaryMap
														.isEmpty()) {
													IPath binaryPath = backend
															.getProgramPath();
													if (binaryPath != null
															&& !binaryPath
																	.isEmpty()) {
												...
												// Because the prompt is a very
												// long operation, we need to
												// run outside the
												// executor, so we don't lock
												// it.
												// Bug 344892
												IPath processPath = new Path(
														process.getName());
												String processShortName = processPath
														.lastSegment();
												...
												new PromptAndAttachToProcessJob(
														pidStr,
														LaunchUIMessages
																.getString("ProcessPrompterDialog.TitlePrefix") + process.getName(), //$NON-NLS-1$
														processShortName,
											...
											// Not allowed to attach to another
											// process. Just stop.
											rm.done();
										}
									}
	...
	/*
	 * 
	 * Checks whether the process's path appended to a shared Library Location is a valid one, 
	 * in order to attach the proceess instantly (in case of an absolute path) 
	 * or to select the image from the dialog (in case of a relative path).
	 */
	private void getAndCheckPathOnLocalSystem(IPath processPath) {
		try {
			...
			if (processPath.isAbsolute()) {
				// absolute path: append to the first item in the list of
				// sharedlibs
				// treating the first item in the sharedlib location list as the
				// sysroot
				if (fSharedLibLocations.size() > 0) {
					...
					if (folder.exists() && folder.isDirectory()) {
						File file = new File(folder, processPath.toFile()
								.getPath());
						if (file.exists()) {
							// Placing it in here ensures that the Dialog won't
							// come up
				...
				for (String value : fSharedLibLocations) {
					...
					if (folder.exists() && folder.isDirectory()) {
						...
						if (file.exists()) {
							// Placing it in here makes the Dialog come up for
							// review

...
public class GdbDisconnectCommand implements IDisconnectHandler {
	...
	public void canExecute(final IEnabledStateRequest request) {
        ...
        getContainerDMContexts(request, new DataRequestMonitor<IContainerDMContext[]>(fExecutor, null) {
        	@Override
    ...
    private void getContainerDMContexts(final IDebugCommandRequest request, final DataRequestMonitor<IContainerDMContext[]> rm) {
            	...
            	fExecutor.execute(new DsfRunnable() { 
                    ...
                    public void run() {
                        ...
                        if (commandControl != null && procService != null) {
                        	...
                        	request.setStatus(new Status(IStatus.ERROR, GdbUIPlugin.PLUGIN_ID, "Service is not available.")); //$NON-NLS-1$
                        	request.done();
                        }
                     }
                });
            } catch (RejectedExecutionException e) {
            	request.setStatus(new Status(IStatus.ERROR, GdbUIPlugin.PLUGIN_ID, e.getLocalizedMessage()));
                request.done();
            }    		
    	}
    }

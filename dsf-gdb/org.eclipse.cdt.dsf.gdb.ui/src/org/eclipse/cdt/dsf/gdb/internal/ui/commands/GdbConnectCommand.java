...
public class GdbConnectCommand extends RefreshableDebugCommand implements IConnectHandler, IConnect {
    ...
    private void getAllDebuggedProcesses(final IDMContext currentCtx, boolean allSessions, final DataRequestMonitor<List<String>> drm) {
    	...
    	// Only for local session types search in all other sessions
    	...
    	final class ProcessRequestMonitor extends DataRequestMonitor<IDMContext[]> {
			...
			protected void handleCompleted() {
				...
				if (!sessions.isEmpty()) {
					final DsfSession session = sessions.remove(0);
					final boolean sameSession = currentCtx.getSessionId().equals(session.getId());
					session.getExecutor().execute(new DsfRunnable() {						
						...
						public void run() {
	    					...
	    					if (sameSession || backend.getSessionType() == SessionType.LOCAL) {
	    						...
	    						procService.getProcessesBeingDebugged(
	    								commandControl.getContext(), new ProcessRequestMonitor(session.getExecutor()));
	    					} else {
	    						// proceed to next session context query
	    						new ProcessRequestMonitor(session.getExecutor()).done(new Status(IStatus.ERROR, GdbUIPlugin.PLUGIN_ID, IDsfStatusConstants.NOT_SUPPORTED, "Only local session", null)); //$NON-NLS-1$
	    					}
	    					tracker.dispose();
		    			}
					});

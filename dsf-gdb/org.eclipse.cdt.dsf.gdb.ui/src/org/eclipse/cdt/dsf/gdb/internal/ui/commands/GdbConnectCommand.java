...
public class GdbConnectCommand extends RefreshableDebugCommand implements IConnectHandler, IConnect {
    ...
    private class PromptAndAttachToProcessJob extends UIJob {
    	...
    	public IStatus runInUIThread(IProgressMonitor monitor) {
    		...
    		if (binaryPath == null) {
				...
				if (shell != null) { // why does the shell is empty
     ...
     * It appends the relative path to the sysroot provided from the launch configuraton dialog
				...
				if (folder.exists() && folder.isDirectory()) {
					if ( processPath.isAbsolute() ) {
						...
						if (file.exists()) {
							...
							//TODO maybe we can check subfolders that contains the binary file with the same name
							//File path not known
						}
					}
				}

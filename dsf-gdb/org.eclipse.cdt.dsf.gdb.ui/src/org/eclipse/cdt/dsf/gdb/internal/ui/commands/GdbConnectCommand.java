...
public class GdbConnectCommand extends RefreshableDebugCommand implements IConnectHandler, IConnect {
     ...
     */
     private void getAndCheckPathOnLocalSystem(IPath processPath) {
		try {
			GDBBackend service = (GDBBackend)fTracker.getService(IGDBBackend.class);
			fSysrootLocation = service.getSystemRootPath();
				...
				if (folder.exists() && folder.isDirectory()) {
					if ( processPath.isAbsolute() ) {
						...
						if (file.exists()) {
							...
							//currently opens the FileDialog to select the binary
						}
					}
				}
			}
		} catch (ClassCastException e){
			GdbUIPlugin.getDefault().getLog().log(new Status(IStatus.ERROR,GdbUIPlugin.PLUGIN_ID,"Failed to cast IGDBBackend into GDBBackend",e)); //$NON-NLS-1$
			return;
		} catch (CoreException e) {
			// Failed to obtain any sysroot path, so we cannot try mapping the remote process path - return directly.
			GdbUIPlugin.getDefault().getLog().log(new Status(IStatus.ERROR,GdbUIPlugin.PLUGIN_ID,"Remote attach failed to get tracker",e)); //$NON-NLS-1$
			return;
		}
     }
}
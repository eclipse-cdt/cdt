...
public class GdbReverseToggleCommand extends AbstractDebugCommand implements IChangeTraceMethodHandler {
            ...
            public void execute(final DataRequestMonitor<Object> rm) {
       			...
       			if (runControl != null) {
                       ...
                       if (runControl_old != null) {
                           if(fTraceMethod != -1 && fTraceMethod != 0) {
    ...
    @Override
    public void setTraceMethod(int traceMethod) {
        fTraceMethod = traceMethod;
    }

    @Override
    public int getTraceMethod(final Object context) {
        IDMContext dmc;
        ...
        if (runControl != null) {
            Query<Integer> ReverseMethodQuery = new Query<Integer>() {
                @Override
                public void execute(final DataRequestMonitor<Integer> rm) {
                    runControl.getReverseTraceMethod(controlDmc, rm);
                }
            };
            try {
                fExecutor.execute(ReverseMethodQuery);
                return ReverseMethodQuery.get();
            } catch (InterruptedException e) {
            } catch (ExecutionException e) {
            } catch (RejectedExecutionException e) {
                // Can be thrown if the session is shutdown
            ...
            return isReverseToggled(context) ? 0 : -1 ;
        }
        return -1;
    }
}

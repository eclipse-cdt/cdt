...
final class GdbTerminalConnector implements IGdbTerminalControlConnector {
	 ...
	 * preferences
	 */
	private final StringBuffer fHistoryRemainder = new StringBuffer();
	 ...
	 */
	private void appendHistory(byte[] b, int read) {
		// Read this new input
		...
		for (int i = 0; i < chunks.length; i++) {
			...
			if (i == 0) {
				synchronized (fHistoryRemainder) {
					// Add the previous incomplete line info ("remainder") first
					lineBuilder.append(fHistoryRemainder.toString());

...
final class GdbTerminalConnector implements IGdbTerminalControlConnector {
	private static final int HIST_BUFFER_SIZE_DEFAULT = 1000; /* lines */
	...
	public GdbTerminalConnector(Process process) {
		...
		// Use the preference store to Resolve the maximum number of lines to save in the history buffer
		IPreferenceStore store = GdbUIPlugin.getDefault().getPreferenceStore();
		int prefBufferLines = store.getInt(IGdbDebugPreferenceConstants.PREF_CONSOLE_BUFFERLINES);
		int max_history_buffer_size = prefBufferLines > HIST_BUFFER_SIZE_DEFAULT ? prefBufferLines
				: HIST_BUFFER_SIZE_DEFAULT;

		fHistoryBuffer = new ConsoleHistoryLinesBuffer(max_history_buffer_size);
		...
		public synchronized void appendHistory(byte[] b, int read) {
			...
			for (int i = 0; i < chunks.length; i++) {
				...
				if (line.endsWith("\n")) { //$NON-NLS-1$
					...
					while (this.size() >= HIST_BUFFER_SIZE_DEFAULT) {
						this.remove();
					}
	...
	private class WriteHistoryJob extends Job {
		...
		protected IStatus run(IProgressMonitor monitor) {
			...
			// Add the accumulated remainder value i.e. not yet a complete line in the buffer

...
public class DebugSourcesView extends ViewPart implements IDebugContextListener {
	...
	public void createPartControl(Composite parent) {
		...
		comparator = new DebugSourcesViewComparator<Object>();
	...
	private TreeViewerColumn createTreeViewerColumn(String title, int bound, final int colNumber) {
		...
		if (colNumber == 0)
			comparator.setColumn(e -> ((DebugTree<?>) e).getData(), colNumber);
		if (colNumber == 1)
			comparator.setColumn(e -> ((DebugTree<?>) e).getLeafData() != null ? ((DebugTree<?>) e).getLeafData() : ((DebugTree<?>) e).getData(), colNumber);
	...
	private SelectionAdapter getSelectionAdapter(final TreeColumn column, final int index) {
		SelectionAdapter selectionAdapter = new SelectionAdapter() {
			...
			public void widgetSelected(SelectionEvent e) {
				if (index == 0)
					comparator.setColumn(e1 -> ((DebugTree<?>) e1).getData(), index);
				if (index == 1)
					comparator.setColumn(e1 -> ((DebugTree<?>) e1).getLeafData() != null ? ((DebugTree<?>) e1).getLeafData() : ((DebugTree<?>) e1).getData(), index);

...
public class DebugSourcesViewComparator<T> extends ViewerComparator {
	...
	private enum Direction {
		...
		/**
		 * 
		 * @param value the ordinal value that is associated with the direction
	...
	public int getDirection() {
		return direction.equals(Direction.DESCENDING) ? SWT.DOWN : SWT.UP;
	}
	...
	public void setColumn(Function<T, Comparable<?>> column, int idx) {
		...
		if (idx == this.propertyIndex) {
			...
			direction = Direction.getDirection(1 - direction.ordinal());
		} else {
			// New column

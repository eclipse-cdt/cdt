...
public class AttachProcessHandler extends AbstractHandler implements IHandler {
	...
	public Object execute(ExecutionEvent event) throws ExecutionException {
		...
		if (selection instanceof IStructuredSelection 
				&& part instanceof OSResourcesView ) {
			...
			if (!attach_steps.isEmpty()) {
				Job.create(Messages.OSView_AttachJobName, new ICoreRunnable() {
					...
					public void run(IProgressMonitor monitor) throws CoreException {
						DsfSession session = DsfSession.getSession(context.getSessionId());
						Sequence sequence = new Sequence(session.getExecutor(), monitor, Messages.OSView_AttachJobName, ""){ //$NON-NLS-1$
							@Override
		...
		try {
			...
			if (procService != null && backend != null) {
				...
				procService.attachDebuggerToProcess(procDmc,
						new DataRequestMonitor<IDMContext>(DsfSession.getSession(context.getSessionId()).getExecutor(), rm) {
					@Override

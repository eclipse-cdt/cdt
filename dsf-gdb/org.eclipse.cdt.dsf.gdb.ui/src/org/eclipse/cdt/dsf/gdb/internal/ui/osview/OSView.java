...
public class OSView extends ViewPart implements DsfSession.SessionEndedListener {
	
	public static final String ID = "org.eclipse.cdt.dsf.gdb.ui.os.view"; //$NON-NLS-1$
	...
	public void createPartControl(Composite xparent) {
		...
		/* Unless we do this, the view's menu won't be shown, and it
		 * won't show even if we add actions to it later.
		 */
		...
		fResourceClass = fResourceClassEditor.getResourceClass();
	...
	public void dispose() {
		...
		setDebugContext((IDMVMContext)null);
	}	
	
	private void setDebugContext(ISelection s) {

		IDMVMContext context = null;
	...
	private void setDebugContext(IDMVMContext context)
	{
		...
		if (context != null)
		{
			...
			newSession = DsfSession.getSession(dmContext.getSessionId());			
		
			fSessionData = fSessionDataCache.get(newSession.getId());
			if (fSessionData == null)
			{
				IExecutionDMContext dmExecutionContext
					= DMContexts.getAncestorOfType(dmContext, IExecutionDMContext.class);
							
				fSessionData = new SessionOSData(newSession, dmExecutionContext);
	...
	public void sessionEnded(DsfSession session) {
		...
		if (fSessionDataCache.containsKey(id))
		{
			SessionOSData data = fSessionDataCache.get(id);
			data.dispose();
			fSessionDataCache.remove(id);
		}	
	}	
	...
	private void update() {
		...
		else
		{
			...
			if (fSessionData.fetchingNow())
			{
				hideTable(Messages.OSView_6);
			}
			else if (data == null)
			{
				hideTable(Messages.OSView_7);
			}
			else
			{
				SimpleDateFormat format = new SimpleDateFormat(Messages.OSView_8);
				setContentDescription(format.format(fSessionData.timestamp(fResourceClass)));
				if (data != fShownData)
				{
					fShownData = data;
	...
	private void populateTable(final OSData data)
	{
		...
		for (int i = 0; i < data.getColumnCount(); ++i) {
			...
			c.addSelectionListener(new SelectionAdapter() {
				...
				public void widgetSelected(SelectionEvent e) {					
					...
					fComparator.setColumn(final_index, data.columnIsInteger(final_index));
		...
		if (sortColumn < data.getColumnCount())
		{
			fComparator.setColumn(sortColumn,
					data.columnIsInteger(sortColumn));
		}
		fComparator.setDirection(fColumnLayout.getSortDirection());
		
		fViewer.getTable().setEnabled(true);
		fViewer.setContentProvider(data);
		fViewer.setLabelProvider(data);

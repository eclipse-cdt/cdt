			...
			if (dmc instanceof IMIExecutionDMContext || dmc instanceof IFrameDMContext) {
				...
				session.getExecutor().execute(new Runnable() {
					...
					public void run() {
						...
						if (gdbSync != null) {
							gdbSync.setCurrentGDBThread(executionDMC);
							
							// If the selected context is a frame, switch to this frame as well
							if (dmc instanceof IFrameDMContext) {
								gdbSync.setCurrentGDBStackFrame((IFrameDMContext) dmc);
							}
						}

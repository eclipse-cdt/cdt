...
public class TraceControlModel {
	...
	private IGDBTraceControl fGDBTraceControl;
	...
	public void setCurrentTraceRecord(final String traceRecordId) {
		...
		getSession().getExecutor().execute(
			new DsfRunnable() {	
				...
				public void run() {
					if (fTargetContext != null && fGDBTraceControl != null) {
						final ITraceRecordDMContext nextRecord = fGDBTraceControl.createTraceRecordContext(fTargetContext, traceRecordId);
	...
	public void setTraceUser(final String user) {
		if (getSession() == null) {
			return;
		}
		...
		getSession().getExecutor().execute(
			new DsfRunnable() {	
				...
				public void run() {
					if (fTargetContext != null && fGDBTraceControl != null && fGDBTraceControl instanceof IGDBTraceControl2) {
						((IGDBTraceControl2)fGDBTraceControl).setTraceUser(fTargetContext, user, new ImmediateRequestMonitor());
					}
				}
			});
	}

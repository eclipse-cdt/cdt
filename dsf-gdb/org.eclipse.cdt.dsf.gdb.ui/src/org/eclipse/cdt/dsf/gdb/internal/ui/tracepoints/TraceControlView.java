...
public class TraceControlView extends ViewPart implements IViewPart, SessionEndedListener {
	 ...
	 */
	protected final class RefreshViewAction extends Action {
		public RefreshViewAction() {
			setText(TracepointsMessages.TraceControlView_refresh_action_label);
	...
	public void createPartControl(Composite parent) {
		...
		// ScrolledComposite changes default background to grey, dunno why, 
		// to avoid this set explicitly background and tell children to use it as default. 
	...
	private void createUserLine(final Composite parent) {
		...
		fUserNameText.addKeyListener(new KeyListener() {
			...
			public void keyPressed(KeyEvent e) {
				if (e.keyCode == 0x1b) {
					...
					handleEditUserButtonPressed(true);
				}
			}
		});
	...
	protected void handleEditUserButtonPressed() {
		handleEditUserButtonPressed(false);
	}
	
	protected void handleEditUserButtonPressed(boolean cancelEditing) {
	...
	private void createNotesLine(final Composite parent) {
		...
		fNotesContentText.addKeyListener(new KeyListener() {
	...
	private void createStopNotesLine(final Composite parent) {
		...
		fStopNotesContentText.addKeyListener(new KeyListener() {
			...
			public void keyPressed(KeyEvent e) {
				if (e.keyCode == 0x1b) {
					// Esc was pressed, cancel editing
					handleEditStopNotesButtonPressed(true);
				}
			}
		});
	...
	protected void handleEditStopNotesButtonPressed() {
		handleEditStopNotesButtonPressed(false);
	}
	protected void handleEditStopNotesButtonPressed(boolean cancelEditing) {
	...
	private void createBufferLine(final Composite parent) {
		...
		// Extra composite to workaround aligning problems
		Composite c = new Composite(fBufferComposite, SWT.NONE);
		c.setLayoutData(new GridData(SWT.END, SWT.CENTER, false, false));
		GridLayout lc = new GridLayout(1, false);
		lc.marginHeight = 0;
		c.setLayout(lc);
		fSetCircularBufferButton = new Button(c, SWT.TOGGLE);
		...
		if (traceData == null || !traceData.isTracingSupported()) {
			...
			asyncExec(new Runnable() {
				...
				public void run() {
	        		...
	        		if ( tData.getStartTime() != null  && tData.getStopTime() != null 
	        				&& tData.getStartTime().length() > 0 && tData.getStopTime().length() > 0 ) {
	        			s += TracepointsMessages.bind(TracepointsMessages.TraceControlView_trace_status_offline_details,
	        				formatTime(tData.getStartTime()), formatTime(tData.getStopTime()) );
	        		}
	    			...
	    			// Stop reason and note.
	    			setStopLineVisible(true, tData);
	    			setStopNotesLineVisible(true, tData, true);
			...
			asyncExec(new Runnable() {
				...
				public void run() {
	    			...
	    			if (fStopReason != null) {
	    				// Tracing has stopped we need stop reason line, stop notes and frames slider
	    				setStopLineVisible(true, tData);
		    			setStopNotesLineVisible(true, tData, false);
		    			setFrameLineVisible(true, tData);
	    			} else {
	    				// Tracing is running, don't show stop reason line, stop notes and frames line. 
		    			setStopLineVisible(false, tData);
		    			setStopNotesLineVisible(false, tData, false);
	    				setFrameLineVisible(false, tData);
	    				if (fAutoRefreshAction.isChecked()) {
	    					startRefreshUIJob();
	    				}
	    			}
	...
	protected void updateActionEnablement() {
		...
		if (fStatusLabel != null && fStatusLabel.getText().toLowerCase().indexOf("off") != -1) { //$NON-NLS-1$
			fExitVisualizationAction.setEnabled(false);
		} else {
			fExitVisualizationAction.setEnabled(fTraceVisualization);
		}
	}

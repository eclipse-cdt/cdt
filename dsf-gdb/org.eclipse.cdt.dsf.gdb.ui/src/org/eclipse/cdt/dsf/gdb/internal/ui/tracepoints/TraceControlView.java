...
import org.eclipse.cdt.dsf.gdb.internal.ui.tracepoints.TraceControlModel.TraceStatusData;
...
public class TraceControlView extends ViewPart implements IViewPart {
	...
	protected void setFrameLineVisible(boolean visible, TraceStatusData traceData) {
		fFrameComposite.setVisible(visible);
		((GridData)fFrameComposite.getLayoutData()).exclude = !visible;
		
		if (visible) {
			fFrameSlider.setMinimum(0);
			fFrameSlider.setMaximum(traceData.tData.getNumberOfCollectedFrame());
			int inc = traceData.tData.getNumberOfCollectedFrame() / 20;
			fFrameSlider.setPageIncrement(inc <= 1 ? 2 : inc);
	
			String fl = EMPTY_STRING;
			if (traceData.currentTraceFrame != null) {
				fl += TracepointsMessages.bind(TracepointsMessages.TraceControlView_frame_looking,
						new Object[] { traceData.currentTraceFrame, 
										new Integer(traceData.tracepointIndexForCurrentTraceRecord)} );
				int recId = 0;
				try {
					recId = Integer.parseInt(traceData.currentTraceFrame);
				} catch (NumberFormatException e) {
				}
				fFrameSlider.setSelection(recId);
			} else {
				fl += TracepointsMessages.bind(TracepointsMessages.TraceControlView_frame_not_looking, traceData.tData.getNumberOfCollectedFrame());
				fFrameSlider.setSelection(0);
			}
			fFrameNumberLabel.setText(fl);
		}
		if (traceData != null && traceData.tData != null) {
			fFrameSlider.setEnabled(traceData.tData.getNumberOfCollectedFrame() != 0);
		}
	}
	...
	protected void updateUI(final TraceStatusData traceData) {
		...
		if (traceData == null ) {
			...
			if (getTimeMilliseconds(traceData.tData.getStartTime()) != 0 && getTimeMilliseconds(traceData.tData.getStopTime()) != 0) {
				setSecondaryStatusLineVisible(true, traceData.tData);
			} else {
				setSecondaryStatusLineVisible(false, traceData.tData);
			}
			setNotesLineVisible(true, traceData.tData, true);			
			setFrameLineVisible(true, traceData);	    			
	    			
			updateActionEnablement(traceData);
			updateLayout();
			
		} else if (!traceData.tData.isTracingActive() && traceData.tData.getStopReason() == null){
			...
			setSecondaryStatusLineVisible(false, null);
			setNotesLineVisible(true, traceData.tData, false);  			
			setBufferLineVisible(true, traceData.tData, false);
			setFrameLineVisible(false, null);
			...
			IGDBTraceControl.STOP_REASON_ENUM fStopReason = traceData.tData.getStopReason();
			if (fStopReason != null) {
				...
				setBufferLineVisible(true, traceData.tData, true);
	...
	protected void updateActionEnablement(TraceStatusData data) {
		fOpenTraceVarDetails.setEnabled(data != null && data.tData.isTracingSupported());
		fExitVisualizationAction.setEnabled(data != null && data.currentTraceFrame != null && !data.tData.isOfflineTracing());
	}

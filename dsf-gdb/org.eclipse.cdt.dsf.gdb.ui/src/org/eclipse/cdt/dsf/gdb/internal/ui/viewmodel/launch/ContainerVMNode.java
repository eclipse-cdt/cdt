...
{
    class ContainerVMC extends DMVMContext
    {
        private final IDMVMContext[] fParents;

        public ContainerVMC(IDMContext dmc, IDMVMContext[] parents) {
            super(dmc);
            fParents = parents;
        }
        
        public IDMVMContext[] getParents() { return fParents; }

        protected boolean baseEquals(Object other) {
            if (other == null) return false;
            if ( !(other.getClass().equals(getClass()))) return false;
            ContainerVMC otherCtx = (ContainerVMC)other;
            return //getSessionId().equals(otherCtx.getSessionId()) &&
                   areParentsEqual(otherCtx.fParents);
        }

        private boolean areParentsEqual(IDMVMContext[] otherParents) {
        	IDMVMContext[] parents = fParents;
...
//    	return new ContainerVMC(dmc, new IDMVMContext[0]);
    	return super.createVMContext(dmc);
    }
	...
	protected void updateElementsInSessionThread(final IChildrenUpdate update) {
		...
		// if they don't belong to a group.

...
public class GdbStackFramesVMNode extends StackFramesVMNode {
	...
	public void buildDelta(final Object e, final VMDelta parentDelta, final int nodeOffset, final RequestMonitor rm) {
		if (e instanceof IGDBFocusChangedEvent) {
			buildDeltaForFocusChangedEvent(parentDelta, rm);
		}
		else {
	...
	private void buildDeltaForFocusChangedEvent(VMDelta parentDelta, RequestMonitor rm) {
		getSession().getExecutor().execute(new Runnable() {
			...
			public void run() {
				...
				if (newFrame != null) {
					IMIExecutionDMContext execDmc = DMContexts.getAncestorOfType(newFrame, IMIExecutionDMContext.class);
					if (execDmc == null) {
						rm.done();
						return;
					}

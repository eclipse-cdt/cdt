    ...
    implements IElementLabelProvider, IElementMementoProvider 
{
    ...
    @Override
    public void getContextsForEvent(VMDelta parentDelta, final Object event, final DataRequestMonitor<IVMContext[]> rm) {
    		...
    		getSession().getExecutor().execute(new Runnable() { 
    			@Override public void run() {
    				...
    				if (tracker != null) {
    					...
    					groupingService.getGroupsForExecutionContext(execCtx, 
    							new DataRequestMonitor<IGroupDMContext[]>(getExecutor(), rm) {
    								@Override
    								public void handleCompleted() {
    									if (isSuccess()) {
    										IGroupDMContext[] r = getData();
    										IVMContext[] r2 = new IVMContext[r.length];
    										for (int i = 0; i < r.length; i++) {
    											r2[i] = createVMContext(r[i]);
    										}
    										rm.setData(r2);
    									}
    									else {

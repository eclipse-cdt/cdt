    ...
    implements IElementLabelProvider, IElementMementoProvider
{
    ...
    private void buildDeltaForFocusChangedEvent(VMDelta parentDelta, int nodeOffset, RequestMonitor rm) {
    	getSession().getExecutor().execute(new DsfRunnable() {
			@Override
			public void run() {
				IDMContext found = null;
				// get new thread focus from sync service
				IGDBSynchronizer fGdbSync = getServicesTracker().getService(IGDBSynchronizer.class);
				Object[] elems = fGdbSync.getFocus();
				for (Object elem : elems) {
					if (elem instanceof IExecutionDMContext) {
						found = (IDMContext)elem;
						break;
					}
				}
				
				final IDMContext newThread = found;
				if (newThread != null) {
			    	...
			    	getVMCIndexForDmc(
			    			...
			    			new DataRequestMonitor<Integer>(getExecutor(), rm) {
								...
								protected void handleSuccess() {
									...
									// policy.
									parentDelta.addNode(createVMContext(newThread), nodeOffset + threadOffset,
											IModelDelta.SELECT | IModelDelta.FORCE);
									rm.done();
								}
			    			});
				} else {
					rm.done(new Status(IStatus.ERROR, GdbUIPlugin.PLUGIN_ID, "ThreadVMNode, Unable to resolve thread context for focus changed event  ", null)); //$NON-NLS-1$
				}
			}
    	});
    }

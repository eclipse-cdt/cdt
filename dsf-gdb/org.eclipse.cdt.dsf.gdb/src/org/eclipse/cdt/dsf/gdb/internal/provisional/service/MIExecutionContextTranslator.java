...
public class MIExecutionContextTranslator extends AbstractDsfService 
	...
	private class ThreadNodeDesc implements ExecNodeDesc{
		...
		ThreadNodeDesc( IThreadDMContext threadDmc) {
	...
	private class UserGroupData {
		...
		IExecutionDMContext[] filterThreads( IExecutionDMContext[] allThreads) {
			...
			for( IExecutionDMContext currentThread : allThreads) {
				IThreadDMContext threadCntx = DMContexts.getAncestorOfType(currentThread, IThreadDMContext.class);
				if( threadCntx != null) 
	...
	private class UserGroups {
		...
		ArrayList<UserGroupData> data = new ArrayList<UserGroupData>();

		UserGroups() {
		}

		UserGroupDMC group( IExecutionDMContext[] contexts) {
	    	...
	    	if( commonParent != null) {
				...
				for( IExecutionDMContext cntx : contexts) {
					...
					if( cntx instanceof UserGroupDMC){
						...
						IThreadDMContext threadCntx = DMContexts.getAncestorOfType(cntx, IThreadDMContext.class);
						if( threadCntx != null)
		...
		String newGroupName() {
			newGroupId++;
			return "Group "+new Integer(newGroupId).toString(); //$NON-NLS-1$
		}
		
		UserGroupData findNode( ExecNodeDesc node) {
			for( UserGroupData current : data) 
		...
		UserGroupData findThreadEntry( IThreadDMContext threadDmc) {
			for( UserGroupData current : data) 
		...
		IContainerDMContext createContainerPath( IContainerDMContext container, IThreadDMContext threadDmc) {
		...
		public IExecutionDMContext[] getExecutionContexts( IContainerDMContext containerDmc, IExecutionDMContext[] threads) {
			...
			// we have user groups, but there are still ungrouped threads. 
		...
		private IExecutionDMContext[] getUngroupedThreads( IExecutionDMContext[] allThreads) {
			...
			for( IExecutionDMContext thread : allThreads) {
				IThreadDMContext threadCntx = DMContexts.getAncestorOfType(thread, IThreadDMContext.class);
				if( threadCntx != null) {
					boolean threadInsideGroup = false;
					for( UserGroupData cur : data) {
	...
	public static class UserGroupDMC extends AbstractDMContext implements  //TODO make private
			IContainerDMContext {
    ...
    @Override
	public void group(IExecutionDMContext[] contexts, final RequestMonitor rm) {
    	if (canGroup(contexts)) {
    		UserGroupDMC groupDmc = userGroups.group(contexts);
    		if (groupDmc != null) {
	...
	public void ungroup(IExecutionDMContext[] contexts, RequestMonitor rm) {
    	if (canUngroup(contexts)) {
    		IContainerDMContext[] ungroupedDMCs = userGroups.ungroup(contexts);
	...
	public IContainerDMContext createContainerPath(
		IContainerDMContext container, IThreadDMContext threadDmc) {
		return userGroups.createContainerPath(container, threadDmc);			
	}

	@Override
	public IContainerDMContext[] getChildContainers( IContainerDMContext container) {
		return userGroups.getChildContainers(container);
	}
	...
	public void getExecutionContexts(final IContainerDMContext containerDmc, final DataRequestMonitor<IExecutionDMContext[]> rm) {
		...
		// now delegate to the process service. 
		...
		procService.getProcessesBeingDebugged(
			...
			new DataRequestMonitor<IDMContext[]>(getExecutor(), rm) {
				@Override
				protected void handleSuccess() {
					// return only the threads that belong to containerDmc  
					if (getData() instanceof IExecutionDMContext[]) {
						IExecutionDMContext[] allThreads = (IExecutionDMContext[])getData();
						IExecutionDMContext[] filteredThreds = userGroups.getExecutionContexts(containerDmc, allThreads);
						rm.setData(filteredThreds);
					} else {
	...
	public IDMContext getStableContext( IExecutionDMContext executionDMContext){
    	IThreadDMContext thredDMContext = DMContexts.getAncestorOfType(executionDMContext, IThreadDMContext.class);

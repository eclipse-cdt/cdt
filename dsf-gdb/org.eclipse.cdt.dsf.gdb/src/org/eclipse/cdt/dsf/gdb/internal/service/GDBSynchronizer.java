...
public class GDBSynchronizer extends AbstractDsfService implements IGDBSynchronizer, IEventListener
{
	...
	@Override
	public void setFocus(final IDMContext[] focus) {
		assert focus != null;
		getExecutor().execute(new DsfRunnable() {
			...
			public void run() {
				...
				// commands with "--thread" and "--stackframe" parameters having the side-
				...
				IExecutionDMContext threadCtx = null;
				...
				// new selection is a thread?
					...
					threadCtx = (IExecutionDMContext)elem; 
				...
				final IExecutionDMContext finalThreadCtx = threadCtx;
				// proceed to set thread and frame focus
				setThreadFocus(threadCtx, new ImmediateRequestMonitor() {
					@Override
					protected void handleCompleted() {
						setFrameFocus(finalFrameCtx, new ImmediateRequestMonitor() {
							@Override
	...
	private String getThreadIdFromContext(IExecutionDMContext threadCtx) {
		...
		for (MIOOBRecord oobr : ((MIOutput)output).getMIOOBRecords()) {
    		if (oobr instanceof MINotifyAsyncOutput) {
    			...
    			if ("thread-selected".equals(miEvent)) { //$NON-NLS-1$
    				...
    				createAndDispatchGDBFocusChangedEvent(false);
    			}
    		}
    	}
	}

	private void createAndDispatchGDBFocusChangedEvent(boolean force) {
		assert fCurrentThreadCtx != null;
		if (fSyncEnabled || force) {
	 ...
	 */
	private IExecutionDMContext createExecContextFromThreadId(String tid) {
		assert tid != null;
	...
	public void sessionSelected() {
		...
		// even if synchronization is off
		createAndDispatchGDBFocusChangedEvent(true);
	}

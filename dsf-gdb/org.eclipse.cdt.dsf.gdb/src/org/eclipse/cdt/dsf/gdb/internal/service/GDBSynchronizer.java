...
public class GDBSynchronizer extends AbstractDsfService implements IGDBSynchronizer, IEventListener
{
	...
	public void setFocus(final IDMContext[] focus) {
		...
		getExecutor().execute(new Runnable() {
			...
			public void run() {
				...
				// new selection is a thread?, lets select this thread and its default frame
	...
	private void setFrameFocus(final IFrameDMContext newFrame, ImmediateRequestMonitor rm) {
		if (newFrame == null) {
			GdbPlugin.logErrorMessage("GdbSynchronizer unable to resolve frame context for the selected element"); //$NON-NLS-1$
			return;
		}
		// The command for the selection of a frame does not support the --thread option, 
		// therefore the command control will issue a -thread-select before sending the actual frame select, 
		// keeping the thread and frame in sync
		IExecutionDMContext newThread = getThreadFromFrame(newFrame);
		if (isThreadSuspended(newThread)) {

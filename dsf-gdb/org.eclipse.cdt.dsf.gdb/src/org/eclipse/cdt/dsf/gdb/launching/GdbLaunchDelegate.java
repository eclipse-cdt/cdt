...
/**
 * The shared launch configuration delegate for the DSF/GDB debugger. This
 * delegate supports all configuration types (local, remote, attach, etc)
 */
@ThreadSafe
public class GdbLaunchDelegate extends AbstractCLaunchDelegate2 {
	...
	protected String getCLILabel(ILaunchConfiguration config, String gdbVersion) throws CoreException {
		return fGdbLaunch.getGDBPath().toString().trim() + " (" + gdbVersion + ")"; //$NON-NLS-1$ //$NON-NLS-2$
	}
	...
	protected IPath checkBinaryDetails(final ILaunchConfiguration config) throws CoreException {
		...
		// LaunchUtils.verifyBinary here. Instead we simply rely on the debugger
		// to
	...
	protected String getGDBVersion(ILaunchConfiguration config) throws CoreException {
		return fGdbLaunch.getGDBVersion();
	}
	...
	public boolean preLaunchCheck(ILaunchConfiguration config, String mode, IProgressMonitor monitor)
		...
		// post-mortem sessions from starting for GDBs <= 6.8 and there is no
		// way to turn it off
		// Bug 348091
	...
	/**
	 * Creates an object of GdbLaunch. Subclasses who wish to just replace the
	 * GdbLaunch object with a sub-classed GdbLaunch should override this
	 * method. Subclasses who wish to replace the GdbLaunch object as well as
	 * change the initialization sequence of the launch, should override
	 * getLaunch() as well as this method. Subclasses who wish to create a
	 * launch class which does not subclass GdbLaunch, are advised to override
	 * getLaunch() directly.
	 * 
	 * @param configuration
	 *            The launch configuration
	 * @param mode
	 *            The launch mode - "run", "debug", "profile"
	 * @param locator
	 *            The source locator. Can be null.
	...
	/**
	 * Returns a sequence that will create and initialize the different DSF
	 * services. Subclasses that wish to add/remove services can override this
	 * method.
	 * 
	 * @param session
	 *            The current DSF session
	 * @param launch
	 *            The current launch
	 * @param rm
	 *            The progress monitor that is to be used to cancel the sequence
	 *            if so desired.
	 * @since 4.5
	 */
	...
	/**
	 * Creates and initializes the source locator for the given launch
	 * configuration and dsf session.
	 * 
	 * @since 4.1
	 */
	 ...
	 * Subclasses who wish to just replace the source locator object with a
	 * sub-classed source locator should override this method. Subclasses who
	 * wish to replace the source locator object as well as change the
	 * initialization sequence of the source locator, should override
	 * getSourceLocator() as well as this method. Subclasses who wish to create
	 * a source locator which does not subclass DsfSourceLookupDirector, are
	 * advised to override getSourceLocator() directly.
	 * 
	 * @since 4.1
	 */
	...
	/**
	 * Returns true if the specified version of GDB supports non-stop mode.
	 * 
	 * @since 4.0
	 */
	...
	/**
	 * Returns true if the specified version of GDB supports post-mortem
	 * tracing.
	 * 
	 * @since 4.0
	 */
	protected boolean isPostMortemTracingSupportedInGdbVersion(String version) {
		...
		if (TRACING_FIRST_VERSION.compareTo(version) <= 0
				// This feature will be available for GDB 7.2. But until that
				// GDB is itself available
	...
	protected IDsfDebugServicesFactory newServiceFactory(String version) {
		...
		if (version.contains(LaunchUtils.MACOS_GDB_MARKER)) {
			...
			// 6.3.50-20050815APPLE1346, we extract the gdb version and apple
			// version
	...
	/**
	 * Method called to create the services factory for this debug session. A
	 * subclass can override this method and provide its own ServiceFactory.
	 * 
	 * @since 4.1
	 */
	protected IDsfDebugServicesFactory newServiceFactory(ILaunchConfiguration config, String version) {
		// Call the deprecated one for now to avoid code duplication.
		// Once we get rid of the deprecated one, we can also get rid of
		// fIsNonStopSession
		fIsNonStopSession = LaunchUtils.getIsNonStopMode(config);

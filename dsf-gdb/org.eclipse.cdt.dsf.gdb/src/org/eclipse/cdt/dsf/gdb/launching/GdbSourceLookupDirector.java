		...
		for (ISourceContainer container : containers) {
			...
			if (container instanceof IMappingSourceContainer) {
				if (container instanceof MappingSourceContainer
					 ...
					 * "set substitute-path"). Therefore it should be ignored
					 * for calculating compilation path from local source name.
					 * 
					 * As the containers themselves do not know whether the
					 * director requesting the compilation path supports backend
					 * mapping we need to prevent the call to getCompilationPath
					 * here.
					 */
				} else {

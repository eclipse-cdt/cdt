...
public class DebugNewProcessSequence_7_2 extends DebugNewProcessSequence {
	...
	protected String[] getExecutionOrder(String group) {
		if (GROUP_TOP_LEVEL.equals(group)) {
			...
			// At this point we have already been connected. See FinalLaunchSequence_7_2.
			orderList.remove(orderList.indexOf("stepRemoteConnection")); //$NON-NLS-1$
	...
	public void stepSetRemoteExecutable(final RequestMonitor rm) {
		if (fBackend.getSessionType() == SessionType.REMOTE && !fBackend.getIsAttachSession()) {
			if (!CDebugUtils.getAttribute(
				...
				// The executable is specified as gdbserver's parameter
				rm.done();
				return;
			}

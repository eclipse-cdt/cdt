...
/**
 * Breakpoints service for GDB 7.5.
 * This version supports dynamic printf
 * 
 * @since 4.2
 */
public class GDBBreakpoints_7_5 extends GDBBreakpoints_7_4 {
	
	private IMICommandControl fConnection;
	private IMIRunControl fRunControl;
	private CommandFactory fCommandFactory;

	public GDBBreakpoints_7_5(DsfSession session) {
		super(session);
	}
						...
						protected void handleSuccess() {
							...
							if (miBpt != null) {
								...
								IBreakpointDMContext dmc = 
									new MIBreakpointDMContext(GDBBreakpoints_7_5.this, new IDMContext[] { context }, newBreakpoint.getNumber());
								drm.setData(dmc);
	 ...
	 */
	protected void doAddDynamicPrintf(final IBreakpointsTargetDMContext context, final Map<String, Object> attributes, final DataRequestMonitor<IBreakpointDMContext> finalRm)
	{
		...
		final String printfStr = (String)getProperty(attributes, MIBreakpoints.PRINTF_STRING, "Default printf string\\n"); //$NON-NLS-1$
	    ...
	    final Step insertDPrintf = new Step() {
    		...
    		public void execute(final RequestMonitor rm) {
    			...
    			fConnection.queueCommand(
    					fCommandFactory.createCLIDPrintf(context, location, printfStr),
    					new DataRequestMonitor<CLIDPrintfInfo>(getExecutor(), rm) {
    						@Override
    						protected void handleSuccess() {
    							final Integer reference = getData().getReference();
    							if (reference == null) {
    								rm.done(new Status(IStatus.ERROR, GdbPlugin.PLUGIN_ID, REQUEST_FAILED, DYNAMIC_PRINTF_INSERTION_FAILURE, null));
    								return;
    							}

    							// The simplest way to convert from the CLIDPrintfInfo to a MIBreakInsertInfo
    							// is to list the breakpoints and take the proper output
    							fConnection.queueCommand(
    									fCommandFactory.createMIBreakList(context),
    									new DataRequestMonitor<MIBreakListInfo>(getExecutor(), rm) {
    										@Override
    										protected void handleSuccess() {
    											MIBreakpoint[] breakpoints = getData().getMIBreakpoints();
    											for (MIBreakpoint bp : breakpoints) {
    												if (bp.getNumber() == reference) {
    													...
    													MIBreakpointDMContext dmc = new MIBreakpointDMContext(GDBBreakpoints_7_5.this, new IDMContext[] { context }, reference);
    													finalRm.setData(dmc);

    													// Flag the event
    													getSession().dispatchEvent(new BreakpointAddedEvent(dmc), getProperties());

    													// By default the dprintf is enabled at creation

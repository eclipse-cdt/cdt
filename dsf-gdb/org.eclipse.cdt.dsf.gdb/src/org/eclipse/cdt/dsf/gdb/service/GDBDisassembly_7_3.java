...
public class GDBDisassembly_7_3 extends MIDisassembly implements IDisassembly3 {
            ...
            final DataRequestMonitor<IInstruction[]> drm)
    {
        ...
        fConnection.queueCommand(fCommandFactory.createMIDataDisassemble(context, start, end, 2),
    ...
    public void getInstructions(IDisassemblyDMContext context, String filename,
            int linenum, int lines, boolean opcodes, final DataRequestMonitor<IInstruction[]> drm)
    {
        // Validate the context
        if (context == null) {
            drm.setStatus(new Status(IStatus.ERROR, GdbPlugin.PLUGIN_ID, INTERNAL_ERROR, "Unknown context type", null)); //$NON-NLS-1$);
        ...
        // Go for it
        fConnection.queueCommand(fCommandFactory.createMIDataDisassemble(context, filename, linenum, lines, 2),
            new DataRequestMonitor<MIDataDisassembleInfo>(getExecutor(), drm) {
                @Override
                protected void handleSuccess() {
                    IInstruction[] result = getData().getMIAssemblyCode();
            ...
            final DataRequestMonitor<IMixedInstruction[]> drm)
    {
        ...
        String start = (startAddress != null) ? startAddress.toString() : "$pc";       //$NON-NLS-1$
        String end   = (endAddress   != null) ? endAddress.toString()   : "$pc + 100"; //$NON-NLS-1$
        fConnection.queueCommand(fCommandFactory.createMIDataDisassemble(context, start, end, 3),
            new DataRequestMonitor<MIDataDisassembleInfo>(getExecutor(), drm) {
                @Override
                protected void handleSuccess() {
            ...
            final DataRequestMonitor<IMixedInstruction[]> drm)
    {
        ...
        // Go for it
        fConnection.queueCommand(fCommandFactory.createMIDataDisassemble(context, filename, linenum, lines, 3),
            new DataRequestMonitor<MIDataDisassembleInfo>(getExecutor(), drm) {
                @Override
                protected void handleSuccess() {
                    IMixedInstruction[] result = getData().getMIMixedCode();

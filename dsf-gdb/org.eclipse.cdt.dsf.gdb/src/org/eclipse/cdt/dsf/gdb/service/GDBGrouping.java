...
public class GDBGrouping extends AbstractDsfService implements IGDBGrouping, ICachingService {
	...
	@Override
	public void getGroupsRecurse(IExecutionDMContext wantedCtx, DataRequestMonitor<IGroupDMContext[]> rm) {
		doGetGroups(wantedCtx, true, rm);
	}
	
	@Override
	public void getGroups(IExecutionDMContext wantedCtx, DataRequestMonitor<IGroupDMContext[]> rm) {
		doGetGroups(wantedCtx, false, rm);
	}
	
	private void doGetGroups(final IExecutionDMContext wantedCtx, boolean recurse, DataRequestMonitor<IGroupDMContext[]> rm) {
			...
			for (final UserGroupData grp : fGroupList) {
				getExecutionContexts(grp.getNode().getGroupContext(), new DataRequestMonitor<IExecutionDMContext[]>(getExecutor(), crm) {
					...
					protected void handleSuccess() {
						...
						// group directly contains wanted context?
						...
						if (recurse) {
							// look for a group that contains the parent of the wanted context
							// note: it's assumed ATM that we do not have recursive groups
							for (int i = 0; i < currentGroupContent.length; i++) {

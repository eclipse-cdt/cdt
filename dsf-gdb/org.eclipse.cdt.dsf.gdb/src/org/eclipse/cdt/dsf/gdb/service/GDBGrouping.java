...
public class GDBGrouping extends AbstractDsfService implements IGDBGrouping, ICachingService {
	...
	@Override
	public void getGroupsForExecutionContext(IExecutionDMContext execCtx, DataRequestMonitor<IGroupDMContext[]> rm) {
//		if (execCtx instanceof IMIExecutionDMContext && fGroupList != null) {
		if (fGroupList != null) {
			ArrayList<IGroupDMContext> groupList = new ArrayList<>();
					
			// go through all groups
			for (UserGroupData g : fGroupList) {
				// group-all is build in a special way
				if (g.equals(fGroupAllData)) {
					groupList.add(g.getNode().getGroupContext());
					continue;
				}
				
				ExecNodeDesc[] children = g.getChildren();
				for (ExecNodeDesc child : children) {
					...
					if (child instanceof ThreadNodeDesc) {
						...
						if (thread.getThreadContext().equals(execCtx)) {
							groupList.add(g.getNode().getGroupContext());
						}
					}
					// process
						...
						// process context we are looking for?
						if (proc.getProcessContext().equals(execCtx)) {
							groupList.add(g.getNode().getGroupContext());
						}
					}
				}
			}
			IGroupDMContext[] groupArray = new IGroupDMContext[groupList.size()];
			int i = 0;
			for (IGroupDMContext c : groupList) {
				groupArray[i] = c;
				i++;
			}
			
//			groupArray = (IGroupDMContext[]) groupList.toArray();

...
public class GDBGrouping extends AbstractDsfService implements IGDBGrouping, ICachingService {
	...
	protected class GroupAddedEvent extends AbstractDMEvent<IGroupDMContext> 
	implements IGroupAddedEvent {
		public GroupAddedEvent(IGroupDMContext context) {
			super(context);
		}
	}
	...
	protected class GroupModifiedEvent extends AbstractDMEvent<IGroupDMContext> 
	implements IGroupModifiedEvent {
		public GroupModifiedEvent(IGroupDMContext context) {
			super(context);
		}
	}
	...
	public void group(IExecutionDMContext[] contexts, DataRequestMonitor<IContainerDMContext> rm) {
		...
		if (fGroupList == null) {
			...
			GroupAddedEvent event = new GroupAddedEvent(groupAllDmc); 
		...
		GroupAddedEvent event = new GroupAddedEvent(newGroupDmc); 
		getSession().dispatchEvent(event, getProperties());
	...
	// TODO: ATM the lookup for groups is non-recursive for regular groups, but is indirectly recursive for
	// group-all. This is inconsistent but appears to work well so far...
	@Override
	public void getGroups(IExecutionDMContext wantedCtx, DataRequestMonitor<IGroupDMContext[]> rm) {
		if (fGroupList != null) {
			...
			for (UserGroupData grp : fGroupList) {
				...
				for (ExecNodeDesc child : grp.getChildren()) {
					// thread?
							...
							for (ExecNodeDesc child : group.getChildren()) {
								if (child instanceof ThreadNodeDesc) {
									if (exitedThreadOrProcess.equals(((ThreadNodeDesc)child).getThreadContext())) {
										...
										GroupModifiedEvent changeEvent = new GroupModifiedEvent(group.getNode().getGroupContext()); 

...
//			deletedAGroup = true;
			...
			// send notification
			ContainerLayoutDMContext layoutDmc = new ContainerLayoutDMContext( 
					getSession().getId(), new IDMContext[] { contexts[0] });
			GroupDeletedEvent event = new GroupDeletedEvent(groupDMC); 
			getSession().dispatchEvent(event, getProperties());
			
		}

//		// TODO: handle notification for more than one group being deleted - ATM just handle first group 
//		if (deletedAGroup) {
//			ContainerLayoutDMContext layoutDmc = new ContainerLayoutDMContext( 
//					getSession().getId(), new IDMContext[] { contexts[0] });
...
//							// remove this execution context from group
//							group.removeChild(child);
//							System.out.println("==================== removing thread from group: " + group.getNode().getGroupContext()); //$NON-NLS-1$
//						}
//					}
//				}
//				if (groupChanged) {

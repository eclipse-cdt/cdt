...
import org.eclipse.cdt.dsf.mi.service.command.commands.MIAddUserDefinedThreadGroup;
...
import org.eclipse.cdt.dsf.mi.service.command.commands.MIRemoveUserDefinedThreadGroup;
...
public class GDBGrouping_7_11 extends GDBGrouping implements IEventListener {
	...
	public void group(final IExecutionDMContext[] contexts, final DataRequestMonitor<IContainerDMContext> rm) {
		...
		final String spec = createITSetContent(contexts);
		fCommandControl.queueCommand(
				new MIAddUserDefinedThreadGroup(fCommandControl.getContext(), name, spec),
				new DataRequestMonitor<MIAddUserDefinedThreadGroupInfo>(getSession().getExecutor(), rm) {
					@Override
					protected void handleSuccess() {
						...
						flushCache();
						String groupId = getData().getGroupId();
						MIUserGroupDMC newGroupDmc = new MIUserGroupDMC(getSession(), new IDMContext[] { fCommandControl.getContext() }, groupId, name);
						
						// send notification that a new group has been created
						AbstractDMEvent<IGroupDMContext> event = new GroupCreatedEvent(newGroupDmc); 
...
//				content.append("tt"); //$NON-NLS-1$  // remove previous line and uncomment this line when the 
	...
	protected void deleteGroup(IGroupDMContext groupDmc, final RequestMonitor rm) {
		...
		fITSetCache.execute(
				new MIRemoveUserDefinedThreadGroup(fCommandControl.getContext(), groupId),
				new DataRequestMonitor<MIInfo>(getExecutor(), rm) {
					@Override
					protected void handleSuccess() {
						// groups have changed - flush the cache
						flushCache();
						MIUserGroupDMC deletedGroupDmc = new MIUserGroupDMC(getSession(), new IDMContext[] { fCommandControl.getContext() }, groupId, null);
						
						// send notification that a new group has been created
						AbstractDMEvent<IGroupDMContext> event = new GroupDeletedEvent(deletedGroupDmc);
						getSession().dispatchEvent(event, getProperties());
...
//					new MIListThreadGroups(fCommandControl.getContext(),MIListThreadGroups.GroupType.GROUP_TYPE_BUILTIN),
	...
	// look for notifications that a user group was added or removed, and react appropriately
	@Override
    		...
    		if (oobr instanceof MINotifyAsyncOutput) {
    			...
    			if ("thread-group-added".equals(miEvent) || "thread-group-removed".equals(miEvent)) { //$NON-NLS-1$ //$NON-NLS-2$	
    				...
    				if ("thread-group-added".equals(miEvent)) { //$NON-NLS-1$
    					assert spec != null && name != null;
    					event = new GroupCreatedEvent(groupDmc); 
    				}
    				else if ("thread-group-removed".equals(miEvent)) { //$NON-NLS-1$

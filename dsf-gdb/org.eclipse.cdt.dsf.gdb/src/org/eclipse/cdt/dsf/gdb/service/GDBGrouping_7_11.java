			...
			fITSetCache.execute(
				...
				new DataRequestMonitor<MIITSetViewInfo>(getExecutor(), rm) {
					...
					protected void handleSuccess() {
						...
						// parse result
						// note: we ignore the inferiors on purpose. what we want is a flat group
						// of all threads that evaluate as being part of the group - we do not care
						// from which inferior they came from - GDB will handle the details.
			...
			fITSetCache.execute(
					...
					new DataRequestMonitor<MIInfoItsetsInfo>(getExecutor(), rm) {
						...
						protected void handleSuccess() {
							...
							rm.done(new MIUserGroupDMData(sets[0].getId(), sets[0].getName(), sets[0].getSpec()));
						};
					});
		} else {
    		...
    		if (oobr instanceof MINotifyAsyncOutput) {
    			...
    			if ("named-itset-created".equals(miEvent) || "named-itset-deleted".equals(miEvent)) { //$NON-NLS-1$ //$NON-NLS-2$
    				...
    				for (int i = 0; i < results.length; i++) {
    		    		...
    		    		} else if (var.equals("num")) { //$NON-NLS-1$
    		    			if (val instanceof MIConst) {

 ...
 * <p>The managed registered groups are user defined subsets of the complete lists of Registers reported by GDB to a specific Target</p>
 * <p>It also trigger the read/write (persistence) of the user defined Register Groups during the start/shutdown process of a session respectively</p>
 * @since 4.5
 */
public class GDBRegisters extends MIRegisters implements IRegisters2 {
	...
	/* (non-Javadoc)
	 * @see org.eclipse.cdt.dsf.debug.service.IRegisters2#getRegisterGroupsData(org.eclipse.cdt.dsf.datamodel.IDMContext, org.eclipse.cdt.dsf.concurrent.DataRequestMonitor)
	 */
	@Override
	public void getRegisterGroupsData(IDMContext ctx, final DataRequestMonitor<IRegisterGroupDMData[]> rm) {
		getRegisterGroups(ctx, new DataRequestMonitor<IRegisterGroupDMContext[]>(getExecutor(), rm) {
			@Override
			protected void handleSuccess() {
				List<IRegisterGroupDMData> groupDataList = new ArrayList<IRegisterGroupDMData>();
				IRegisterGroupDMContext[] groups = getData();
				if (groups != null) {
					for (IRegisterGroupDMContext group: groups) {
						if (group instanceof MIRegisterGroupDMC) {
							groupDataList.add(createRegisterGroupData((MIRegisterGroupDMC) group));
						}
					}
				}
				
				rm.setData(groupDataList.toArray(new IRegisterGroupDMData[groupDataList.size()]));
				rm.done();
			}
		});
	}
	...
	@Override
	public void getRegistersData(IRegisterDMContext[] regDmcArray, final DataRequestMonitor<IRegisterDMData[]> rm) {
		...
		// Using a counting request monitor to loop over individual async requests
		final List<IRegisterDMData> registerDataList = new ArrayList<IRegisterDMData>();
		CountingRequestMonitor crm = new CountingRequestMonitor(getExecutor(), rm) {
			@Override
			protected void handleSuccess() {
				// Call back at the end of the counting loop
				rm.setData(registerDataList.toArray(new IRegisterDMData[registerDataList.size()]));
				rm.done();
			}
		};
		...
		for (int i = 0; i < regDmcArray.length; i++) {
			...
			gerRegisterData(regDmc, registerDataList, crm);
		}
		
		// The counting Request can not be completed before setting the count
		crm.setDoneCount(regDmcArray.length);

	}

	private void gerRegisterData(IRegisterDMContext register, final List<IRegisterDMData> registerDataList, CountingRequestMonitor crm) {
		//Handling individual calls for Register Data
		getRegisterData(register, new DataRequestMonitor<IRegisterDMData>(getExecutor(), crm) {
			@Override
			protected void handleSuccess() {
				registerDataList.add(getData());
	...
	@Override
	public void addRegisterGroup(final IDMContext containerContext, final String name, final IRegisterDMContext[] registers, RequestMonitor rm) {
		if (registers == null || registers.length < 1) {
		...
		generateGroupsChangedEvent(contDmc);
		rm.done();
	}
			...
			for (IRegisterGroupDMContext group : groups) {
				...
				if (groupsCtx != null) {			
					...
					if (fGroupToRegistersMap.remove(group) != null) {
						// Create event notification
						generateGroupsChangedEvent(containerDmc);
					}
				} else {
					rm.setStatus(new Status(IStatus.ERROR, GdbPlugin.PLUGIN_ID, INVALID_HANDLE,
	...
	private void generateGroupsChangedEvent(IContainerDMContext contDmc) {
		// Create event notification
		DsfSession session = DsfSession.getSession(contDmc.getSessionId());
		session.dispatchEvent(new GroupsChangedDMEvent(contDmc), null);
	}
	...
	public void findTargetRegisterGroup(IDMContext ctx, DataRequestMonitor<IRegisterGroupDMContext> rm) {
		findRegisterGroup(ctx, ROOT_GROUP_NAME, rm);
	}

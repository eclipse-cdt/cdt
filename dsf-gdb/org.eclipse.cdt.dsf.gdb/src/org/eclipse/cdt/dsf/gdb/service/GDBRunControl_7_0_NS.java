...
public class GDBRunControl_7_0_NS extends AbstractRunControl implements IMIRunControl, IMultiRunControl, ICachingService {
	...
	protected static class StepIntoSelectionActiveOperation {
		...
		private String fBaseFunctionName = null;
		private final boolean fSkipBreakpoints;
		...
		public StepIntoSelectionActiveOperation(IMIExecutionDMContext threadContext, int line, boolean skipBreakpoints, IFunctionDeclaration targetFunction,
		...
		private void init() {
			...
			fBaseFunctionName = fRunToLineFrame.getFunction();
		}
	...
	private void step(IExecutionDMContext context, StepType stepType, boolean resume, final RequestMonitor rm) {
	...
	// Step into Selection
	// ------------------------------------------------------------------------
	private void stepIntoLocation(final IExecutionDMContext context, final int baseLine, final String baseLineLocation, final boolean skipBreakpoints, final IFunctionDeclaration targetFunction,
		...
		getStackDepth(context, new DataRequestMonitor<Integer>(getExecutor(), null) {
			@Override
			public void handleCompleted() {
				if (getData() != null) {
					...
					if (currentFrame.getLine() == baseLine) {
						// Save the step into selection information
						fStepInToSelectionActiveOperation = new StepIntoSelectionActiveOperation(dmc, baseLine, skipBreakpoints, targetFunction, framesSize,
								currentFrame);
						// Ready to step into a function selected
						// within a current line
						step(context, StepType.INSTRUCTION_STEP_INTO, rms);
					} else {
						// Save the step into selection information
						fStepInToSelectionActiveOperation = new StepIntoSelectionActiveOperation(dmc, baseLine, skipBreakpoints, targetFunction, framesSize, null);
						// Pointing to a line different than the current line
						// Needs to RunToLine before stepping to the selection
						runToLocation(context, baseLineLocation, skipBreakpoints, rms);
					}
				}
			}
		});
	...
	public void eventDispatched(final MIStoppedEvent e) {
		if (fRunToLineActiveOperation != null) {
			if (runToLineContinue(e) || fStepInToSelectionActiveOperation == null) {
	...
	private void evaluateStepIntoSelection(final MIStoppedEvent e) {
		...
		if (fStepInToSelectionActiveOperation.getThreadContext().equals(threadDmc)) {
			...
			// make sure base line search has been initialized
			if (prepStepIntoSelection(e) == false) {
				//Aborted
				return;
			}
			...
			getStackDepth(threadDmc, new DataRequestMonitor<Integer>(getExecutor(), null) {
				...
				protected void handleCompleted() {
					...
					if (framesSize > originalStackDepth) {
						// Located deeper in the stack, Shall continue step / search
						// Step return
						continueStepping(frame, e, StepType.STEP_RETURN);
					} else if (framesSize == originalStackDepth) {
						...
						if (currentLocation.equals(searchLineLocation)) {
							continueStepping(frame, e, StepType.STEP_INTO);
						} else {
	...
	private boolean prepStepIntoSelection(MIStoppedEvent e) {
		MIFrame frame = e.getFrame();
		boolean status = true;
		// if RunToLine preparation for StepIntoSelection was needed,
		// save the base frame / line needed to establish the scope of the
		// search
		if (fStepInToSelectionActiveOperation.getRunToLineFrame() == null) {
			// Shall now be at the runto line location
			if (fStepInToSelectionActiveOperation.getLine() == frame.getLine()) {
				fStepInToSelectionActiveOperation.setRunToLineFrame(frame);
				status = true;
			} else {
				// Did not stopped at the runto line location. probably a break point before reaching the actual target
				// Abort the action
				stopStepIntoSelection(e);
				status = false;
			}
	...
	private void continueStepping(final MIFrame atFrame, final MIStoppedEvent event, StepType steptype) {
	...
	private boolean runToLineContinue(final MIStoppedEvent e) {
		// First check if it is the right thread that stopped
		IMIExecutionDMContext threadDmc = DMContexts.getAncestorOfType(e.getDMContext(), IMIExecutionDMContext.class);
	...
	private boolean sameSignature(MIFrame currentFrame) {
		...
		boolean sameName = currentFunctionName.equals(targetFunctionName);
		...
		if (currentFunctTokens.length > 1 && targetFunctTokens.length > 1) {
			String simpleCurrentFunction = currentFunctTokens[currentFunctTokens.length - 1];
			String simpleTargetFunction = targetFunctTokens[targetFunctTokens.length - 1];
			if (simpleCurrentFunction.equals(simpleTargetFunction)) {
	...
	@Override
	public void stepIntoSelection(final IExecutionDMContext context, String sourceFile, final int lineNumber, final boolean skipBreakpoints, final RequestMonitor rm,
			final IFunctionDeclaration selectedFunction) {
		determineDebuggerPath(context, sourceFile, new ImmediateDataRequestMonitor<String>(rm) {
			@Override
			protected void handleSuccess() {
				stepIntoLocation(context, lineNumber, getData() + ":" + Integer.toString(lineNumber), skipBreakpoints, selectedFunction, rm); //$NON-NLS-1$
			}
		});
	}
	 ...
	 * @param rm
	 */
	private void getStackDepth(final IDMContext dmc, final DataRequestMonitor<Integer> rm) {
		final IMIExecutionDMContext execDmc = DMContexts.getAncestorOfType(dmc, IMIExecutionDMContext.class);
		if (execDmc != null) {
			ICommand<MIStackInfoDepthInfo> depthCommand = fCommandFactory.createMIStackInfoDepth(execDmc);
			;

			fConnection.queueCommand(depthCommand, new DataRequestMonitor<MIStackInfoDepthInfo>(getExecutor(), rm) {
				@Override
				protected void handleSuccess() {
				...
				protected void handleError() {
					...
					// For 7.2, we can say we have one stack
					// But flag the error

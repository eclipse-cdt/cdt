...
public class GDBRunControl_7_10 extends GDBRunControl_7_6 implements IReverseRunControl2 {
	...
	private static class EnableReverseAtLocOperation {
		private IContainerDMContext fContainerContext;
		private ReverseDebugMethod fTraceMethod;
		private String fBpId;
		private String fFileLocation;
		private String fAddrLocation;
		private boolean fTriggerContinue;
		...
		public IContainerDMContext getContainerContext() { return fContainerContext; }
		public ReverseDebugMethod getReverseDebugMethod() { return fTraceMethod; }
		...
		public boolean shouldTriggerContinue() { return fTriggerContinue; }
	    @Override
	    public int hashCode() {return fBpId.hashCode(); }
	    @Override 
		public boolean equals(Object other) {
			if (other instanceof EnableReverseAtLocOperation) {
				if (fContainerContext.equals(((EnableReverseAtLocOperation) other).fContainerContext)
			...
			if (evt instanceof MIBreakpointHitEvent) {
				...
				for (EnableReverseAtLocOperation enableReverse : fBpIdToReverseOpMap.values()) {
					if (breakpointHitMatchesLocation(miEvt, enableReverse)) {
						// We are now stopped at the right place to initiate the recording
						// Remove the operation from our internal map and process it
						fBpIdToReverseOpMap.remove(enableReverse.fBpId);
						IContainerDMContext containerContext = enableReverse.fContainerContext;
						ICommandControlDMContext controlDmc = DMContexts.getAncestorOfType(containerContext, ICommandControlDMContext.class);
						enableReverseMode(controlDmc, enableReverse.fTraceMethod, new RequestMonitor(getExecutor(), null) {
							@Override
							protected void handleSuccess() {
								if (enableReverse.fTriggerContinue) {

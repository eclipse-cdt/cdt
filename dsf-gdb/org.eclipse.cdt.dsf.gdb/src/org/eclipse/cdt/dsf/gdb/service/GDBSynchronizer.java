...
public class GDBSynchronizer extends AbstractDsfService implements IGDBSynchronizer, IEventListener
{
	...
	private class ThreadSwitchedEvent extends AbstractDMEvent<IExecutionDMContext> implements IThreadSwitchedEvent 
	{
	...
	private class StackFrameSwitchedEvent extends AbstractDMEvent<IFrameDMContext> implements IStackFrameSwitchedEvent 
	{
	...
	public String getCurrentStackFrameId() {
		System.out.println("*** current frame id: " + fCurrentStackFrameId);
		...
		for (MIOOBRecord oobr : ((MIOutput)output).getMIOOBRecords()) {
    		if (oobr instanceof MINotifyAsyncOutput) {
    			...
    			if ("thread-selected".equals(miEvent)) { //$NON-NLS-1$
    				...
    				frameLevel = frameLevel != null ? frameLevel : "-1";  //$NON-NLS-1$
    				
    				// thread stayed the same - assume the frame changed
    				...
    				// note: ATM GDB can generate the =thread-selected event even if in some cases 
    				//       the thread has not changed (i.e. same thread is re-selected). We could 
	...
	protected void createAndDispatchThreadSwitchedEvent(String tid) {
		...
		fCommandControl.getSession().dispatchEvent(new ThreadSwitchedEvent(execDmc),
	...
	protected void createAndDispatchStackFrameSwitchedEvent(String tid, String frameId) {
		...
		// create DSF event and dispatch
		fCommandControl.getSession().dispatchEvent(new StackFrameSwitchedEvent(frameDmc),
				fCommandControl.getProperties());
	}

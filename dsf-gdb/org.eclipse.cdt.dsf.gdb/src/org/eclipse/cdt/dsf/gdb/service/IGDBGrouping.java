...
public interface IGDBGrouping extends IExecutionContextTranslator {
    ...
    /**
     * Returns all group contexts that directly contains the specified execution context. 
     * For example, if the wanted context is a thread, and the process containing the thread is in a 
     * group, this method will *not* return that group as a match. 
     * 
     ...
     */
    void getGroups(IExecutionDMContext wantedCtx, DataRequestMonitor<IGroupDMContext[]> rm);
    
    /**
     * Returns all group contexts that directly or indirectly contains the specified execution context.
     * For example, if the wanted context is a thread, and the process containing the thread is in a 
     * group, this method *will* return that group as a match. 
     * 
     *   @param wantedCtx The execution context that is looked-for
     *   @param rm The request monitor
     *   @return array of group contexts, having the given execution context as direct child
     */
    void getGroupsRecurse(IExecutionDMContext wantedCtx, DataRequestMonitor<IGroupDMContext[]> rm);

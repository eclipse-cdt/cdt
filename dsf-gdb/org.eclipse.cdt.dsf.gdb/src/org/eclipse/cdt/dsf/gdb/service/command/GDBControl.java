...
public class GDBControl extends AbstractMIControl implements IGDBControl {
	...
	private class ControlEventProcessor implements IEventProcessor {
		...
		public void eventReceived(Object output) {
			if (output instanceof MIOutput) {
				verifyConnectionLost((MIOutput)output, true);
			} else {
				assert false;
			}
		}
	    ...
	    public void commandDone(ICommandToken token, ICommandResult cmdResult) {
			if (cmdResult instanceof MIInfo) {
				// False as second parameter because we don't want to check OOB records
				// in this case.  This is because the output from the command contains
				// previous OOB records, but we have already processed those records,
				// in eventReceived(), so we don't want to process them again.
				verifyConnectionLost(((MIInfo)cmdResult).getMIOutput(), false);
			} else {
				assert false;
			}
		}
	 ...
	 *               
	 * @param checkOutOfBand Specifies if out-of-band records should be checked or not.
	 * 
	 * @return True if the connection was lost, false otherwise.
	 * @since 4.3
	 */
	protected boolean verifyConnectionLost(MIOutput output, boolean checkOutOfBand) {
		...
		if (!connectionLost) {
			...
			// Also, it can happen that GDB sends multiple responses to the same command.  
			// In those cases, the second command result is sent as an out-of-band event,
			// so must must check for such a result even when dealing with out-of-band records.

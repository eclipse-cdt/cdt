...
public class MIBreakpoints extends AbstractDsfService implements IBreakpoints, IBreakpointsExtension
{
	...
	public void removeBreakpoint(final IBreakpointDMContext dmc, final RequestMonitor finalRm) {
		...
		deleteBreakpoint(context, reference, new RequestMonitor(ImmediateExecutor.getInstance(), finalRm) {
			@Override
	 ...
	 * @since 4.2
	 */
	protected void deleteBreakpoint(final IBreakpointsTargetDMContext context, final int reference, RequestMonitor finalRm) {
	    final Step deleteBreakpointStep = new Step() {
    		@Override
    		public void execute(final RequestMonitor rm) {
    			// Queue the command
    			fConnection.queueCommand(fCommandFactory.createMIBreakDelete(context, new int[] { reference }),
    					new DataRequestMonitor<MIInfo>(getExecutor(), rm) {
    						@Override
    						protected void handleCompleted() {
    							rm.done();
    ...
    /**
     * Returns all breakpoint target contexts for given breakpoint number.
	 * @since 4.2
	 */
    protected IBreakpointsTargetDMContext[] getBreakpointTargetContexts(int reference) {
    	List<IBreakpointsTargetDMContext> list = new ArrayList<IBreakpointsTargetDMContext>();
    	...
    	return list.toArray(new IBreakpointsTargetDMContext[list.size()]);
    }
}

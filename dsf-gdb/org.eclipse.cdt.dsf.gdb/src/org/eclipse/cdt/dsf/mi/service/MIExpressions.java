...
public class MIExpressions extends AbstractDsfService implements IMIExpressions, ICachingService {
	...
	private class ReturnValueAliasing {
		...
		/**
		 * Map of thread to alias list.  This map allows to know which aliases are related
		 * to a thread of execution.  This is important to allow us to delete aliases when a
		 * thread exits.
		 */
		private Map<IMIExecutionDMContext, List<String>> fExecutionToAliasMap = new HashMap<IMIExecutionDMContext, List<String>>();
		/**
		 ...
		 */
		private Map<IMIExecutionDMContext, String> fTopMethodName = new HashMap<IMIExecutionDMContext, String>();
	    ...
	    public void createAlias(IMIExecutionDMContext threadDmc, String expr) {
    		String alias = expr;
    		String methodName = fTopMethodName.get(threadDmc);
    		if (methodName != null) {
    			alias = methodName + "()"; //$NON-NLS-1$
    		}

    		fExpressionAliasesMap.put(expr, alias);

    		List<String> aliasList = fExecutionToAliasMap.get(threadDmc);
    		if (aliasList == null) {
    			aliasList = new ArrayList<String>();
    			fExecutionToAliasMap.put(threadDmc, aliasList);
    		}
    		aliasList.add(expr);
	    }
	    ...
	    public void clearThread(IMIExecutionDMContext threadDmc) {
	    	fTopMethodName.remove(threadDmc);
	    	clearAliases(threadDmc);
	    }

	    /**
	     * Clear all aliases related to a particular thread of execution.
	    ...
	    public void clearAliases(IMIExecutionDMContext threadDmc) {
	    	List<String> aliasList = fExecutionToAliasMap.remove(threadDmc);
	    	if (aliasList != null) {
	    		for (String alias : aliasList) {
	    			fExpressionAliasesMap.remove(alias);        			
	    		}
	    	}
	    }
	    ...
	    public void updateStoppedLocation(IMIExecutionDMContext threadDmc, String methodName) {
	    	fTopMethodName.put(threadDmc, methodName);
	    }
	    
	    /**
	     * @return The alias for 'expression' if there is one.  null if there
	     *         is no alias for that expression.
	     */
	    public String getAlias(String expression) {
	    	return fExpressionAliasesMap.get(expression);
	    }
	}
    ...
    public void eventDispatched(MIStoppedEvent e) {
    	...
    	if (dmc instanceof IMIExecutionDMContext) {
    		IMIExecutionDMContext threadDmc = (IMIExecutionDMContext)dmc;
    		if (e instanceof MIFunctionFinishedEvent) {
    			// When getting an MIFunctionFinishedEvent we must set
    			// a proper alias for the convenience variable
	    		String resultVar = ((MIFunctionFinishedEvent)e).getGDBResultVar();
	    		fReturnValueAliases.createAlias(threadDmc, resultVar);
	    	}
    		...
    		if (frame != null) {
    			fReturnValueAliases.updateStoppedLocation(threadDmc, frame.getFunction());
    		}
		}
    }

...
public class MIModules extends AbstractDsfService implements IModules2, ICachingService {
	...
	private static class SymbolsAllLoadedEvent extends AbstractDMEvent<ISymbolDMContext>
	implements ISymbolsAllLoadedDMEvent {
		
        public SymbolsAllLoadedEvent(ISymbolDMContext context) {
        	super(context);
        }
	}
	...
	public void loadSymbolsForAllModules(final ISymbolDMContext symDmc, final RequestMonitor rm) {
		...
		if (symDmc != null) {
			fModulesLoadCache.execute(fCommandFactory.createCLISharedLibrary(symDmc),
				...
				@Override
				protected void handleSuccess() {
					getSession().dispatchEvent(new SymbolsAllLoadedEvent(symDmc), getProperties());
    ...
    @DsfServiceEventHandler
    public void eventDispatched(ISymbolsAllLoadedDMEvent e) {
    	fModulesCache.reset();
    	// Do not clear fModulesLoadCache since those commands do not need to be resent.
    }

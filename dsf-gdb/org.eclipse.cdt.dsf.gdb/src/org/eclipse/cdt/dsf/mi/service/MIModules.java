...
public class MIModules extends AbstractDsfService implements IModules2, ICachingService {
	...
	private static class ModulesAllChangedEvent extends AbstractDMEvent<ISymbolDMContext>
	implements ModulesChangedDMEvent {

        public ModulesAllChangedEvent(ISymbolDMContext context) {
        	super(context);
        }
	}

	private static class ModuleLoadedEvent extends AbstractDMEvent<ISymbolDMContext>
	implements ModuleLoadedDMEvent {

		private IModuleDMContext fModule;
		
        public ModuleLoadedEvent(ISymbolDMContext context, IModuleDMContext module) {
        	super(context);
        	fModule = module;
        }
	...
	public void loadSymbolsForAllModules(final ISymbolDMContext symDmc, final RequestMonitor rm) {
		...
		if (symDmc != null) {
			fModulesLoadCache.execute(fCommandFactory.createCLISharedLibrary(symDmc),
				...
				@Override
				protected void handleSuccess() {
					getSession().dispatchEvent(new ModulesAllChangedEvent(symDmc), getProperties());
	...
	public void loadSymbols(final IModuleDMContext modDmc, final RequestMonitor rm) {
        ...
        if (symDmc != null && modDmc instanceof ModuleDMContext) {
			fModulesLoadCache.execute(fCommandFactory.createCLISharedLibrary(symDmc, ((ModuleDMContext)modDmc).getFile()), 
				...
				@Override
				protected void handleSuccess() {
					getSession().dispatchEvent(new ModuleLoadedEvent(symDmc, modDmc), getProperties());
    ...
    public void eventDispatched(ModulesChangedDMEvent e) {
    	fModulesCache.reset();

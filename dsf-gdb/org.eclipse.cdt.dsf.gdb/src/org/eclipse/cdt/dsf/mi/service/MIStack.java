	...
	implements IStack, ICachingService
{
	 ...
	 */
	private Map<IMIExecutionDMContext, VariableData> fReturnVariables = new HashMap<IMIExecutionDMContext, VariableData>();
    ...
    private void handleReturnValues(IResumedDMEvent e) {
        ...
        if (e instanceof IContainerResumedDMEvent) {
        	// All-stop mode
        	assert fReturnVariables.size() <= 1;
        	fReturnVariables.clear();
        } else {
        	// Non-stop mode
        	IDMContext ctx = e.getDMContext();
        	if (ctx instanceof IMIExecutionDMContext) {
        		fReturnVariables.remove(ctx);
        ...
        if (e instanceof IMIDMEvent) {
        	...
        	if (miEvent instanceof MIFunctionFinishedEvent) {
        		...
        		if (finishedEventThread != null) {
        			...
        			fReturnVariables.put(finishedEventThread, new VariableData(new MIArg(name, value)));
        		}
        	}
        }
    }

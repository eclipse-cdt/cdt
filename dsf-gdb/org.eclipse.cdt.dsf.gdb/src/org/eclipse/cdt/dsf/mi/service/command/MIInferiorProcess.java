    ...
    implements IEventListener, ICommandListener 
{
	...
	/**
	 * Sets the label for this terminated process.
	 */
    @ConfinedToDsfExecutor("fSession#getExecutor")
	private void setTerminatedLabel(int exitCode) {
			...
			if (proc instanceof InferiorRuntimeProcess) {
				...
				if (fContainerDMContext instanceof IMIContainerDMContext) {
					if (groupAttribute == null || groupAttribute.equals(MIProcesses.UNIQUE_GROUP_ID) ||
						...
						// Use the exact same label as before but add the exit code
						String label = process.getLabel();
						String exitStr = Integer.toString(exitCode);
						label += " " + MessageFormat.format(Messages.MIInferiorProcess_ExitValue, new Object[] { exitStr }); //$NON-NLS-1$
    ...
    private synchronized void setTerminated() {
        ...
        // If the inferior is terminated but we don't have an exit code,
        // it implies the inferior did not terminate normally (user killed it
        // or detached from it).  In that case we should not show an exit code.
        //
        // Note that this reasoning only works properly for GDB >= 7.3.
        // For older GDBs, we also won't have the exit code here, but there is
        // no easy way to know if we are dealing with an old GDB or not, so 
        // we just handle the newer GDB versions.  Older GDBs just won't show
        // the exit code.
        if (fExitCode != null) {
        	setTerminatedLabel(fExitCode);
        }

...
function help_and_exit() {
  ...
  echo ""
  echo "Description:"
  echo "  This script downloads, builds and installs multiple the given versions of gdb."
...
function fixup_gdb() {
  ...
  case "$version" in
    "6.6"|"6.7.1"|"6.8"|"7.0.1"|"7.1"|"7.2"|"7.3.1"|"7.4.1")
      ${dryrun} find "${build}" -type f -exec sed -i -e 's/struct siginfo;/#include <signal.h>/g' {} \;
      ${dryrun} find "${build}" -type f -exec sed -i -e 's/struct siginfo/siginfo_t/g' {} \;
      ;;
  esac
}
...
function configure_gdb() {
  ...
  ${dryrun} ./configure --prefix="${install_dir}/gdb-${version}" CFLAGS="${cflags}"

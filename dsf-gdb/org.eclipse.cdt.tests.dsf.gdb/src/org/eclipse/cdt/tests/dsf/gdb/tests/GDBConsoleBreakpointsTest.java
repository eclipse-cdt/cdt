...
public class GDBConsoleBreakpointsTest extends BaseTestCase {
	...
	public void eventDispatched(IBreakpointsChangedEvent e) {
  		synchronized(this) {
  			...
  			System.out.println(String.format("%s: Got %s", DsfPlugin.getDebugTime(), e.getClass().getName()));
  			notifyAll();
  		}
	}
	...
	private void testConsoleBreakpoint(Class<? extends ICBreakpoint> type, Map<String, Object> attributes) throws Throwable {
		...
		// and its install count is 1. Check for a duplicate 
		// target breakpoint.
		// We need delays between operations to allow the UI 
		// updates to finish. We can't use BreakpointManager 
		// notifications here.
		...
		waitForInstallCountChange((CBreakpoint)plBpt, (!miBpts[0].isPending() ? 1 : 0));
  	...
  	private void waitForBreakpointEvent(Class<? extends IBreakpointsChangedEvent> eventType, int timeout) throws Exception {
  		System.out.println(String.format("%s Waiting for '%s' ...", DsfPlugin.getDebugTime(), eventType.getName()));
		if (!breakpointEventReceived(eventType)) {
			...
			if (!breakpointEventReceived(eventType)) {
				System.out.println(String.format("%s Event '%s' is timed out", DsfPlugin.getDebugTime(), eventType.getName()));
  	...
  	private void queueConsoleCommand(final String command, int timeout, TimeUnit unit) throws Throwable {
  		System.out.println(String.format("%s %s", DsfPlugin.getDebugTime(), command));
  	...
  	private void deletePlatformBreakpoint(final IBreakpoint plBpt) throws Throwable {
  		new Job("") {
			...
			protected IStatus run(IProgressMonitor monitor) {
		  		System.out.println("Deleting platform breakpoint...");
  	...
  	private synchronized boolean breakpointEventReceived(Class<? extends IBreakpointsChangedEvent> eventType) {
		for (IBreakpointsChangedEvent e : fBreakpointEvents) {
			if (eventType.isAssignableFrom(e.getClass())) {
				System.out.println(String.format("%s Event '%s' received", DsfPlugin.getDebugTime(), eventType.getName()));

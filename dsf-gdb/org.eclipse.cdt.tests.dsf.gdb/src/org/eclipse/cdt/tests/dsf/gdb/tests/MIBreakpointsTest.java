...
public class MIBreakpointsTest extends BaseTestCase {
     ...
     * ------------------------------------------------------------------------
     */
	private BigInteger evaluateExpression(IDMContext ctx, String expression)
			throws Throwable {
		// Get a stack context (temporary - should be an MIcontainerDMC)
		final IExpressionDMContext expressionDMC = SyncUtil.createExpression(
				ctx, expression);
		final FormattedValueDMContext formattedValueDMC = SyncUtil
				.getFormattedValue(fExpressionService, expressionDMC,
		...
		Query<FormattedValueDMData> query = new Query<IFormattedValues.FormattedValueDMData>() {
			...
			protected void execute(DataRequestMonitor<FormattedValueDMData> rm) {
				fExpressionService.getFormattedExpressionValue(
						formattedValueDMC, rm);
			}
		};

		fExpressionService.getExecutor().submit(query);

		return new BigInteger(query.get().getFormattedValue());
	}
     ...
     * ------------------------------------------------------------------------
     */
	protected IBreakpointDMContext[] getBreakpoints(
     ...
     * ------------------------------------------------------------------------
     */
	protected IBreakpointDMData getBreakpoint(
			final IBreakpointDMContext breakpoint) throws InterruptedException,
			ExecutionException {
	 ...
	 */
	protected void insertInvalidBreakpoint(
			final IBreakpointsTargetDMContext context,
	 ...
	 *            a string expected to be found in the exception message
	 */
	protected void removeInvalidBreakpoint(final IBreakpointDMContext breakpoint, String expectedMessage) throws InterruptedException {
		boolean exceptionThrown = false;
	 ...
	 *            a string expected to be found in the exception message
	 */
	protected void updateInvalidBreakpoint(final IBreakpointDMContext breakpoint,
	...
	protected void validateBreakpoint(MIBreakpointDMData breakpoint,
			String filename, int lineNumber, String condition, int ignoreCount,
			boolean state, boolean pending) {
		assertThat("filename", breakpoint.getFileName(), is(filename));
		assertThat("line number", breakpoint.getLineNumber(), is(lineNumber));
		assertThat("condition", breakpoint.getCondition(), is(condition));
		assertThat("ignore count", breakpoint.getIgnoreCount(), is(ignoreCount));
		assertThat("state", breakpoint.isEnabled(), is(state));
		assertThat("pending", breakpoint.isPending(), is(pending));
	}

	protected void validateBreakpoint(MIBreakpointDMData breakpoint,
			String filename, String functionName, String condition,
			int ignoreCount, boolean state, boolean pending) {
		assertThat("filename", breakpoint.getFileName(), is(filename));
		assertThat("functionName", breakpoint.getFunctionName(), is(functionName));
		assertThat("condition", breakpoint.getCondition(), is(condition));
		assertThat("ignore count", breakpoint.getIgnoreCount(), is(ignoreCount));
		assertThat("state", breakpoint.isEnabled(), is(state));
		assertThat("pending", breakpoint.isPending(), is(pending));
	...
	protected void validateBreakpoint(MIBreakpointDMData breakpoint,
			...
			boolean state, boolean pending) {
		assertThat("address", breakpoint.getAddresses()[0].getValue(), is(address));
		assertThat("condition", breakpoint.getCondition(), is(condition));
		assertThat("ignore count", breakpoint.getIgnoreCount(), is(ignoreCount));
		assertThat("state", breakpoint.isEnabled(), is(state));
		assertThat("pending", breakpoint.isPending(), is(pending));
	...
	protected void validateWatchpoint(MIBreakpointDMData watchpoint,
			String expression, boolean read, boolean write, boolean access,
			boolean state, boolean pending) {
		assertThat("expression", watchpoint.getExpression(), is(expression));
		assertThat("read state", watchpoint.isReadWatchpoint(), is(read));
		assertThat("write state", watchpoint.isWriteWatchpoint(), is(write));
		assertThat("access state", watchpoint.isAccessWatchpoint(), is(access));
		assertThat("state", watchpoint.isEnabled(), is(state));
		assertThat("pending", watchpoint.isPending(), is(pending));
	}
	...
	public void insertBreakpoint_IgnoreCnt() throws Throwable {
		...
		// Ensure that the breakpoint was correctly installed
		MIBreakpointDMData breakpoint1 = (MIBreakpointDMData) getBreakpoint(ref);
		validateBreakpoint(breakpoint1, SOURCE_NAME, LINE_NUMBER_1, NO_CONDITION, LINE_NUMBER_1, true, false);
	...
	public void insertInvalidBreakpoint_WhileTargetRunning() throws Throwable {
		...
		insertInvalidBreakpoint(fBreakpointsDmc, breakpoint, "");

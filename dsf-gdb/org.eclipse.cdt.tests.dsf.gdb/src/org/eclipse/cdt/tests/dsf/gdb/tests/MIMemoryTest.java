...
public class MIMemoryTest extends BaseTestCase {
	...
	private void readMemoryByteAtOffset(final IMemoryDMContext dmc, final IAddress address,
			final long offset, final int fWordSize, final int count, final MemoryByte[] result)
	throws InterruptedException
	{
		...
		final DataRequestMonitor<MemoryByte[]> drm = 
			new DataRequestMonitor<MemoryByte[]>(fSession.getExecutor(), null) {
				...
				protected void handleCompleted() {
					if (isSuccess()) {
						// Save the results to the proper location as per address offset and fWordSize
						for (int i=0; i < fWordSize; i++) {
							result[(int) offset*fWordSize+i] = getData()[i];
						}
					}
		...
		fSession.getExecutor().submit(new Runnable() {
			@Override
			public void run() {
				fMemoryService.getMemory(dmc, address, offset, fWordSize, count, drm);
			}
		});
	}
	...
	private void writeMemory(final IMemoryDMContext dmc, final IAddress address,
			final long offset, final int fWordSize, final int count, final byte[] buffer)
	throws InterruptedException
	{
		// Set the Data Request Monitor
		...
		fSession.getExecutor().submit(new Runnable() {
			@Override
			public void run() {
				fMemoryService.setMemory(dmc, address, offset, fWordSize, count, buffer, rm);
			}
		});
	}
	...
	public void fillWithInvalidAddress() throws Throwable {
		...
		byte[] pattern = ByteBuffer.allocate(count * fWordSize).put(valueToBytes(count)).array();
		fBaseAddress = new Addr64("0");		

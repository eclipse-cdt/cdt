...
public class MIRegistersTest extends BaseTestCase {
	...
	private IRegisterGroupDMContext getTargetRegisterGroup() throws Throwable {
		Query<IRegisterGroupDMContext> queryTargetGroupCtx = new Query<IRegisterGroupDMContext>() {
			@Override
			protected void execute(DataRequestMonitor<IRegisterGroupDMContext> rm) {
				fRegService.findTargetRegisterGroup(fCompositeDmc, rm);
			}
		};

		fRegService.getExecutor().execute(queryTargetGroupCtx);

		IRegisterGroupDMContext regGroupsDMC = queryTargetGroupCtx.get(500, TimeUnit.MILLISECONDS);
	...
	private IRegisterDMContext[] getAllRegisters(final IFrameDMContext frameDmc) throws Throwable {
		...
		final IRegisterGroupDMContext targetsGroup = getTargetRegisterGroup();
		...
		Query<IRegisterDMContext[]> queryRegisters = new Query<IRegisterDMContext[]>() {
			...
			protected void execute(DataRequestMonitor<IRegisterDMContext[]> rm) {
				fRegService.getRegisters(new CompositeDMContext(new IDMContext[] { targetsGroup, frameDmc }), rm);
			}
		};
    ...
    private IRegisterDMContext[] getTargetRegisters(final IFrameDMContext frameDmc) throws Throwable {
    	final IRegisterGroupDMContext targetGroupDMC = getTargetRegisterGroup();
    	
    	IRegisterDMContext[] regContexts = getRegisters(new CompositeDMContext(new IDMContext[] { targetGroupDMC, frameDmc}));
    	assertEquals("Wrong number of registers", get_X86_REGS().size(), regContexts.length);
	...
	private IRegisterDMData[] getRegisterNames(final IRegisterDMContext[] registersDmc) throws Throwable {
		...
		Query<IRegisterDMData[]> queryRegistersData = new Query<IRegisterDMData[]>() {
			@Override
			protected void execute(DataRequestMonitor<IRegisterDMData[]> rm) {
				fRegService.getRegistersData(registersDmc, rm);
			}
		};

		fRegService.getExecutor().submit(queryRegistersData);
		IRegisterDMData[] regNames = queryRegistersData.get(500, TimeUnit.MILLISECONDS);
	...
	private IRegisterGroupDMData[] getRegisterGroupsData(final IDMContext dmc) throws Throwable {
		Query<IRegisterGroupDMData[]> groupDataQ = new Query<IRegisterGroupDMData[]>() {
			@Override
			protected void execute(DataRequestMonitor<IRegisterGroupDMData[]> rm) {
				fRegService.getRegisterGroupsData(dmc, rm);
			}
		};

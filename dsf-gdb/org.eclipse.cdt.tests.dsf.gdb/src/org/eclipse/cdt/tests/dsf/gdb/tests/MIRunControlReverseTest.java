...
public class MIRunControlReverseTest extends BaseParametrizedTestCase {
	...
	/**
     * This test verifies that we properly handle a query send from GDB.
     * When running the old console (GDB < 7.12) GDB answer's itself and
     * so we should not answer.  When using the new Console, GDB does
	...
	public void testQueryHandling() throws Throwable {
		...
		try {
			writeQuery.get(2, TimeUnit.SECONDS);

...
public class RunGDBScriptTest extends BaseTestCase {
	...
	public void runGDBScript() throws Throwable {
        ...
        ServiceEventWaitor<ISuspendedDMEvent> suspendedEventWaitor = 
        	new ServiceEventWaitor<ISuspendedDMEvent>(getGDBLaunch().getSession(), ISuspendedDMEvent.class);

		fSession.getExecutor().submit(new DsfRunnable() {			
			@Override
			public void run() {
				fRunControl.canRunGDBScript(fContainerDmc, new ImmediateDataRequestMonitor<Boolean>() {
					@Override
					protected void handleCompleted() {
						if (isSuccess()) {
							if (getData()) {
								...
								wait.waitFinished(new Status(IStatus.ERROR, TestsPlugin.PLUGIN_ID, "Can not run GDB script"));
							}
						}
						else {
							wait.waitFinished(getStatus());

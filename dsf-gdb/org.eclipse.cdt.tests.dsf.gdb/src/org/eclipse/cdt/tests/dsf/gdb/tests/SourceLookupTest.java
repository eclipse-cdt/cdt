...
public class SourceLookupTest extends BaseParametrizedTestCase {
	...
	public void twoLaunchesTerminate() throws Throwable {
		 ...
		 * JUnit test thread, to simulate the behaviour and issues a query on a
		 * new thread to ensure that the executor is still responsive. Do the
		 * test repeatedly until after both sessions terminates. Note the
		 * executor thread hangs around by design after the session terminates
		 * until the launch is removed. (See GdbLaunch#launchRemoved(ILaunch))
		 */

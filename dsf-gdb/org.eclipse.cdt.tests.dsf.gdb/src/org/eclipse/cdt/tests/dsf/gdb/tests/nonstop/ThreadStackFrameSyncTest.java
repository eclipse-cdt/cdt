...
public class ThreadStackFrameSyncTest extends BaseParametrizedTestCase {
	...
	public void testGdbSyncServiceCanSwitchGDBThread() throws Throwable {
		...
		fGdbSync.setFocus(new IDMContext[] {getContextForThreadId(5)});
		assertEquals("5", getCurrentThread());
		
		fGdbSync.setFocus(new IDMContext[] {getContextForThreadId(4)});
		assertEquals("4", getCurrentThread());
		
		fGdbSync.setFocus(new IDMContext[] {getContextForThreadId(3)});
		assertEquals("3", getCurrentThread());
		
		fGdbSync.setFocus(new IDMContext[] {getContextForThreadId(2)});
		assertEquals("2", getCurrentThread());
		
		fGdbSync.setFocus(new IDMContext[] {getContextForThreadId(1)});
	...
	public void testGdbSyncServiceCanSwitchGDBStackFrame() throws Throwable {
		...
		for (int i = 0; i < 5; i++) {
			...
			fGdbSync.setFocus(new IDMContext[] {SyncUtil.getStackFrame(1, 1)});
			assertEquals("1", getCurrentStackFrameLevel());

			// have the sync service switch stack frame to 0
			fGdbSync.setFocus(new IDMContext[] {SyncUtil.getStackFrame(1, 0)});

...
public class CSourceNotFoundDescriptionFactory implements IAdapterFactory {
			...
			return (T) new ICSourceNotFoundDescription() {
				...
				public String getDescription() {
					...
					if (session != null && session.getExecutor() != null) {
						...
						try {
							...
							return getFrameDescription(dmData)[0];
						} catch (Exception e) {
				...
				public boolean isAdress() {
					...
					if (session != null && session.getExecutor() != null) {
						...
						try {
							dmData = query.get();
							if (getFrameDescription(dmData)[1].equals("true")) { //$NON-NLS-1$
								return true;
							}
				...
				private void createQuery(DataRequestMonitor<IFrameDMData> rm) {
	...
	private static String[] getFrameDescription(IStack.IFrameDMData frame) {
		...
		// acts like a boolean, takes values true and false
		String isAdress = "false"; //$NON-NLS-1$
		...
		String description = new MessageFormat(formatString).format(propertyValues, new StringBuffer(), null)
				.toString();
		String[] returnValues = { description, isAdress };
		return returnValues;
	}

...
public class DsfRegisterGroupActions implements IRegisterGroupActions {
	...
	public void addRegisterGroup(final IWorkbenchPart part, final IStructuredSelection selection) throws DebugException {
		...
		RequestMonitor asyncRequest = new RequestMonitor(selectionContext.fsession.getExecutor(), null) {
			...
			protected void handleSuccess() {
				...
				registersService.findRegisterGroup(selectionContext.fcontext, ROOT_GROUP_NAME, new DataRequestMonitor<IRegisterGroupDMContext>(
						selectionContext.fsession.getExecutor(), null) {
					@Override
					protected void handleSuccess() {
	...
	/* (non-Javadoc)
	 * @see org.eclipse.cdt.debug.internal.ui.actions.IRegisterGroupActions#canAddRegisterGroup(org.eclipse.ui.IWorkbenchPart, org.eclipse.jface.viewers.IStructuredSelection)
	 */
	@Override
                ...
                Query<Boolean> query = new Query<Boolean>() {
                    ...
                    protected void execute(final DataRequestMonitor<Boolean> rm) {                        
                        ...
                        if (registersService != null) {
                        	registersService.canAddRegisterGroup(selectionContext.fcontext, new DataRequestMonitor<Boolean>(selectionContext.fsession.getExecutor(), null){
                        		@Override
								protected void handleSuccess() {
                        			rm.setData(getData());
                        			rm.done();
	...
	public void editRegisterGroup(final IWorkbenchPart part, IStructuredSelection selection) throws DebugException {
		...
		RequestMonitor asyncRequest = new RequestMonitor(selectionContext.fsession.getExecutor(), null) {
			...
			protected void handleSuccess() {
				...
				try {
					regServiceManager = selectionContext.resolveService();
				} catch (CoreException e) {
					failed(e);
					return;
				}

				// Get handle to the group context
				final IRegisterGroupDMContext groupDmc = DMContexts.getAncestorOfType(selectionContext.fcontext, IRegisterGroupDMContext.class);
				...
				// Getting the children of the selected group
						new DataRequestMonitor<IRegisterDMContext[]>(session.getExecutor(), null) {
							@Override
							@ConfinedToDsfExecutor("fExecutor")
							protected void handleSuccess() {
								// Get children Register Contexts
								final IRegisterDMContext[] childRegisters = getData();
								// resolve the root group context from any of the registers
								IRegisterGroupDMContext rootGroupContext = null;
								if (childRegisters != null && childRegisters.length > 0) {
									rootGroupContext = DMContexts.getAncestorOfType(childRegisters[0],
											IRegisterGroupDMContext.class);

									// Getting all available gdb registers from the root
									regServiceManager.getRegisters(rootGroupContext,
											new DataRequestMonitor<IRegisterDMContext[]>(session.getExecutor(), null) {
												@Override
												@ConfinedToDsfExecutor("fExecutor")
												protected void handleSuccess() {
													final IRegisterDMContext[] registers = getData();
													// resolve the names for all registers
													regServiceManager.getRegisterNames(registers,
															new DataRequestMonitor<IRegisterDMData[]>(session.getExecutor(),
																	null) {
																@Override
																protected void handleSuccess() {
																	final IRegisterDMData[] registerData = getData();
																	regServiceManager.getRegisterGroupData(groupDmc,
																			new DataRequestMonitor<IRegisterGroupDMData>(
																				...
																				protected void handleSuccess() {
																					...
																					// all and children registers
																					final DialogRegisterProvider descriptors = buildDescriptors(
																							groupName, registers, registerData,
																							childRegisters);
	...
	public boolean canEditRegisterGroup(IWorkbenchPart part, IStructuredSelection selection) {
		...
		// true if the first item selected is a RegisterGroup and its allowed to add a new group
		IDMContext context = selectionContext.fcontext;
		
		return ((context instanceof IRegisterGroupDMContext) && canAddRegisterGroup(part, selection));
	}
	...
	private void processAddRegisterGroup(final Shell shell, final SelectionDMContext selectionContext,
			final IRegisterGroupDMContext groupDmc, final IRegisterDMContext[] selectedRegisters, final IRegisters2 regServiceManager) {

		final DsfSession session = selectionContext.fsession;

		regServiceManager.getRegisters(groupDmc, new DataRequestMonitor<IRegisterDMContext[]>(session.getExecutor(),
				null) {
			@Override
			@ConfinedToDsfExecutor("fExecutor")
			protected void handleSuccess() {
							...
							protected void handleSuccess() {
								...
								regServiceManager.getRegisterGroupData(groupDmc,
										new DataRequestMonitor<IRegisterGroupDMData>(executor, null) {
											...
											protected void handleSuccess() {
												IRegisterGroupDMData groupData = getData();
												final String groupName = (groupData == null) ? BLANK_STRING : groupData
														.getName();
												// Create the Register descriptors
												final DialogRegisterProvider descriptors = buildDescriptors(groupName,
														registers, registerData, selectedRegisters);
												// request for the next unused name
												regServiceManager.proposeGroupName(new DataRequestMonitor<String>(executor, null) {
													@Override
													@ConfinedToDsfExecutor("fExecutor")
													protected void handleSuccess() {
														final String proposedGroupName = getData();
														// Create Dialog Resolve selection to DSF Registers
														getDialogSelection(
																shell,
																proposedGroupName,
																descriptors.getAllRegisters(),
																descriptors.getcheckedRegisters(),
																new DataRequestMonitor<IRegisterGroupDescriptor>(executor, null) {
																	@Override
																	protected void handleSuccess() {
																		try {
																			addRegisterGroup(regServiceManager, session,
																					getData());
																		} catch (CoreException e) {
																			failed(e);
																		}
	...
	private DialogRegisterProvider buildDescriptors(String groupName, IRegisterDMContext[] registers,
			IRegisterDMData[] registerData, IRegisterDMContext[] checkedRegisters) {
		assert (registers.length == registerData.length);

		List<IRegisterDMContext> selectedContextsList = Arrays.asList(checkedRegisters);
		...
		for (int i = 0; i < registers.length; i++) {
			...
			if (selectedContextsList.contains(registers[i])) {
				checkedDescriptorsList.add(regDescriptors[i]);
			}
		}

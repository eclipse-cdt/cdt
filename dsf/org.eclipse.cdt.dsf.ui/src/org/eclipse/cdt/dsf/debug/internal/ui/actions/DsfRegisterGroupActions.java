...
public class DsfRegisterGroupActions implements IRegisterGroupActionsTarget {
	...
	public void addRegisterGroup(final IWorkbenchPart part, final IStructuredSelection selection) throws DebugException {
		...
		selectionContext.fsession.getExecutor().execute(new DsfRunnable() {
			...
			public void run() {
				...
				// We need to resolve the group containing all the registers provided by target
				registersService.findTargetRegisterGroup(selectionContext.fcontext,
	...
	public void editRegisterGroup(final IWorkbenchPart part, IStructuredSelection selection) throws DebugException {
		...
		selectionContext.fsession.getExecutor().execute(new DsfRunnable() {
			...
			public void run() {
				...
				// We need to resolve the root group to obtain the full list of registers available in the target
				registersService.findTargetRegisterGroup(selectionContext.fcontext,
						new DataRequestMonitor<IRegisterGroupDMContext>(selectionContext.fsession.getExecutor(), null) {
							@Override
							protected void handleSuccess() {
	...
	private void processAddRegisterGroup(final Shell shell, final SelectionDMContext selectionContext,
		...
		regServiceManager.getRegisters(rootGroupDmc, new DataRequestMonitor<IRegisterDMContext[]>(
			...
			protected void handleSuccess() {
				...
				regServiceManager.getRegistersData(rootRegisters, new DataRequestMonitor<IRegisterDMData[]>(executor,
						null) {
					@Override
					protected void handleSuccess() {
						...
						regServiceManager.getRegistersData(selectedRegisters,
										...
										regServiceManager.getRegisterGroupData(rootGroupDmc,
												new DataRequestMonitor<IRegisterGroupDMData>(executor, null) {
													...
													protected void handleSuccess() {
														...
														proposeGroupName(rootRegisters, regServiceManager, new DataRequestMonitor<String>(
																		executor, null) {
																	@Override
																	protected void handleSuccess() {
	...
	private void proposeGroupName(IRegisterDMContext[] registers, IRegisters2 regServiceManager, final DataRequestMonitor<String> rm) {
		assert(registers != null && registers.length > 0);
		if (registers != null && registers.length > 0) {
			//Any valid register context can be used to resolve the container context expected by the service
			regServiceManager.getRegisterGroupsData(registers[0], new DataRequestMonitor<IRegisterGroupDMData[]>(regServiceManager.getExecutor(), rm){
				@Override
				protected void handleSuccess() {
					//Got the list of Register Group Names
					IRegisterGroupDMData[] groupsData = getData();
					...
					protected void handleSuccess() {
						...
						regServiceManager.getRegisters(rootGroupDmc, new DataRequestMonitor<IRegisterDMContext[]>(
							...
							protected void handleSuccess() {
								...
								regServiceManager.getRegistersData(rootRegisters,
										new DataRequestMonitor<IRegisterDMData[]>(executor, null) {
											@Override
											protected void handleSuccess() {
												...
												regServiceManager.getRegistersData(childRegisters,
														new DataRequestMonitor<IRegisterDMData[]>(executor, null) {
															@Override
															protected void handleSuccess() {
																...
																regServiceManager.getRegisterGroupData(groupDmc,
																		new DataRequestMonitor<IRegisterGroupDMData>(
																			...
																			protected void handleSuccess() {
																				...
																				String groupName = (groupData == null) ? BLANK_STRING
																						: groupData.getName();

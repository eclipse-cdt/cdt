...
public final class RulerJumpToMemoryAction extends AbstractDisassemblyRulerAction {
	...
	public void run() {
		...
		if (activePage == null) {
			reportException(new Exception("Failed to retrieve active workbench page.")); //$NON-NLS-1$
			return;
		}

		IViewPart viewPart = activePage.findView(MEMORY_VIEW_ID);
		if (viewPart != null) {
			activePage.activate(viewPart);
		} else {
			try {
				viewPart = activePage.showView(MEMORY_VIEW_ID);

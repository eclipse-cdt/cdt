...
public class DisassemblyDocument extends REDDocument implements IDisassemblyDocument {
	 ...
	 * Map (i.e. Map<?>), by this class it was treated sometimes as a
	 * Map<String, List<Position>> and sometimes as Map<String, List
	 * <AddressRangePosition>>. Once the generification happened, the latter
	 * case became a compile error.
	 *
	 * An earlier fix made a copy of the list to work around the problem,
	 * however many of the methods that work on the positions expected access to
	 * the real list.
	 *
	 ...
	 * requires a double-cast to avoid the compilation error, hence the three
	...
	private List<AddressRangePosition> getAddressRangePositions(String category) {
		...
		List<AddressRangePosition> positions = (List<AddressRangePosition>) (List) list;
		return positions;
	}

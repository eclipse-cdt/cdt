...
public class DsfSourceSelectionResolver implements Runnable  {
	...
	private LineLocation fLineLocation = null;
		...
		if (fEditorPage != null) {
			...
			if (selection != null) {
				...
				if (selectedFunctions == null || selectedFunctions.length != 1 || selectedFunctions[0] == null) {
					DsfUIPlugin.debug("Unable to resolve selection: " + selection.getText() + " to a function"); //$NON-NLS-1$ //$NON-NLS-2$
					return;
				} else {
	...
	private ITextEditor resolveEditor() {
		if (fEditorPage != null) {
			return fEditorPage;
		}
	...
	private ITextSelection resolveSelection() {
		if (fSelection != null) {

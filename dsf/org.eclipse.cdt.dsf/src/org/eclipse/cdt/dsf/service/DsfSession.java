...
public class DsfSession {
	...
	static {
		DEBUG_SESSION = DsfPlugin.DEBUG
				&& Boolean.parseBoolean(Platform.getDebugOption("org.eclipse.cdt.dsf/debug/session")); //$NON-NLS-1$
		DEBUG_SESSION_LISTENERS = DEBUG_SESSION
				&& Boolean.parseBoolean(Platform.getDebugOption("org.eclipse.cdt.dsf/debug/session/listeners")); //$NON-NLS-1$
		DEBUG_SESSION_DISPATCHES = DEBUG_SESSION
				&& Boolean.parseBoolean(Platform.getDebugOption("org.eclipse.cdt.dsf/debug/session/dispatches")); //$NON-NLS-1$
		DEBUG_SESSION_MODELADAPTERS = DEBUG_SESSION
	...
	public void addServiceEventListener(Object listener, Filter filter) {
		...
		if (DEBUG_SESSION_LISTENERS) {
			...
			String msg = formatter.format("%s %s added as a service listener to %s (id=%s)", //$NON-NLS-1$
					DsfPlugin.getDebugTime(), LoggingUtils.toString(listener), LoggingUtils.toString(this), getId())
					.toString();
			formatter.close();
	...
	public void removeServiceEventListener(Object listener) {
		...
		if (DEBUG_SESSION_LISTENERS) {
			Formatter formatter = new Formatter();
			String msg = formatter.format("%s %s removed as a service listener to %s (id=%s)", //$NON-NLS-1$
					DsfPlugin.getDebugTime(), LoggingUtils.toString(listener), LoggingUtils.toString(this), getId())
					.toString();
			formatter.close();
	...
	public void dispatchEvent(final Object event, final Dictionary<?, ?> serviceProperties) {
		...
		if (DEBUG_SESSION_DISPATCHES) {
			Formatter formatter = new Formatter();
			String msg = formatter.format("%s Dispatching event %s to session %s from thread \"%s\" (%d)", //$NON-NLS-1$
					DsfPlugin.getDebugTime(), LoggingUtils.toString(event), LoggingUtils.toString(this),
					Thread.currentThread().getName(), Thread.currentThread().getId()).toString();
			formatter.close();
	...
	public void registerModelAdapter(Class<?> adapterType, Object adapter) {
		if (DEBUG_SESSION_MODELADAPTERS) {
			Formatter formatter = new Formatter();
			String msg = formatter.format("%s Registering model adapter %s of type %s to session %s (%s)", //$NON-NLS-1$
					DsfPlugin.getDebugTime(), LoggingUtils.toString(adapter), adapterType.getName(),
					LoggingUtils.toString(this), getId()).toString();
			formatter.close();
	...
	public void unregisterModelAdapter(Class<?> adapterType) {
		if (DEBUG_SESSION_MODELADAPTERS) {
			Formatter formatter = new Formatter();
			String msg = formatter.format("%s Unregistering model adapter of type %s from session %s (%s)", //$NON-NLS-1$
					DsfPlugin.getDebugTime(), adapterType.getName(), LoggingUtils.toString(this), getId()).toString();
			formatter.close();
	...
	private void doDispatchEvent(Object event, Dictionary<?, ?> _serviceProperties) {
		...
		for (Map.Entry<ListenerEntry, List<Method>> entry : listeners.entrySet()) {
			for (Method method : entry.getValue()) {
				try {
					if (DEBUG_SESSION_DISPATCHES) {
						DsfPlugin.debug(DsfPlugin.getDebugTime() + " Listener " //$NON-NLS-1$
								+ LoggingUtils.toString(entry.getKey().fListener) + " invoked with event " //$NON-NLS-1$
	...
	private Method[] getEventHandlerMethods(Object listener) {
			...
			for (Method method : methods) {
				if (method.isAnnotationPresent(DsfServiceEventHandler.class)) {
					...
					if (paramTypes.length != 1) { // must have one and only param
						throw new IllegalArgumentException(
								"@DsfServiceEventHandler method has incorrect number of parameters"); //$NON-NLS-1$
					}
					retVal.add(method);
				}
			}
		...
		if (retVal.isEmpty()) {
			throw new IllegalArgumentException(
					"No methods annotated with @DsfServiceEventHandler in listener, is listener declared public?"); //$NON-NLS-1$
		}

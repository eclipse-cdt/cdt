...
public class PDABreakpoints extends AbstractDsfService implements IBreakpoints {
	...
	private void doInsertWatchpoint(final Map<String, Object> attributes,
			final DataRequestMonitor<IBreakpointDMContext> rm) {
		...
		final WatchpointDMContext watchpointCtx = new WatchpointDMContext(getSession().getId(),
				fCommandControl.getContext(), function, variable);
	...
	private void doRemoveBreakpoint(BreakpointDMContext bpCtx, RequestMonitor rm) {
		...
		fCommandControl.queueCommand(new PDAClearBreakpointCommand(fCommandControl.getContext(), bpCtx.fLine),
				new DataRequestMonitor<PDACommandResult>(getExecutor(), rm));
	}
	...
	private void doRemoveWatchpoint(WatchpointDMContext bpCtx, RequestMonitor rm) {
		...
		fCommandControl.queueCommand(
				new PDAWatchCommand(fCommandControl.getContext(), bpCtx.fFunction, bpCtx.fVariable,

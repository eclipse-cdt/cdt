...
public class TriggerCellModifier implements ICellModifier {
	...
	public void modify(Object element, String property, Object value) {
		...
		if (value instanceof String) {
			try {
				intValue = Integer.valueOf(((String) value).trim());
	...
	private synchronized AlarmService getService(TriggerDMContext dmc) {
		...
		if (fServiceTracker == null) {
			try {
				fServiceTracker = new ServiceTracker(DsfExamplesPlugin.getBundleContext(),
						DsfExamplesPlugin.getBundleContext().createFilter(serviceId), null);

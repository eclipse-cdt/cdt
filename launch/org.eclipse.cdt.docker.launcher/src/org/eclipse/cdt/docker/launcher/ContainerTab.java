...
import org.eclipse.linuxtools.docker.ui.SWTImagesFactory;
...
public class ContainerTab extends AbstractLaunchConfigurationTab implements
	...
	public void setDefaults(ILaunchConfigurationWorkingCopy configuration) {
		configuration.setAttribute(ILaunchConstants.ATTR_ADDITIONAL_DIRS,
				(String) null);
		configuration.setAttribute(ILaunchConstants.ATTR_CONNECTION_NAME, ""); //$NON-NLS-1$
		Preferences prefs = InstanceScope.INSTANCE
				.getNode(org.eclipse.linuxtools.docker.ui.Activator.PLUGIN_ID);
		...
		Preferences prefs2 = InstanceScope.INSTANCE
				.getNode(Activator.PLUGIN_ID);
		Boolean keepContainer = prefs2.getBoolean(
				PreferenceConstants.KEEP_CONTAINER_AFTER_LAUNCH, false);
	...
	public void initializeFrom(ILaunchConfiguration configuration) {
		try {
			...
			connectionName = configuration.getAttribute(
					ILaunchConstants.ATTR_CONNECTION_NAME, (String) null);
	...
	public void performApply(ILaunchConfigurationWorkingCopy configuration) {
		...
		configuration.setAttribute(ILaunchConstants.ATTR_IMAGE, image);
		configuration.setAttribute(ILaunchConstants.ATTR_CONNECTION_NAME,
				connectionName);
		configuration.setAttribute(ILaunchConstants.ATTR_KEEP_AFTER_LAUNCH,
	...
	public void changeEvent(int type) {
		...
		for (int i = 0; i < connections.length; ++i) {
			connectionNames[i] = connections[i].getName();
			if (connectionNames[i].equals(currName))
				index = i;
		}

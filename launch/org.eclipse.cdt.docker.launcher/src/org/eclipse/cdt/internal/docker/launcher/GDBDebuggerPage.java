...
public class GDBDebuggerPage extends AbstractCDebuggerPage implements Observer {
	...
	final protected String[] protocolItems = new String[] {
			DEFAULT_MI_PROTOCOL, "mi1", "mi2", "mi3" }; //$NON-NLS-1$//$NON-NLS-2$ //$NON-NLS-3$
	...
	public void setDefaults(ILaunchConfigurationWorkingCopy configuration) {
		configuration.setAttribute(
				IMILaunchConfigurationConstants.ATTR_DEBUG_NAME, "gdb"); //$NON-NLS-1$
		configuration.setAttribute(
				IMILaunchConfigurationConstants.ATTR_GDB_INIT,
				IMILaunchConfigurationConstants.DEBUGGER_GDB_INIT_DEFAULT);
		configuration.setAttribute(
				IMILaunchConfigurationConstants.ATTR_DEBUGGER_PROTOCOL, "mi"); //$NON-NLS-1$
		if (fSolibBlock != null)
	...
	public void performApply(ILaunchConfigurationWorkingCopy configuration) {
		...
		str.trim();
		configuration.setAttribute(
				IMILaunchConfigurationConstants.ATTR_DEBUG_NAME, str);
		str = fGDBInitText.getText();
		str.trim();
		configuration.setAttribute(
				IMILaunchConfigurationConstants.ATTR_GDB_INIT, str);
		str = fProtocolCombo.getText();
		if (str.compareTo(DEFAULT_MI_PROTOCOL) == 0) {
			str = "mi"; //$NON-NLS-1$
		}
		configuration.setAttribute(
				IMILaunchConfigurationConstants.ATTR_DEBUGGER_PROTOCOL, str);
		configuration
				.setAttribute(
						IMILaunchConfigurationConstants.ATTR_DEBUGGER_FULLPATH_BREAKPOINTS,
						fBreakpointsFullPath.getSelection());
		if (fSolibBlock != null)
	...
	public IMILaunchConfigurationComponent createSolibBlock(Composite parent) {
		IMILaunchConfigurationComponent block = MIUIUtils.createGDBSolibBlock(
				true, true);
	...
	public void createMainTab(TabFolder tabFolder) {
		...
		tabItem.setText(Messages.GDBDebuggerPage2); //$NON-NLS-1$
		...
		Label label = ControlFactory.createLabel(subComp,
				Messages.GDBDebuggerPage3); //$NON-NLS-1$
		...
		Button button = createPushButton(subComp, Messages.GDBDebuggerPage4,
				null);
		button.addSelectionListener(new SelectionAdapter() {
			...
			private void handleGDBButtonSelected() {
				...
				dialog.setText(Messages.GDBDebuggerPage5);
		...
		label = ControlFactory.createLabel(subComp, Messages.GDBDebuggerPage6);
		gd = new GridData();
		// gd.horizontalSpan = 2;
		label.setLayoutData(gd);
		fGDBInitText = ControlFactory.createTextField(subComp, SWT.SINGLE
				| SWT.BORDER);
		...
		button = createPushButton(subComp, Messages.GDBDebuggerPage7, null);
		button.addSelectionListener(new SelectionAdapter() {
			...
			private void handleGDBInitButtonSelected() {
				FileDialog dialog = new FileDialog(getShell(), SWT.NONE);
				dialog.setText(Messages.GDBDebuggerPage8);
		...
		label = ControlFactory.createLabel(subComp, Messages.GDBDebuggerPage9,

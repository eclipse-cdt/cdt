...
public class LaunchBarManagerTest extends TestCase {
	...
	public void testRestart() throws Exception {
		...
		// Change the actives
		assertNotSame(desc1, manager.getActiveLaunchDescriptor());
		manager.setActiveLaunchDescriptor(desc1);
		assertNotSame(target1, manager.getActiveLaunchTarget());
		manager.setActiveLaunchTarget(target1);
		assertNotSame(runMode, manager.getActiveLaunchMode());
		manager.setActiveLaunchMode(runMode);

		// now create a new manager and check they are the same
		...
		assertNotNull(desc2);
		assertNotSame(desc2, manager.getActiveLaunchDescriptor());

...
public class NewLaunchTargetTypePage extends WizardPage {
	...
	public NewLaunchTargetTypePage() {
	...
	public void createControl(Composite parent) {
		...
		LaunchBarUIManager uiManager = ((NewLaunchTargetWizard) getWizard()).uiManager;
	...
	public IWizardPage getNextPage0() {
		@SuppressWarnings("unchecked") // These are the data objects I put there.
		ExecutableExtension<INewWizard> extension = (ExecutableExtension<INewWizard>) table.getSelection()[0].getData();
		if (extension != currentExtension) {
			try {
				((NewLaunchTargetWizard) getWizard()).wizardSelected(extension.create());
				currentExtension = extension;
			} catch (CoreException e) {
				Activator.log(e.getStatus());
			}

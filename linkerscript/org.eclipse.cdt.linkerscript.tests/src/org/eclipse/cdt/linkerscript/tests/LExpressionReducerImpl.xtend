...
class LExpressionReducerImpl implements LExpressionReducer {
	...
	override Map<String, Long> getMemorySizesMap() {
		return memorySizes
	}
	...
	def LExpression reduce(LTernaryOperation exp) {
	...
	override public LExpression reduce(LExpression exp) {
		return switch (exp) {

...
public class MemoryTableViewer extends AbstractLinkerScriptViewer {
	...
	public MemoryTableViewer(Composite parent, FormToolkit toolkit) {
		...
		Table table = toolkit.createTable(control, SWT.MULTI);
		table.setLayoutData(GridDataFactory.fillDefaults().grab(true, false).hint(-1, 300).create());
		...
		nameViewerColumn.setLabelProvider(new MemoryLabelProvider(this::getModel, Memory::getName));
		...
		originViewerColumn.setLabelProvider(
				new MemoryLabelProvider(this::getModel, mem -> util.expressionToString(mem.getOrigin())));
		...
		lengthviewerColumn.setLabelProvider(
				new MemoryLabelProvider(this::getModel, mem -> util.expressionToString(mem.getLength())));

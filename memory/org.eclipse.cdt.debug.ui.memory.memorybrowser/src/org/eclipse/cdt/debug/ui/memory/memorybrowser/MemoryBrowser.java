...
public class MemoryBrowser extends ViewPart implements IDebugContextListener, IMemoryRenderingSite, IDebugEventSetListener, IMemoryBrowser, ILaunchesListener2
{
	...
	public void selectRendering(String renderingTypeId) {
		if (fStackLayout.topControl instanceof CTabFolder) {
			...
			CTabItem item = activeFolder.getSelection();
			if (item != null) {
				IMemoryRenderingType type = (IMemoryRenderingType) item.getData(KEY_RENDERING_TYPE);
				if (type.getId().equals(renderingTypeId)) {
					return;
				}
				
				IMemoryBlockRetrieval retrieval = (IMemoryBlockRetrieval) activeFolder.getData(KEY_RETRIEVAL);
				...
				// dispose renderings and memory blocks for tab
				disposeTab(item);				
				try {
					populateTabWithRendering(item, retrieval, context,
							memorySpaceID, expression, renderingTypeId);
					...
					// update memento with the new rendering type id
					saveMemento(activeFolder);
				}
				catch (CoreException e) {
					MemoryBrowserPlugin.getDefault().getLog().log(new Status(Status.ERROR,

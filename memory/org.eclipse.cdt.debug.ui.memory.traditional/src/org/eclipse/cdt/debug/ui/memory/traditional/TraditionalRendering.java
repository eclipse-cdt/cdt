...
public class TraditionalRendering extends AbstractMemoryRendering implements IRepositionableMemoryRendering, IResettableMemoryRendering, IMemoryRenderingViewportProvider, IModelChangedListener, IDebugContextListener
{
    ...
    class RegisterDataContainer {
        ...
        private final int fAddressableRange;
         ...
         *            - Register Name
         * @param value
         *            - Register String value representation expected to use radix 16, using invalid radix 16 characters will not return an error, <br>
         *            However the value returned by the getter will be zero
         */
        RegisterDataContainer(IRegisterDMContext regDmc, String name, String value) {
            this(regDmc, name, value, 1);
        }
         ...
         *            - Register String value representation expected to use radix 16, using invalid radix 16 characters will not return an error, <br>
         *            However the value returned by the getter will be zero
         * @param addressableRange
         *            Number of addressable units if an end address is known
         */
        RegisterDataContainer(IRegisterDMContext regDmc, String name, String value, int addressableRange) {
            fRegDmc = regDmc;
            fName = name;
            fValue = convertValue(value);
            fAddressableRange = addressableRange;
        }
        ...
        /**
         * Returns the number of addressable units to the end of range register if known
         */
        public int getAddressableRange() {
            return fAddressableRange;
        }
    }
        ...
        executor.execute(new DsfRunnable() {
            ...
            public void run() {
                ...
                if (service != null && fLastSelectedFrame != null) {
                    ...
                    rm.done(new Status(IStatus.INFO, TraditionalRenderingPlugin.getUniqueIdentifier(),
                            "Unable to resolve registers for the currently selected context"));
                }
            }
        });
    }
    ...
    public void createMenus()
    {
        ...
        // display registers
        final Action displayRegistersAction = new Action(TraditionalRenderingMessages.getString("TraditionalRendering.ADDRESS_INFO_REGISTERS"), //$NON-NLS-1$
                IAction.AS_CHECK_BOX) {
            @Override
            public void run() {
                TraditionalRendering.this.fRendering.setExtraInfo(isChecked());
            }
        };

        boolean enabled = this.fRendering.getPaneVisible(Rendering.PANE_BINARY);
        if (!enabled) {
            this.fRendering.setExtraInfo(false);
        }

        displayRegistersAction.setEnabled(enabled);
        displayRegistersAction.setChecked(this.fRendering.isExtraInfo());
            ...
            IAction.AS_CHECK_BOX)
        {
			...
			public void run()
            {
                ...
                displayRegistersAction.setEnabled(isChecked());
            }
        };
...
//            .getTextMode() == Rendering.TEXT_UTF16);
        ...
        getPopupMenuManager().addMenuListener(new IMenuListener()
        {
            public void menuAboutToShow(IMenuManager manager)
            {
                ...
                sub = new MenuManager(TraditionalRenderingMessages.getString("TraditionalRendering.ADDRESS_INFO")); //$NON-NLS-1$
                sub.add(displayRegistersAction);
                manager.add(sub);

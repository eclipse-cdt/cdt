		...
		Job job = new Job("Memory Export to Plain Text File"){ //$NON-NLS-1$
			...
			public IStatus run(IProgressMonitor monitor) {
				...
				{	
					...
					final BigInteger outputChunkSize = BigInteger.valueOf(1);
					...
					final BigInteger dataChunksPerLine = outputChunkSize.multiply(numberOfColumns);
					
					BigInteger transferAddress = fStartAddress;
					
					FileWriter writer = new FileWriter(fOutputFile);
					
					BigInteger jobs = fEndAddress.subtract(transferAddress).divide(dataChunksPerLine);
					...
					{
						BigInteger length = dataChunksPerLine;
						...
						for(int i = 0; i < length.divide(outputChunkSize).intValue(); i++)
						{
							if(i != 0)
								buf.append(" "); //$NON-NLS-1$
							MemoryByte bytes[] = ((IMemoryBlockExtension) fMemoryBlock).getBytesFromAddress(
								transferAddress.add(outputChunkSize.multiply(BigInteger.valueOf(i))), 
								outputChunkSize.longValue());

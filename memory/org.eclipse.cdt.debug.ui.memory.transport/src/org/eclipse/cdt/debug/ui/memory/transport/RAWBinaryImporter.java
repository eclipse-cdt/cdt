...
import org.eclipse.cdt.debug.internal.ui.memory.transport.RAWBinaryImport;
...
public class RAWBinaryImporter implements IMemoryImporter {
	...
	public void importMemory() {
		...
		ImportRequest request = new ImportRequest(block, fStartAddress, new WriteMemoryBlock(block));
		RAWBinaryImport memoryImport = new RAWBinaryImport(fInputFile, request, scroll);
		FileImportJob job = new FileImportJob(//
				"Memory Import from RAW Binary File", //$NON-NLS-1$
				memoryImport);
		job.setUser(true);
		job.schedule();
	}

...
public class SRecordImporter implements IMemoryImporter {
	...
	public void importMemory() {
		Consumer<BigInteger> scroll = fScrollToStart ? new ScrollMemory(fParentDialog)
				: new ScrollMemory.Ignore();
		SRecordImport memoryImport = new SRecordImport(fInputFile, fStartAddress, (IMemoryBlockExtension) fMemoryBlock,
				scroll, fProperties.getBoolean(TRANSFER_CUSTOM_START_ADDRESS));
		Job job = new Job("Memory Import from S-Record File") {

			@Override
			public IStatus run(IProgressMonitor monitor) {
				try {
					memoryImport.run(monitor);

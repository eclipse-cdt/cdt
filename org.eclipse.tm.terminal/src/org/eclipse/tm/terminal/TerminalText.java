...
import org.eclipse.tm.terminal.TerminalCtrl.TerminalSerialPortHandler;


/**
 * This class processes character data received from the remote host and displays it to
 * the user using the Terminal view's StyledText widget.  This class processes ANSI
 * control characters, including NUL, backspace, carriage return, linefeed, and a
 * subset of ANSI escape sequences sufficient to allow use of screen-oriented
 * applications, such as vi, Emacs, and any GNU readline-enabled application (Bash, bc,
 * ncftp, etc.). <p>
 *
 * @author Fran Litterio <francis.litterio@windriver.com>
 * @author Chris Thew <chris.thew@windriver.com>
 */
class TerminalText extends Thread implements ControlListener
{
    ...
    /**
     * This is a character processing state: We've seen a ']' after an escape
     * character.  We are now expecting an operating system command that reprograms an
     * intelligent terminal.
     */
    protected static final int ANSISTATE_EXPECTING_OS_COMMAND = 3;

    /**
     * This field holds the current state of the Finite State Automaton (FSA) that
     * recognizes ANSI escape sequences.
     *
     * @see #processNewText()
     */
    ...
    /**
     * This field holds a reference to the {@link TerminalCtrl} object that
     * instantiates this class.
     */
    protected TerminalCtrl terminal;

    /**
     * This field holds a reference to the StyledText widget that is used to display
     * text to the user.
     */
    protected StyledText text;

    /**
     * This field holds the characters received from the remote host before they are
     * displayed to the user.  Method {@link #processNewText()} scans this text
     * looking for ANSI control characters and escape sequences.
     */
    protected StringBuffer newText;

    /**
     * This field holds the index of the current character while the text stored in
     * field {@link #newText} is being processed.
     */
    protected int characterIndex = 0;

    /**
     * This field holds the width of a character (in pixels) for the font used to
     * display text.
     */
    ...
    /**
     * This field holds the number of the column in which the cursor is logically
     * positioned.  The leftmost column on the screen is column 0, and column numbers
     * increase to the right.  The maximum value of this field is {@link
     * #widthInColumns} - 1.  We track the cursor column using this field to avoid
     * having to recompute it repeatly using StyledText method calls. <p>
     *
     ...
     * line is scrolled off the top edge of the screen. <p>
     */
     ...
     * methods {@link #setNewText(StringBuffer)} and {@link #clearTerminal()}. <p>
     *
     * IMPORTANT: This method must be called in strict alternation with method
     *            {@link #setNewText(StringBuffer)}. <p>
     */
     ...
     * ';'-separated sequence of numeric parameters. <p>
     */
     ...
     *
     * @return The <i>parameterIndex</i>th numeric ANSI parameter or -1 if the index
     *         is out of range.
     */
     ...
     * cursor is not at the very end of the screen's text. <p>
     *
     ...
     * wrapping text that extends past the right edge of the screen. <p>
     *
     * There are never any ANSI control characters or escape sequences in the text
     * being displayed by this method (this includes newlines, carriage returns, and
     * tabs). <p>
     ...
     * extends past the right edge of the screen. <p>
     *
     * There are never any ANSI control characters or escape sequences in the text
     * being displayed by this method (this includes newlines, carriage returns, and
     * tabs). <p>
     ...
     * as if a carriage return (CR) and a NL were written. <p>
     *
     ...
     * mean "move the cursor to the same column on the next line". <p>
     */
     ...
     * frequently, it must be fast when there is no resizing to be done. <p>
     */
    ...
    /**
     * This method returns a String containing <i>count</i> <i>ch</i> characters.
     *
     ...
     * <i>targetLine</i> is the line number of a screen line, so it has a minimum value
    ...
    /**
     * This method moves the cursor down <i>lines</i> lines, but won't move the cursor
    ...
    /**
     * This method moves the cursor up <i>lines</i> lines, but won't move the cursor
    ...
    /**
     * This method moves the cursor forward <i>columns</i> columns, but won't move the
    ...
    /**
     * This method moves the cursor backward <i>columnsToMove</i> columns, but won't

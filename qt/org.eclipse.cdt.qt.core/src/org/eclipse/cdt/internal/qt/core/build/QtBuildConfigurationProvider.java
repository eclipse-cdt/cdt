...
public class QtBuildConfigurationProvider implements ICBuildConfigurationProvider {
	...
	public ICBuildConfiguration getDefaultCBuildConfiguration(IProject project) {
		try {
			...
			IQtBuildConfiguration qtConfig = getConfiguration(project, localTarget, "run", new NullProgressMonitor()); //$NON-NLS-1$
			if (qtConfig == null) {
				qtConfig = createConfiguration(project, localTarget, "run", new NullProgressMonitor()); //$NON-NLS-1$
			}
			if (qtConfig != null) {
				return qtConfig;
			}

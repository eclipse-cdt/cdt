...
find * -name feature.xml -not -path */target/* | while read feature_xml; do
    ...
    feature_start_year=$(git log --reverse --format='%ad' --date="format:%Y" -- $feature_xml */$plugin | head -1)
    feature_end_year=$(git log --format='%ad' --date="format:%Y" -- $feature_xml */$plugin | head -1)

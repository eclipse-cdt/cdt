...
import org.eclipse.tm.internal.terminal.textcanvas.AnsiColorNames;
...
public class VT100Emulator implements ControlListener {
	 ...
	 */
	private ProcessExtendedColorsReturn processExtendedColors(CharSequence[] paramStrings, Style style,
			boolean colorspace) {
		...
		if (params.length > 1) {
			...
			case 2: // 24-bit RGB color
				...
				if (colorspace) {
					if (params.length < 6) {
						Logger.log(
								"Not enough parameter for 24-bit color depth, expected 5, one for color space and one for each of RGB"); //$NON-NLS-1$
					}
				} else {
					if (params.length < 5) {
						Logger.log("Not enough parameter for 24-bit color depth, expected 3, one for each of RGB"); //$NON-NLS-1$
					}
				}
			...
			case 5: // 8-bit color table lookup
				...
				if (foreground) {
					style = style.setForground(AnsiColorNames.table(index));
				} else {
					style = style.setBackground(AnsiColorNames.table(index));
				}

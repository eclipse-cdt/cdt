...
public class TerminalTextDataStore implements ITerminalTextData {
	...
	public int getCodePoint(int line, int column) {
		...
		if (fChars[line] == null || column >= fChars[line].getWidth())
			return 0;
		// TODO this assume all code points are not surrogate pairs
	...
	@Override
	public void setCodePoints(int line, int column, int[] codePoints, TerminalStyle style) {
		ensureLineLength(line, column + UnicodeCalc.width(codePoints));
		int c = 0;
		for (int i = 0; i < codePoints.length; i++) {
			fChars[line].setCodePointAt(column + c, codePoints[i], style);
			c += UnicodeCalc.width(codePoints[i]);
		}
	}

	@Override
	public void setCodePoints(int line, int column, int[] chars, int start, int numColumns, TerminalStyle style) {
		throw new UnsupportedOperationException();
		//		ensureLineLength(line, column + numColumns);
		//		for (int i = 0; i < numColumns; i++) {
		//			fChars[line].setCodePointAt(column + i, chars[i + start]);

...
abstract public class AbstractTextCanvasModel implements ITextCanvasModel {
	...
	public Point getHoverSelectionEnd() {
		if (!fHoverRange.isEmpty()) {
			...
			// use same convention as for selection
			end.x--;
			end.y--;
			return end;
		}
	...
	public void expandHoverSelectionAt(int line, int col) {
		...
		// TODO: consider wrapped lines
		char[] chars = fSnapshot.getChars(line);
		String lineText = chars == null ? "" : String.valueOf(chars); //$NON-NLS-1$
		if (lineText.length() <= col || isBoundaryChar(lineText.charAt(col))) {
			return;
		}
		int wordStart = 0;
		...
		for (int c = col; c < lineText.length(); c++) {
		...
		if (wordStart < wordEnd) {
			fHoverRange = new TextRange(line, line + 1, wordStart, wordEnd, lineText.substring(wordStart, wordEnd));
		}
	}
